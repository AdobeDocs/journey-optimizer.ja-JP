...
+ 基本を学ぶ {#get-started}
   ...
   + 効率的な作業 {#work-efficiently}
      ...
      + [ トラブルシューティングに関するFAQ](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/monitor/troubleshooting/troubleshooting)
...
+ テストと承認 {#test}
   ...
   + コンテンツのプレビューとテスト {#preview-test}
      ...
      + {hide-from-toc}[ コンテンツのバリエーションの自動生成](using/test-approve/simulate-sample-input-generated.md)
   ...
   + メールチャネル {#email}
      ...
      + メールコンテンツのデザイン {#design-email}
         ...
         + コンテンツのデザイン {#add-content}
            ...
            + [AIを活用した受信トレイへの電子メールの最適化](using/email/llm-email-optimizer.md)
...
+ コンテンツ管理 {#content-management}
   ...
   + パーソナライゼーション {#personalization}
      ...
      + ヘルパー関数リスト {#functions}
         ...
         + {hide-from-toc}[外部データ検索ヘルパー](using/personalization/external-data-lookup.md)
   ...
   + 再利用可能なコンテンツフラグメント {#fragments}
      ...
      + {hide-from-toc}[公開されたフラグメントにコンテキスト属性を追加](using/content-management/manage-fragments-contextual-attributes.md)
   + 動的コンテンツ {#dynamic}
   ...
   + クロスソリューションのユースケース {#combine}
      ...
      + {hide-from-toc}[MCP](/help/using/integrations/ajo-mcp.md)を介したAI ツールの操作
   + コンテンツ生成用 AI アシスタント {#ai-assistant}
      ...
      + [PERSONALIZATION Expressions用AI アシスタント](using/content-management/generative-personalization-expressions.md)
