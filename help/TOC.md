...
+ キャンペーン {#campaigns}
   ...
   + オーケストレーションキャンペーン {#orchestrated-campaigns}
      ...
      + 調整されたキャンペーンのアクティビティ {#design-campaigns}
         ...
         + {hide-from-toc}[ ファイルを読み込む](using/orchestrated/activities/load-file.md)
...
+ テストと承認 {#test}
   ...
   + コンテンツのプレビューとテスト {#preview-test}
      ...
      + {hide-from-toc}[ コンテンツのバリエーションのシミュレーション （GA レビュー） ](using/test-approve/simulate-sample-input-ga.md)
...
+ 通信チャネル {#channels}
   ...
   + メールチャネル {#email}
      ...
      + メールコンテンツのデザイン {#design-email}
         ...
         + コンテンツの作成 {#add-content}
            ...
            + [URLのパーソナライズ](using/email/url-personalization.md)
   ...
   + インボックス {#inbox}
      ...
      + [インボックスのデザイン](using/inbox/inbox-design.md)
   + WhatsApp {#whatsapp}
