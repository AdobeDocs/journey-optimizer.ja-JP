...
このページでは、外部エンドポイントのアクションを設定し、パーソナライゼーションエディターで`externalDataLookup` ヘルパーを使用して、インバウンドチャネルコンテンツをパーソナライズするために実行時にそのデータを動的に取得する方法について説明します。

**インテント**

* 外部エンドポイントを定義するアクション（URL、HTTP メソッド、パラメーター、リクエスト/応答スキーマ）の設定
* インバウンドアクションのパーソナライゼーション式に`externalDataLookup` ヘルパーを挿入する
* 変数ヘッダー、クエリ、ペイロード、またはパスパラメーターを呼び出し時に外部エンドポイントに渡します
* パーソナライゼーション式とヘルパー関数を使用して、結果エイリアスを介して取得したデータにアクセスする
* フォールバックコンテンツパターンで、タイムアウトとエラーを適切に処理
* Adobe Experience Platform Assuranceを使用した外部参照の問題のデバッグ

>[!TAB 用語集]

* **externalDataLookup**: パーソナライゼーションエディターのヘルパー関数で、インバウンドチャネルコンテンツのパーソナライゼーションで使用するために、設定された外部エンドポイントからリクエスト時にデータを動的に取得します。 *（製品固有）*
* **Action**：外部エンドポイントを定義するJourney Optimizerの設定オブジェクト（管理/設定）。URL、HTTP メソッド、ヘッダー/クエリパラメーター、POST ボディスキーマ、応答スキーマです。 `externalDataLookup`を使用する前に必要です。 *（製品固有）*
* **結果変数**: `externalDataLookup`呼び出しで割り当てられた任意のエイリアス。後続のパーソナライゼーション式で、取得した応答からすべてのフィールドを参照するために使用されます。
* **インバウンドチャネル**: ユーザーがサーフェスを開いたときに、コンテンツがオンデマンドで配信されるチャネル（コードベースのエクスペリエンス、web、アプリ内メッセージ）。 *（製品固有）*
* **AEP Edge Network**：実行時にパーソナライゼーションリクエストを受け取り、外部データルックアップコールをトリガーするインフラストラクチャ。

>[!TAB 用語]

* **正規名：** externalDataLookup — バリアント：外部データルックアップ、外部データルックアップヘルパー、外部データルックアップヘルパー
* **同義語：** 「externalDataLookup」 = 「外部データ参照ヘルパー」
* **パラメーター名（呼び出し時にエンドポイントに渡される変数値）とパラメーター名≠合わせて、** `actionId` （設定されたアクションのID、外部エンドポイントの識別）≠`result` （取得された応答データのエイリアス）を混同しないでください
* **インバウンドパーソナライゼーションアクションで`externalDataLookup`を使用する**&#x200B;と、ジャーニーアクティビティでカスタムアクションを使用する（ジャーニーフロー内のコンテンツを取得する）≠Edge Network リクエスト時にデータを動的に取得する）とを混同しないでください

>[!TAB  ガードレールと制限]

* 機能は限定的に利用できます。一部の組織でのみ利用できます。
* 外部エンドポイント呼び出しのデフォルトのタイムアウト：300 ミリ秒（デフォルト。特定のエンドポイントに対してこのタイムアウトを増やすには、Adobe担当者にお問い合わせください）。
* 応答スキーマの参照は、パーソナライゼーションエディターではサポートされていません。Journey Optimizerでは、式で使用される応答からJSON属性への参照を検証しません。
* ペイロード変数パラメーターでサポートされているデータ型：`String`、`Integer`、`Decimal`、`Boolean`、`listString`、`listInt`、`listInteger`、`listDecimal`。
* `externalDataLookup`個のヘルパーパラメーター内の変数置換は、現在サポートされていません。
* 動的URL パスは現在サポートされていません。
* アクション設定の認証オプションは、現在`externalDataLookup`でサポートされていません。回避策として、API キーベースまたはプレーンテキスト認証にヘッダーフィールドを使用してください。
* アクション設定への変更は、そのアクションを使用するライブキャンペーンまたはジャーニーには反映されません。変更を適用するには、ライブキャンペーン/ジャーニーをコピーまたは変更します。
* マルチパスレンダリングがサポートされています。
* Journey Optimizerは現在、外部エンドポイントの応答をキャッシュしません。
* 外部エンドポイントは、特定のサーフェスに対してAEP Edge Networkに送信されるインバウンドトラフィックと同じくらい、少なくとも同時の負荷とスループットを処理できる必要があります。

>[!TAB FAQ]

**Q：外部エンドポイントがタイムアウトした場合、またはエラーが返された場合はどうなりますか？**

結果変数は空になります。 結果内の属性参照は空白として表示され、配列の反復は項目を返しません。 これらのケースを適切に処理するには、フォールバックコンテンツパターン（単一属性の`?: "none found"`やコンテンツブロック全体の`{%#if result%}…{%else%}…{%/if%}`など）を使用します。

**Q: リクエストのコンテキスト属性をパラメーターとして外部データルックアップに渡すにはどうすればよいですか？**

パーソナライゼーションエディターのコンテキスト属性/データストリーム/イベントメニューを使用して、エクスペリエンスイベントスキーマを参照し、関連する属性をパラメーター値として挿入します（例：`query.myQueryParameter=context.datastream.event.<schemaId>.my.xdm.attribute`）。

**Q: Journey Optimizerは外部エンドポイントの応答をキャッシュしますか？**

現在はサポートされていません。 キャッシュは今後サポートされる予定です。

**Q: externalDataLookupの問題をデバッグする方法を教えてください。**

Adobe Experience Platform Assuranceの活用。 Assurance セッションを開始し、webまたはモバイル実装からJourney Optimizer呼び出しを開始し、Edge Delivery ビューを使用して、タイムアウトまたはエラーの詳細についてcustomActions ブロックを調べます。

**Q: externalDataLookupを使用してアクション設定で認証を使用できますか？**

アクション設定の認証オプションは現在サポートされていません。 API キーベースまたはその他のプレーンテキスト認証の場合は、アクション設定で資格情報をヘッダーフィールドとして指定します。
