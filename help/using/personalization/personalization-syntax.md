...
[!DNL Journey Optimizer] のPersonalizationは、Handlebars と呼ばれるテンプレート構文に基づいています。 Handlebars 構文の詳細については、[HandlebarsJS ドキュメント](https://handlebarsjs.com/) を参照してください。
...
これらのブロックヘルパーは、ヘルパー名の前にある `#` で識別され、一致する同じ名前の閉じ `/` が必要になります。
