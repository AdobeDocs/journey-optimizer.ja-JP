...
ヘルパーの構文は```{{#each ArrayName}}``` YourContentです `{{/each}}`
...
|---|---|
| `originalUrl` | 短縮するURL。 |
| `type` | リンクタイプ。 `DEEPLINK`を使用して、モバイルアプリで特定の画面を開きます。 |
| `action` | トラッキングアクション。 `CLICK`を使用して、リンク上のクリックを追跡します。 |
...
>
>現時点では、`datasetLookup` ヘルパー関数は、限られた顧客セットの式フラグメント内で使用できます。 アクセス権を取得するには、アドビ担当者にお問い合わせください。

`datasetLookup` ヘルパーは、パーソナライゼーション中にAdobe Experience Platform レコードデータセットからデータを取得するので、プロファイルまたはイベントペイロードに保存されていないフィールド値を使用できます。
