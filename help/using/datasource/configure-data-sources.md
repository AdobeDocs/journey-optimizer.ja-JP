...
フィールドグループが 1 つ以上のドラフトまたはライブジャーニーで使用されている場合は、選択したスキーマから新しいフィールドを増分的に追加できますが、既に選択されているフィールドを選択解除/削除/変更することはできません。 ドラフトジャーニーまたはライブジャーニーで既に使用されているスキーマの既存のフィールドを変更した場合（フィールドのデータタイプの変更など）は、フィールドグループの更新は許可されません。 これにより、ジャーニーが中断するのを回避できます
