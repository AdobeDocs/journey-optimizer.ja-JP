...
>[!CAUTION]
>
>カスタムアクションにカスタム認証を設定する場合、ネストされた JSON オブジェクト（`bodyParams` 内のサブオブジェクトなど）は現在 **サポートされていない** に注意してください。 フラットなキーと値のペアのみが最終的なリクエストペイロードに含まれます。 認証エンドポイントにネストされたオブジェクトが必要な場合、フィールドが見つからなくなり、認証が失敗する可能性があります。
