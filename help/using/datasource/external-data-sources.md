...
1. 外部サービスの設定に応じて認証を&#x200B;**[!UICONTROL 認証なし]**、**[!UICONTROL 基本]**、**[!UICONTROL カスタム]**&#x200B;または **[!UICONTROL API キー]**&#x200B;に設定します。
   ...
   >
   >* Adobe Journey Optimizerは、カスタムアクションで定義された秘密鍵を自動的に暗号化します。 各組織の暗号化キーは、組織に関連付けられた専用のコンテナで安全に管理されます。 認証情報がインターフェイスに表示される場合、誤って公開されないように、デフォルトではマスクされます。


   カスタム認証モードについて詳しくは、[ この節 ](../datasource/external-data-sources.md#custom-authentication-mode) を参照してください。 この例では、次に示すように、API キー認証モードを選択します。
...
カスタム認証モードは複雑な認証に使用され、OAuth2 などの API ラッピングプロトコルを呼び出すために頻繁に使用されます。これにより、アクションの実際の HTTP 要求に挿入するアクセストークンが取得されます。

カスタム認証を設定する場合は、「**[!UICONTROL クリックして認証を確認]**」ボタンを使用して、カスタム認証ペイロードが正しく設定されているかどうかを制御します。

![](assets/journey29-bis.png)

テストが正常に完了すると、ボタンが緑色に変わります。
