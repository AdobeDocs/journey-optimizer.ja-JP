...
| AJOセカンダリ受信者フィードバックイベントデータセット | 13 か月 | 該当なし |
...
+++TTL の対象となるデータセットのタイプはどれですか？

TTL は、時系列データセットにのみ適用されます。 レコードタイプのデータセット（エンティティデータセット、分類データセット、決定オブジェクトリポジトリなど）は TTL の対象ではないので、上記の影響を受けるデータセットの表には表示されません。
