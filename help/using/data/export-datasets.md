...
| 決定オブジェクトリポジトリ - パーソナライズされたオファー | ユーザーが作成するオファータイプのリポジトリー。 したがって、このデータセットには、このタイプのオファーに関する属性が含まれています。</br> 新しいカスタム属性フィールドをオファー項目スキーマに追加した後、これらの新しい属性がデータセットに表示されるまで、最大 1 時間の遅延が発生する場合があります。 潜在的なデータの損失や不整合を防ぐために、新しく追加された属性に依存する変更や更新を行う前に、少なくとも 1 時間待つことをお勧めします。 |
