...
| AJO メッセージフィードバックイベントデータセット | メッセージ配信ログ。レポートやオーディエンス作成を目的とした Journey Optimizer からのすべてのメッセージ配信に関する情報です。バウンスに関するメール ISP からのフィードバックも、このデータセットに記録されます。このデータセットには、メールを除くすべてのチャネル（SMS/MMS、ダイレクトメールなど）のイベントが含まれます。 |
