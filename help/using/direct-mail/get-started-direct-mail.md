...
ダイレクトメールキャンペーンまたはジャーニーを作成する場合、Journey Optimizerでは、すべてのターゲットプロファイルと、選択したデータ（住所やプロファイル属性など）を含むファイルを自動的に生成します。 このファイルは選択したサーバーに送信されるので、選択したサードパーティ（実際のメール送信プロセスを処理する）のダイレクトメールプロバイダーからアクセスできるようになります。
...
>[!AVAILABILITY]
>
>ダイレクトメールメッセージは、ジャーニーおよびスケジュールされたキャンペーンのコンテキストでのみ作成できます。 オーケストレートキャンペーンや API トリガーキャンペーンでは使用できません。
...
Adobe Journey Optimizerのダイレクトメールチャネルを活用して、ジャーニー内のダイレクトメール配信を自動化し、スケジュールする方法を説明します。
