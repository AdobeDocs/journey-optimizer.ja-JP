...
* **動的な項目属性** - 決定項目のカスタム属性を、プロファイル、コンテキスト、オーディエンスデータを使用して、配信時にパーソナライズできるようになりました。 これにより、わずかなコンテンツバリエーションのために重複するオファーを維持する必要がなくなり、マーケターはより少なく、より柔軟な決定項目を管理できます。
...
<thead>
<tr>
<th><strong>E メールデザイナーのモジュール</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>E メールデザイナーには、ヘッダー、製品カード、情報ブロック、フッターなど、すぐに使用できるレイアウトモジュールのライブラリが含まれ、メールキャンバスに直接ドラッグ＆ドロップできます。</p>
<p>各モジュールには、編集可能なプロパティ（画像、タイトル、テキスト、ボタン、リンク）が事前に設定され、WYSIWYG インターフェイスを通じて完全にカスタマイズできるので、ゼロから構造を作成する必要がなく、メール作成を高速化できます。</p>
<p>公開日：2026年6月下旬</p>
</td>
</tr>
</tbody>
...
* **ジャーニーとキャンペーン用のフォルダー** - ジャーニーやキャンペーンを&#x200B;**フォルダー**&#x200B;に整理できるようになり、インターフェイスのナビゲーションと管理が向上しました。
