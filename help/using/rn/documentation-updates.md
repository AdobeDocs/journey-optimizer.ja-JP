...
* 意思決定管理の新しいページが使用できるようになりました。 オファーコンテンツ、決定ルールおよびランキング式で式を作成する際にサポートされているすべての演算子、ヘルパー、関数を一覧表示します。 このリストを使用して、実行時エラーを回避します。 意思決定管理では、ドキュメントに記載された関数のみがサポートされます。 [詳細情報](../offers/personalization-editor-supported-functions.md)
