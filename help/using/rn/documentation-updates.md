...
* **リンクの追加とメッセージの追跡** ドキュメントが更新され、トラッキング識別子（urlID）の生成方法に関するガイダンスが追加されました。一意の urlID は、URL とラベルの両方が一意の場合にのみ割り当てられます。 複数のメール（または 1 つのメールで複数回）で同じ URL を追跡するには、ユーザーは類似した URL ごとに一意のラベルを使用する必要があります。使用しない [!DNL Journey Optimizer]、クリックされたリンクを特定できません。 [詳細情報](../email/message-tracking.md#insert-links)
