...
* Journey Optimizerで使用可能なAdobe統合が、「システムと環境の接続」セクションにリストされるようになりました。 [詳細情報](../integrations/ajo-integrations.md)
...
* コードベースのエクスペリエンスで意思決定を使用する際に、クライアント実装に次のフラグを追加する方法を説明する新しい [ ページ ](../code-based/code-based-decisioning-implementations.md) が追加されました。

   * コードベースのエクスペリエンスでテスト決定を行うための `dryRun` フラグの追加。 [詳細情報](../code-based/code-based-decisioning-implementations.md#code-based-test-decisions)
