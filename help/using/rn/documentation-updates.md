...
* ジャーニーでの追加の識別子の使用方法に関するドキュメントに、追加の ID を使用するジャーニーで終了条件が適用された場合のプロファイルの動作について詳しく説明した表が含まれるようになりました。 [詳細情報](../building-journeys/supplemental-identifier.md#exit-criteria)

* オーケストレートキャンペーンに使用する標準スキーマとリレーショナルスキーマを区別する方法について、スキーマの概要ドキュメントに情報を追加しました。 [詳細情報](../data/gs-data.md)
