...
* コードベースのエクスペリエンスサーフェスとは何か、サーフェス URI、サーフェスが任意の階層レベル（web ページ、モバイルアプリ、コンテンツの場所）でコンテナとして機能する方法、アプリケーション実装をチャネル設定に接続する方法などを説明する新しいページが使用可能になりました。 [詳細情報](../code-based/code-based-surface.md)

* キャンペーンのアクティブ化、配信の監視、変更の作成、キャンペーンの停止など、コードベースのエクスペリエンスを公開および管理する方法に関する新しいページが使用できるようになりました。 [詳細情報](../code-based/publish-code-based.md)

* コードベースのエクスペリエンスのテストに関する新しいページが追加され、テストプロファイルを使用してコンテンツをプレビューする方法、ブラウザー開発者ツールまたはモバイルデバッグツールを使用して実装を検証する方法、公開前にコンテンツが正しく表示されることを確認する方法が説明されました。 [詳細情報](../code-based/test-code-based.md)
