...
* **ディメンションを変更**&#x200B;アクティビティのドキュメントが更新され、このアクティビティは外部結合を使用し、ディメンション変更ステップですべてのレコードを保持する一方で、新しいターゲットディメンションに一致するプロファイルを持たないレコードは、メッセージ配信時に暗黙的に除外されることが明確になりました。 [詳細情報](../orchestrated/activities/change-dimension.md)

* **メールへの CC フィールドの追加**&#x200B;ドキュメントのガードレールが強化されました。 CC アドレスは同意や抑制に対してチェックされないことと、CC アドレスに送信されたメールの開封数とクリック数が送信分析の開封数とクリック数の合計に含まれることが指定されるようになりました。 [詳細情報](../configuration/cc-email-field.md)

* **チャネルアクティビティ**&#x200B;ドキュメントが更新され、**マーケティングメッセージとトランザクションメッセージ**&#x200B;という新しい節が追加されました。この節では、オプトイン要件、ビジネスルールの適用、チャネル設定タイプ、推奨されるユースケースなど、2 つのチャネルカテゴリ間の動作上の違いについて説明します。 [詳細情報](../orchestrated/activities/channels.md#marketing-vs-transactional)

* **分岐アクティビティ**&#x200B;ドキュメントが強化され、**例**&#x200B;という新しい節が追加されました。この節では、分岐アクティビティを使用して、単一のキャンペーン実行でオーディエンスをマーケティング用とトランザクション用の 2 つの並行したメール分岐に分割する方法について説明します。 [詳細情報](../orchestrated/activities/fork.md#fork-examples)

* **オーディエンスを作成アクティビティ**&#x200B;ドキュメントが強化され、ルールビルダーを使用してサブスクリプションプラン属性でプロファイルをフィルタリングする方法を示す新しい例が追加されました。 [詳細情報](../orchestrated/activities/build-audience.md#build-audience-examples)

* **オーケストレーションキャンペーンの基本を学ぶ**&#x200B;ページでは、**オーケストレーションキャンペーンの内部とは**&#x200B;でエントリレベルの&#x200B;**オーディエンスの作成／分岐／チャネル A + チャネル B** パターンを文書化し、分岐アクティビティページおよびマーケティングメッセージとトランザクションメッセージページへの相互参照を提供しています。 [詳細情報](../orchestrated/gs-orchestrated-campaigns.md#gs-ms-campaign-inside)

* 高度なHTML エディター&#x200B;**ページを使用した**&#x200B;電子メールコンテンツの編集が、コンテンツ管理セクションからドキュメントの&#x200B;**電子メール** セクションに移動されました。 このページでは、高度な HTML エディターが E メールデザイナーで、メールメッセージとメールコンテンツテンプレートに使用できることを文書化しています。 [詳細情報](../email/email-expert-mode.md)

* **オーケストレーションキャンペーンの開始と監視**&#x200B;ドキュメントが更新され、内部の公開時の実行シーケンスの詳細を説明する新しい節、キャンペーンライフサイクルステータステーブル、公開前チェックリスト、繰り返し以外のキャンペーンの送信確認警告が追加されました。 [詳細情報](../orchestrated/start-monitor-campaigns.md#publication-sequence)

* **オーディエンスを保存**&#x200B;アクティビティドキュメントが更新され、オーディエンスを保存アクティビティは公開時にメッセージアクティビティよりも常に前に実行されることを明確にするメモが追加されました。 [詳細情報](../orchestrated/activities/save-audience.md)

* **オーケストレーションキャンペーンに関する FAQ** に、新しい 3 つの Q&amp;A（公開時に内部で発生する内容、公開後にメッセージが送信されない理由の 7 つのポイントのチェックリスト、プロファイルスナップショット検索とリアルタイムのプロファイル解決の違い）が追加されました。 [詳細情報](../orchestrated/orchestrated-campaigns-faq.md)

* ジャーニーのトラブルシューティングドキュメントに、**[ブロックされたジャーニーインスタンスによって破棄されるイベント](../building-journeys/troubleshooting-execution.md#max-instance-stack-events-reached)**&#x200B;という新しい節が追加されました。この節では、`maxInstanceStackEventsReached` が破棄される理由、発生するタイミング、その軽減方法について説明します。 ガードレールとステップイベントフィールドリストページもそれに応じて更新されました。

* **決定ポリシーでのフラグメントの活用**&#x200B;ドキュメントに、**メール**&#x200B;チャネルに関するガードレールメモが追加されました。**[!UICONTROL コンテンツをシミュレート]**&#x200B;では決定項目の式フラグメントは表示されませんが、**[!UICONTROL 本配信前確認を送信]**&#x200B;とアクティブ化されたキャンペーンでは表示されます。 また、このページには、**[!UICONTROL ビジュアルフラグメント]**&#x200B;は決定項目に割り当てることができず、このコンテキストでは&#x200B;**式フラグメント**&#x200B;のみがサポートされているとも記載されています。 [詳細情報](../experience-decisioning/fragments-decision-policies.md)

## 2026 年 3 月 {#march-2026}

* **エクスペリエンス決定を使用したコードベースのエクスペリエンスのプレビュー**&#x200B;に関するドキュメントで、**[!UICONTROL コンテンツをシミュレート]**&#x200B;はコンテンツのプレビューのみであることが明確になりました。 オーサリングプレビューでは、ライブの Edge リクエストからのコンテキストデータはシミュレートされません。 [詳細情報](../code-based/test-code-based.md#preview-code-based)

* **Adobe Experience Platform データの使用**&#x200B;ドキュメントが更新されました。ガードレールには、現在の製品動作を反映して、データセット参照を連結できないという記述が削除されました。 [詳細情報](../data/lookup-aep-data.md)

* **プロファイルを更新**&#x200B;アクティビティドキュメントが更新され、1 回のアクションで最大 5 つのプロファイル属性を更新するためのサポートが文書化されました。 [詳細情報](../building-journeys/update-profiles.md)

* **オーディエンスを読み取り**&#x200B;アクティビティと&#x200B;**ジャーニープロパティ**&#x200B;のドキュメントが更新され、常時稼動の繰り返しジャーニーでの 91 日間のライフサイクルが明確になりました。 スケジュールの節では、終了日が設定されていない繰り返しジャーニーは 91 日を過ぎてもライブのままであることが明示的に確認されるようになり、グローバルタイムアウトに関する FAQ が拡張され、91 日間のプロファイル TTL と 91 日間のレポートウィンドウが区別されるようになりました。 [詳細情報](../building-journeys/read-audience.md#schedule)

* **データセット参照**&#x200B;アクティビティのドキュメントが更新され、ダウンストリームの条件アクティビティで `@datasetLookup{}` 構文が機能するには、参照キーを詳細設定モードで設定する必要があることが明確になりました。 「データセット参照が見つかりません」エラーの解決に関するガイダンスを含むトラブルシューティングの節が追加されました。 [詳細情報](../building-journeys/dataset-lookup.md#troubleshooting)

* **日時関数**&#x200B;ドキュメントが更新され、コンテキストイベント属性からタイムスタンプを書式設定する方法を示す新しい例が追加されました。これには、`toDateTime()` の要件、数値イベント ID のバックティック構文、PQL の「入力が一致しません」エラーに対する一般的なエラーコールアウトが含まれています。 [詳細情報](../personalization/functions/dates.md#format-date)

* **オーケストレーションキャンペーンのガードレールと制限**&#x200B;ドキュメントと&#x200B;**ソースコネクタの基本を学ぶ**&#x200B;ドキュメントが更新され、ファイルベースのデータキャプチャを変更の場合、「`_change_request_type`」フィールドが必須で、その値は大文字ではなく小文字の `u`（アップサート）または `d`（削除）にする必要があることが明確になりました。 [詳細情報](../orchestrated/guardrails.md)

* **リンクの追加とメッセージのトラッキング**&#x200B;ドキュメントが更新され、トラッキング識別子（urlID）の生成方法に関するガイダンスが追加されました。一意の urlID は、URL とラベルの両方が一意の場合にのみ割り当てられます。 複数のメール（または 1 つのメール内で複数回）をまたいで同じ URL を追跡するには、類似する URL ごとに一意のラベルを使用する必要があります。それ以外の場合、[!DNL Journey Optimizer] はクリックされたリンクを判断できません。 [詳細情報](../email/message-tracking.md#track-across-multiple-emails)

* **テストプロファイルの作成**&#x200B;ドキュメントが更新され、ID 記述子の要件に関する重要なメモが追加されました。データセットが削除および再作成される場合、スキーマはプライマリ ID フィールドに正しい ID 記述子を保持する必要があります。 これがないと、取り込みが正常に完了しても、取り込まれたプロファイルは `testProfile = true` としてフラグ付けされません。 トラブルシューティングチェックリストが追加されました。 [詳細情報](../audience/creating-test-profiles.md)

* **オーディエンスを読み取り**&#x200B;アクティビティドキュメントが更新され、**ビジネスイベント**&#x200B;アクティビティは、オーディエンスを読み取りをジャーニーの最初のアクティビティにする必要があるルールに対する例外であることが明確になりました。 また、オーディエンスターゲティングを制御するための高度な代替手段として、**最適化**&#x200B;アクティビティを参照するメモも追加されました。 [詳細情報](../building-journeys/read-audience.md)

* ジャーニーでの&#x200B;**ウェーブを使用した送信**&#x200B;が一般提供されるようになりました。 限定提供フラグがドキュメントから削除されました。 [詳細情報](../building-journeys/send-using-waves.md)

* **ジャンプ**&#x200B;アクティビティドキュメントが強化され、新しいデザイン戦略の節の&#x200B;**バイトサイズのサブジャーニー**&#x200B;が追加されました。この節では、複雑なエンドツーエンドのフローを、ジャンプアクティビティ経由で接続された、より小さく焦点を当てたサブジャーニーに分割する方法を説明します。 [詳細情報](../building-journeys/jump.md#jump-strategy)

* **タグ**&#x200B;ドキュメントが更新され、複雑な命名規則の代替手段としてタグカテゴリの使用に関するガイダンスが追加されました。 新しい節では、スケーラブルなジャーニー管理用のタグカテゴリの設定方法について説明します。 [詳細情報](../building-journeys/tags.md)

* **データソースについて**&#x200B;ドキュメントに、実務担当者が 3 つのデータアクセス戦略（カスタムアクション経由の外部データへのアクセス、プロファイルが有効になっていないデータセットの使用、プロファイル対応データセットの使用）から選択するのに役立つ新しい節が追加されました。 各オプションについては、トレードオフと推奨されるユースケースを使用して説明します。 [詳細情報](../datasource/about-data-sources.md#data-access-strategy)

* **プッシュ通知デザイン**&#x200B;ドキュメントが更新され、iOS でのユニバーサルリンクの動作を明確にするメモが追加されました。通知 URL がユニバーサルリンクとして登録されている場合、選択された Web URL アクションに関係なく、関連付けられたアプリが開きます。 ブラウザーを強制的に開く方法に関するガイダンスが追加されました。 [詳細情報](../push/design-push.md)

* 決定ドキュメントに、新しい **AI モデルの監視**&#x200B;ページが追加されました。 [!DNL Journey Optimizer] で、パーソナライズされた最適化モデルの健全性、トレーニングステータス、パフォーマンスを直接追跡する方法について説明します。 [詳細情報](../experience-decisioning/ranking/ai-model-observability.md)

* メールテンプレート用の&#x200B;**高度な HTML エディター**（エキスパートモード）が、限定提供で使用できるようになりました。 ドキュメントページが一般公開されました。 この機能を使用すると、E メールデザイナーからメールコンテンツテンプレートの生の HTML ソースを直接表示および編集できます。 [詳細情報](../email/email-expert-mode.md)

* **URL トラッキング**&#x200B;と&#x200B;**ジャーニーのトラブルシューティング**&#x200B;ドキュメントが更新され、クローズ済みのジャーニーでの `context.system.source.actionId` の動作が文書化されました。 クローズ済みまたは再公開されていないジャーニーでは、トラッキング URL に空の `{}` プレースホルダーが生成される場合があります。 ジャーニーを再公開するか、影響を受けるパラメーターを削除して問題を解決する方法に関するガイダンスが追加されました。 [詳細情報](../email/url-tracking.md)

* **Adobe Experience Platform データソース**&#x200B;ドキュメントが更新され、データソース設定では XDM 個人プロファイルベースのスキーマのみがサポートされているというメモが追加されました。 [詳細情報](../datasource/adobe-experience-platform-data-source.md)

* **データセットの有効期間（TTL）ガードレール**&#x200B;ドキュメントが強化され、TTL の対象となるデータセットを明確に特定するための新しい FAQ エントリが追加されました。 TTL は、時系列データセットにのみ適用されます。エンティティデータセット、分類データセット、決定オブジェクトリポジトリなどのレコードタイプデータセットは TTL の対象外で、ガードレールのロールアウトによる影響を受けません。 [詳細情報](../data/datasets-ttl.md)

* **ジャーニーのプロパティ**&#x200B;および&#x200B;**ジャーニーの一時停止**&#x200B;ドキュメントが更新され、ジャーニーの技術的な詳細で使用可能になった新しい一時停止フィールドと再開フィールドが文書化されました。 「**技術的な詳細をコピー**」ボタンには、既存の `pausedJourneySettings` ブロックに加えて、`lastPausedAt`、`lastPausedBy`、`lastPausedById`、`lastResumedAt`、`lastResumedBy`、`lastResumedById` が含まれるようになりました。 **ジャーニーの一時停止**&#x200B;ページに、ジャーニープロパティから一時停止と再開のタイムスタンプを直接表示する方法を説明する新しい節が追加されました。 [詳細情報](../building-journeys/journey-properties.md)

## 2026年2月 {#february-2026}

* 意思決定管理用の新しいページが使用できるようになりました。 パーソナライズエディターを使用してオファーコンテンツ（表示域）をパーソナライズする際にサポートされているすべての演算子、ヘルパー、関数を一覧表示します。 ランタイムエラーを回避するには、このリストを使用します。 Offer Decisioning でコンテンツをパーソナライズする際、文書化された関数のみがサポートされます。 [詳細情報](../offers/offer-library/personalization-editor-supported-functions.md)

* メールの&#x200B;**決定ポリシーを作成**&#x200B;および&#x200B;**メッセージでの決定ポリシーの使用**&#x200B;ドキュメントが更新されました。メール本文で同じオファーが複数の決定ポリシーにより選択できる場合、エンジンがオファーを重複排除する（各プレースメントに異なるオファーが渡される）というメモが追加されました。 同じオファーを複数のレースメント（例えば、ヘッダーとフッター）に表示するには、**決定出力を再利用**&#x200B;を使用します。 [詳細情報](../experience-decisioning/create-decision-policy.md)

* 決定項目ページが更新され、プッシュチャネルとカスタムイベントキャップに関する情報が追加されました。 [詳細情報](../experience-decisioning/items.md#capping)

* **ジャーニーのエクスペリエンスイベント参照**&#x200B;ドキュメントが更新され、非推奨（廃止予定）タイムラインが追加されました。2026年4月1日（PT）以降、過去 90 日間にジャーニー式でエクスペリエンスイベント属性を使用していない組織は、この機能へのアクセス権が付与されなくなります。 FAQ では、廃止タイムラインと影響を受ける対象者に焦点を当て、エクスペリエンスイベントスキーマページには、代替アプローチへの直接リンクが追加されました。 [詳細情報](../building-journeys/exp-event-lookup.md)

* **決定**&#x200B;ドキュメントが、Adobe Experience Platform データを使用した&#x200B;**データセットの参照**&#x200B;について更新されました。サポートされているチャネルのガードレールには、決定が使用可能なすべてのチャネル（コードベースのエクスペリエンス、メール、プッシュ、SMS、ジャーニーのコンテンツ決定アクティビティ）でデータセットの参照が機能することが記載されています。 限定提供およびパブリックベータ版のメモは、決定ルール、ランキング式、決定項目ページから削除されました。 [詳細情報](../experience-decisioning/aep-data-exd.md)

* 外部システム統合ページが更新され、カスタムデータソースとカスタムアクションへのリンクが追加されました。また、エグレスプロキシが、**カスタムアクション**&#x200B;から外部システムへのアウトバウンド呼び出しに対して静的 IP アドレスを提供することが明確になりました。 [詳細情報](../configuration/external-systems.md)

* ジャーニーのドライランドキュメントが明確になりました。ステップイベント属性の `inDryRun` と `dryRunID` は、ドライランモードの場合は `true`インスタンス ID を返し、テストまたはライブジャーニーの場合は `null` を返すことが文書化されました。 レポートクエリでドライランステップイベントを除外するためのガイダンスが、それに応じて更新されました。 [詳細情報](../building-journeys/journey-dry-run.md)

* **Web プッシュ**&#x200B;が一般提供されるようになりました。 プッシュ通知ドキュメントが再作成され、それに応じて更新されました（開始、デザイン、送信、作成）。 [詳細情報](../push/get-started-push.md)

* Web プッシュ設定ページがドキュメントに追加されました。 [詳細情報](../push/push-configuration-web.md)
...
* Decisioning migration API ドキュメントが更新されました。 [詳細情報](../experience-decisioning/decisioning-migration-api.md)

* **コンテンツ決定**&#x200B;アクティビティが一般提供されるようになりました。 コンテンツ決定アクティビティページが更新され、ステップイベントで使用可能な決定データに関する節が追加されました。 [詳細情報](../building-journeys/content-decision.md)

* ロイヤルティの課題の API ドキュメントへのリンクが、ロイヤルティの課題の節（開始、課題の作成、タスクの作成、ロイヤルティの課題へのアクセス）に追加されました。 [詳細情報](../loyalty-challenges/get-started.md)

* キャンペーン作成ウィザードのドキュメントでサポートされているチャネル情報が修正されました。 チャネルとオーケストレーションキャンペーンの基本を学ぶに関する FAQ ページが、それに応じて更新されました。 [詳細情報](../campaigns/get-started-with-campaigns.md)

* **ジャーニーの管理**&#x200B;および&#x200B;**承認**&#x200B;権限に関する権限ドキュメントが修正されました。 [詳細情報](../administration/ootb-permissions.md)

* AEM（Adobe Experience Manager）統合ドキュメントが更新され、名前が変更されました（AEM 動的コンテンツと AEM フラグメント）。 [詳細情報](../integrations/aem-fragments.md)

* 除外リストに新しい除外理由 **UnsubscribeLinkNotValid**（エラーコード 050081）が追加されました。 この除外は、リスト登録解除の宛先の件名の長さが RFC の制限である 998 文字の制限を超える場合に生成されます。 [詳細情報](../reports/exclusion-list.md)

* formatDate ヘルパー関数ドキュメントが強化され、この関数は日時フィールドタイプ（文字列ではない）を必要とするというメモと、日時フィールドの書式設定、文字列を最初に日付に変換する方法、曜日名を含む完全な日付、システム時刻からの動的な日付、小文字出力を含む曜日の書式設定など、複数の例が追加されました。 [詳細情報](../personalization/functions/dates.md#format-date)

* テキストバージョンのメールドキュメントが強化され、カスタムプレーンテキストと自動同期のどちらを使用するかの判断条件、実際のシナリオを使用した実用的な例、よくある質問を含む FAQ の節など、包括的なユースケースのガイダンスが追加されました。 [詳細情報](../email/text-version-email.md#when-to-use)

* E メールデザイナーのテーマドキュメントが更新され、Web Fonts のサポート制限と代替フォントの重要性に関する情報が追加されました。 [詳細情報](../email/apply-email-themes.md#themes-guardrails)

* 実行メタデータヘルパードキュメントに制限が追加され、アクションから除外されたプロファイルのメタデータはキャプチャされないことが明確になりました。 [詳細情報](../personalization/functions/helpers.md#execution-metadata)

* コードベースの実装サンプルドキュメントが更新され、決定での正確なトラッキングとアトリビューションのために、propositionAction に tokens フィールドが含まれるようになりました。 [詳細情報](../code-based/code-based-implementation-samples.md#client-side-how)

* RLトラッキングおよびリスト登録解除ドキュメントにメモが追加され、URL に追加される URL トラッキングパラメーターの順序はランダムで、制御できないことが明確になりました。 [詳細情報](../email/url-tracking.md)

## 2026年1月 {#january-2026}

* ライセンス使用状況ダッシュボードドキュメントが、**エンゲージメント可能なプロファイル**&#x200B;に関する最新のガイダンス（定義の詳細やトラブルシューティングのガイダンスを含む）により明確になりました。 [詳細情報](../audience/license-usage.md#what-is-engageable-profile)

* E メールデザイナーのテーマドキュメントにメモが追加され、Web Fonts のサポート制限が明確になりました。 [詳細情報](../email/apply-email-themes.md#themes-guardrails)
