...
* **キャンペーンとジャーニーの最適化**:
   ...
   * 実験を作成する際は、追跡する指標（コンバージョン、クリック数、カスタムイベント）を定義します。 Journey Optimizerは、各処理のパフォーマンスデータを自動的に収集します。 [詳細情報](../campaigns/optimization-experimentation.md)

   * ターゲティングルールを作成し、様々なコンテンツを様々なオーディエンスセグメントに配信します。 Journey Optimizerは、各ターゲットグループのエンゲージメント指標を自動的に追跡し、セグメント間でパフォーマンスを比較できるようにします。 [詳細情報](../campaigns/optimization-targeting.md)
