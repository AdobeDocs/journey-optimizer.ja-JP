...
>[!TIP]
>
>Journey Optimizerのデータを初めて利用する場合 スキーマ、データセット、ID、プロファイルフラグメントモデル、完全なデータ準備チェックリストについて理解するには、[ データ管理の基本](../../data/gs-data.md)の概要から始めて、設定に入る前に確認してください。
...
1. **メッセージ書き出しデータセット** （オプション）。 メッセージの書き出しがチャネル設定レベルで有効になっている場合、送信された電子メールとSMS コンテンツは、コンプライアンス、アーカイブ、ダウンストリーム分析のために、専用のExperience Platform データセットに自動的に書き出されます。 [ メッセージの書き出し](../../configuration/message-export.md)について説明します。
