...
1. **メッセージ書き出しデータセット** （任意）。 メッセージの書き出しがチャネル設定レベルで有効になると、送信されたメールと SMS コンテンツは、コンプライアンス、アーカイブまたはダウンストリーム分析のために、専用のExperience Platform データセットに自動的に書き出されます。 [&#x200B; メッセージの書き出し &#x200B;](../../configuration/message-export.md) について説明します。
