...
* データ管理およびスキーマ作成の権限の付与
* 開発およびテスト用のサンドボックスアクセスの承認
* データ保持ポリシーとガバナンスルールの調整
* Federated Audience Composition などの高度な機能へのアクセスを有効にする

>[!TAB  開発者を有効にする ]

API アクセスとテストに関する [&#x200B; デベロッパー &#x200B;](developer.md) との共同作業：

* Adobe Developer Consoleを通じて API 資格情報を提供する
* 開発およびテスト用のサンドボックス環境の設定
* チャネル設定（プッシュ証明書、SMS プロバイダー）を承認
* テスト環境とデプロイメント戦略に関する調整

>[!TAB  マーケターを支援 ]

権限とチャネル設定に関する [&#x200B; マーケター &#x200B;](marketer.md) との共同作業：

* ジャーニーとキャンペーンを作成するための適切な権限を割り当てます
* 使用するチャネルの設定（メール、プッシュ、SMS など）
* テスト環境と承認ワークフローのサポート
* 新機能へのアクセスを有効にする
