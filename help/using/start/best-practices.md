...
1. ブランドが、放棄から 30 分後にサイト訪問者を再エンゲージしたいと考えています（例：買い物かごの放棄されたメール）:
...
* ライブジャーニーの制限に近づいている場合、最初に実行できる手順は、**ジャーニーの下の** 概要 **タブに移動して、アクティブなプロファイルを持つ過去 24 時間にアクティブだったジャーニーの数を確認することです** このセクションでは、ジャーニーにエントリまたは離脱するプロファイル数を確認して判断できます。
...
* バッチセグメント（A）を使用する **オーディエンスを読み取り** ジャーニーがあり、除外するジャーニー内で（A ～ B を実行する） inAudience ストリーミングセグメント（B）を使用している場合は、そのロジックをセグメント化ロジックに移動し、セグメント化ロジック自体の一部として除外を使用することを検討します。