...
ジャーニーを保存または公開すると、Journey Optimizerによってジャーニーの合計ペイロードサイズが検証され、安定性とパフォーマンスが維持されます。

**デフォルトの設定**

* **デフォルトの最大リクエストサイズ**:2 MB （2,000,000 バイト）。 一部の組織には、Adobeによって設定されたカスタム制限があります。
* **警告しきい値**：上限の 90%。
* **エラーしきい値**：上限の 100%。 保存または公開がブロックされ、リクエストで **HTTP 413 リクエストエンティティが大きすぎます** が返されます。

**ユーザーエクスペリエンスのシナリオ**

* **ペイロードが制限の 90% 未満**:ジャーニーは、正常に保存および公開されました。 警告やエラーは表示されません。
* **ペイロードの制限の 90～99%**:ジャーニーは、保存と公開に成功し、最適化するよう警告を表示します。 警告メッセージ：**警告**:ジャーニーペイロードのサイズが制限に近づいています。 最大ノード：&#39;[NodeName]&#39; （型：&#39;[NodeType]&#39;、サイズ：[N] バイト）。
* **ペイロード >= 100% の制限**:ジャーニーの保存または公開はブロックされ、エラーが発生します。 エラーメッセージ：**エラー**:ジャーニーのペイロードサイズが制限を超えています。 最大ノード：&#39;[NodeName]&#39; （型：&#39;[NodeType]&#39;、サイズ：[N] バイト）。

**エラー応答の詳細**

リクエストが許可された最大サイズを超えた場合、応答には **リクエストエンティティが大きすぎます** と含まれます。 ジャーニーペイロードが許可された最大サイズを超えています。 エラーの詳細を確認し、ジャーニーを最適化します。

**トラブルシューティングと推奨事項**

* 警告またはエラーでハイライト表示されている最大のノードを確認します。
* 条件を簡素化し、データマッピングを減らし、不要な手順やパラメーターを削除します。
* 必要に応じて、ジャーニーをより小さなジャーニーに分割することを検討します。
* 組織でさらに上限が必要だと思われる場合は、Adobeの担当者にお問い合わせください。
