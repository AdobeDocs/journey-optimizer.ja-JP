...
## 概要の主な制限 {#quick-reference}

この表を使用して、ビルドまたはパブリッシュする前に最も重要な数値制限を検索します。 詳細とコンテキストは以下のセクションにあります。
...
| ガードレール | 値 | メモ |
|---|---|---|
| 上限 – 0.75秒未満の応答 | **300,000回の呼び出し/分** （ホストごと、サンドボックスごと） | スライディングウィンドウ。 サンドボックスおよびエンドポイントごとに適用されます。 |
| キャップ制限 – 0.75秒を超える応答 | **150,000回の呼び出し/ ホストおよびサンドボックスごとに30 s** | スライディングウィンドウ。 エンドポイントの待ち時間が0.75秒を超えた場合に個別の制限が適用されます。 |
| URL タイプ | 静的のみ | カスタムアクション URLの動的パラメーターはサポートされていません。 |
| サポートされているHTTP メソッド | 投稿、PUT、GET | その他の方法はサポートされていません。 |
| クエリパラメーター/ヘッダー名形式 | `.`または`$`で始めてはいけません | これらの文字で始まる名前は拒否されます。 |
| URL内のIP アドレス | 許可されていません | IP アドレスの代わりにホスト名を使用します。 |
| 内部Adobe アドレス | 許可されていません | `.adobe.*`個のアドレスは、URLとAPIでは許可されていません。 |
| ビルトインのカスタムアクション | 削除できません | 組み込みのカスタムアクションは永続的です。 |
| ペイロード形式 | JSONのみ | リクエストとレスポンスの両方のペイロードにはJSON形式が必要です。 [このページ](../action/about-custom-action-configuration.md#custom-actions-limitations)を参照してください。 |
| エンドポイントの最小スループット | 200 TPS | カスタムアクションによってターゲットされるエンドポイントは、少なくとも200 TPSをサポートする必要があります。 |

>[!TIP]
>
>**これが意味すること：** デフォルトの300,000 コール/分キャップは、外部エンドポイントがジャーニースループットに圧倒されるのを防ぎます。 エンドポイントがより多くの負荷を処理できる場合は、[Capping API](../configuration/capping.md)または[Throttling API](../configuration/throttling.md)を使用してこの制限を上げることができます。 より高い組織制限が必要な場合は、Adobeの担当者にお問い合わせください。

この制限は、カスタムアクションによって対象となる外部エンドポイントを保護することを目的に、顧客の使用状況に基づいて設定されています。 必要に応じて、Capping API と Throttling API でキャップまたはスロットルキャップを大きく定義することで、この設定を上書きできます。 [このページ](../configuration/external-systems.md)を参照してください。
...
| ガードレール | 値 | メモ |
|---|---|---|
| 同時インスタンス（すべてのサンドボックス + ジャーニー） | **5** | 「オーディエンスを読み取り」で5つ以上のジャーニーを同時にスケジュールすることは避け、5～10分間隔で展開します。 |
| サンドボックスのスループット | **20,000 プロファイル/秒** （共有） | サンドボックス内のすべての「オーディエンスを読む」アクティビティで共有されます。 制限に達した場合、ジョブがキューに入れられる可能性があります。 |
| アクティビティごとに設定可能なスループット | **500 ～ 20,000 プロファイル/秒** | サンドボックスの共有制限内で、アクティビティごとに設定します。 |
| ジョブ処理のタイムアウト | **12時間** | 12時間以内に処理できないジョブは、自動的にクリーンアップされ、実行されません。 |
| 再試行ウィンドウ | 最大&#x200B;**1時間** （10分間隔） | 書き出しジョブの取得中に再試行が適用されます。 ジャーニーは、スケジュールされた時間から最大1時間後に実行できます。 |
| 補足ID読み取り率 | ジャーニーインスタンスあたり&#x200B;**500 プロファイル/秒** | 補足IDが使用されている場合に適用されます。 |
| ジャーニーごとのオーディエンスインスタンスの読み取り | **1** | ジャーニーには、「オーディエンスを読み取り」アクティビティを1つだけ設定できます。 |
| オーディエンスタイプ | ストリーミングオーディエンスは常に最新です | バッチオーディエンスは、毎日のバッチ評価時間に1日に1回のみ評価されます。 |
| プロファイル属性の更新 | **待機** アクティビティで更新されました | ジャーニーのエントリ時に、プロファイルはバッチスナップショット値を使用します。 プロファイルが待機アクティビティに達すると、属性が更新されます。 |
| ジャーニーでのポジショニング | 最初のアクティビティまたはビジネスイベント後 | オーディエンスを読み取りアクティビティは、ジャーニーの最初のアクティビティとして、またはビジネスイベントアクティビティの後にのみ使用できます。 |
| Adobe Campaignでの使用 | サポートなし | オーディエンスを読み取りアクティビティは、Adobe Campaign アクティビティでは使用できません。 |
| 複数の顧客 | 直接サポートされていません | 「オーディエンスを読み込む」アクティビティでは、ジャーニーごとに1人のオーディエンスのみをターゲットにできます。 複数のオーディエンスを使用するには、まずそれらを結合します。 [詳細情報](../audience/get-started-audience-orchestration.md) |
