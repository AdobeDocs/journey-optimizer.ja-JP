...
* ジャーニーランタイムは、プロファイルとジャーニーのバージョンごとに、最大10件の保留中のイベントの内部キューを処理中に保持します。 この制限に達すると、スタックがドレインするまで、`maxInstanceStackEventsReached`の理由で追加のイベントが破棄されます。 maxInstanceStackEventsReached[の](../building-journeys/troubleshooting-execution.md#max-instance-stack-events-reached)破棄されたイベントを参照してください。
...
* Journey Optimizer でジャーニーをトリガーするには、イベントをデータ収集コアサービス（DCCS）にストリーミングする必要があります。バッチで取り込まれたイベント、クエリサービス **を介して挿入されたイベント、またはJourney Optimizerの内部データセット（メッセージフィードバック、メールトラッキングなど）からのイベントは、ジャーニーのトリガーに使用できません。**&#x200B;ストリーミングイベントを取得できないユースケースについては、代わりに、それらのイベントに基づいてオーディエンスを作成し、「**オーディエンスを読み取り**」アクティビティを使用する必要があります。オーディエンスクオリフィケーションは技術的に使用できますが、使用されたアクションに基づいて下流の課題を引き起こす可能性があるため、推奨されません。
