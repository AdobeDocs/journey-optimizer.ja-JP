...
メールメッセージを含むジャーニーを公開する場合は、バックエンド処理後のメッセージコンテンツの合計サイズを **2 MB** 以下にする必要があります。 公開中、システムはリンクや画像にパッチを適用したり、変換を適用したりすることで、メッセージコンテンツを自動的に処理します。これにより、ペイロードサイズが作成されたコンテンツサイズよりも大きくなります。

>[!CAUTION]
>
>最終的に処理されたメッセージコンテンツが 2 MB を超えると、ジャーニーの公開が失敗します。 公開エラーを回避するには、作成したメッセージコンテンツを 2 MB 未満に保ち（理想的には **1 MB 以下**、バックエンド処理のオーバーヘッドに対して 300～400 KB のバッファーを許可します。

**公開エラーを防ぐためのベストプラクティス：**

* 作成したメールコンテンツを 1 MB 未満に保持
* コンテンツバリアントの数を最小限に抑える
* 画像をメッセージに追加する前に最適化および圧縮
* 未使用のアセットや不要なHTML要素を削除する
* ジャーニーを実稼動環境に公開する前に、メッセージサイズをテストします

コンテンツのサイズが原因でジャーニーの公開に失敗した場合は、メッセージコンテンツを減らし、ジャーニーを再公開します。
...
* コンテンツカードで偽名プロファイル（未認証の訪問者）をターゲットにする場合は、自動プロファイル削除の有効期間（TTL）を設定して、エンゲージメント可能なプロファイル数と関連コストを管理することを検討してください。 [詳細情報](#profile-management-inbound)
...
* すべてのカスタムアクションには、各ホストおよび各サンドボックスに、1 分間で 300,000 件の呼び出しというキャップが定義されています。「ホストあたり」の制限は、ドメインレベル（例：example.com）で適用されます。 このキャップは、応答時間が 0.75 秒未満のエンドポイントのサンドボックスごとエンドポイントごとのスライディングウィンドウとして適用されます。 応答時間が 0.75 秒を超えるエンドポイントの場合、30 秒あたり 150,000 回の呼び出しという別の制限（スライディングウィンドウも含む）が適用されます。 [このページ](../action/about-custom-action-configuration.md)を参照してください。この制限は、カスタムアクションの対象となる外部エンドポイントを保護することを目的に、顧客の使用状況に基づいて設定されています。必要に応じて、キャップ／スロットリング API でキャップまたはスロットルキャップを大きく定義することで、この設定を上書きできます。[このページ](../configuration/external-systems.md)を参照してください。
