...
配信スループットは、指定された時間内に Decision Management アプリサービスで配信できる判定応答の数に対応します。 次の表に、1 秒あたりの決定の数を示します。

| API | 1 秒あたりの決定数 |
|---------|----------|
| API リクエストの判定 | 毎秒 500 |
| Edge Decisioning API リクエスト | 毎秒 5000 |

### 制限事項 {#offers-limitations}

決定管理の制限を以下に示します。

* **承認済みのパーソナライズされたオファーとフォールバックオファー**  — 最大 10,000 個の承認済みパーソナライズされたオファーと承認済みのフォールバックオファーを組み合わせたもの。
* **決定**  — 最大 10,000 件の決定。
* **ライブの決定** -Offer decisioningアプリサービスは、最大 1,000 個のライブ判定をサポートします。
* **応答ごとに返されるオファー** -Offer decisioningは、リクエストのすべての判定範囲にわたって、リクエストごとに最大 100 個のオファーをサポートします。
* **コレクション**  — 最大 10,000 個のコレクション。
* **決定ごとのコレクション** - 1 つの決定につき最大 30 個のコレクション。
* **決定ルールとランキング関数** 最大 10,000 個の決定ルールとランキング関数を組み合わせたもの。
* **配置**  — 最大 1,000 個の配置。
* **決定ごとの配置** - 1 回の決定につき最大 30 個の配置。
* **決定ごとのランキング方法** -Offer decisioningアプリサービスは、1 つの決定につき最大 30 個のランキング関数をサポートします。
* **AI ランキングモデル** -Offer decisioningアプリサービスは、最大 5 つの AI ランキングモデルをサポートします。
* **オファーまたはコレクションごとの収集修飾子** -Offer decisioningアプリサービスは、1 つのパーソナライズされたオファーまたは 1 つのコレクションで、最大 20 個のコレクション修飾子をサポートします。
* **合計収集修飾子** -Offer decisioningアプリサービスは、最大 1,000 個のコレクション修飾子をサポートします。