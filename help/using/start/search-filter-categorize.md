...
The **[!UICONTROL タグ]** 「 」フィールドでは、オブジェクトのタグを定義できます。 タグは、次のオブジェクトで使用できます。

* [ジャーニー](../building-journeys/journey-gs.md#change-properties)
* [キャンペーン](../campaigns/create-campaign.md#create)
* [テンプレート](../email/content-templates.md)
* [フラグメント](../email/fragments.md)
* [購読リスト](../landing-pages/subscription-list.md)
* [ランディングページ](../landing-pages/create-lp.md)

既存のタグを選択することも、新しいタグを作成することもできます。これを行うには、以下の手順に従います。

1. 目的のタグの名前を入力し始めるか、リストから選択します。
...
1. 検索しているタグが使用できない場合は、 **[!UICONTROL &quot;&quot;を作成]** 新しいオブジェクトを定義する場合 — 現在のオブジェクトに自動的に追加され、他のすべてのオブジェクトで使用できるようになります。

   ![](assets/tags4.png)

1. 選択したタグまたは作成したタグのリストが、 **[!UICONTROL タグ]** フィールドに入力します。 必要な数だけタグを定義できます。

>[!NOTE]
> 
> オブジェクトを複製または新しいバージョンを作成した場合、タグは保持されます。

### タグに対するフィルター

各オブジェクトリストには専用の列が表示され、タグを簡単に視覚化できます。

フィルタは、特定のタグを持つオブジェクトのみを表示する場合にも使用できます。
