...
[Experience PlatformのAI アシスタントの詳細](https://experienceleague.adobe.com/ja/docs/experience-platform/ai-assistant/home){target="_blank"}
...
Journey AgentのAI アシスタントには、分析と作成という2つのスキルが備わっています。 自然言語プロンプトから、既存のジャーニーを最適化したり、新しいジャーニーを構築したりできます。

+++**必要な権限**

* **ジャーニーの表示** - AI アシスタントでジャーニーに関するインサイトを直接表示します
* **ジャーニーの管理** - AI アシスタントで新規ジャーニーを直接作成します
* **セグメントを表示** - オーディエンスに関するインサイトを表示し、既存のオーディエンスを検索します
* **セグメントの管理** - AI アシスタントで新規オーディエンスを直接作成します
* **ジャーニーイベント、データソース、およびアクションを表示** - ジャーニーイベントとカスタムアクションを検索する作成スキルに必要
...
* **自然言語ジャーニーの作成** – 目的のジャーニーを記述し、自動的に作成する
* **イベントベースおよびオーディエンスベースの開始** - イベントトリガー、オーディエンスベース、ビジネスイベント、またはオーディエンスの選定ジャーニーを作成
* **条件付きロジック** – 顧客の属性または行動に基づいて分割パスを作成します
* **マルチチャネル メッセージ** – 電子メール、プッシュ通知、SMS アクションを追加する
* **スケジュール** - ステップ間の開始日とタイミングを設定します

+++

+++**サンプルプロンプト**

* 「顧客がオンラインで購入し、サンキュープッシュ通知を送信したときに開始するジャーニーを作成します。」
* 「2020年12月から2週間にわたって3通のメールを送信し、私の日帰りハイカーのオーディエンスをターゲットにしたジャーニーを構築します。」
* 「ユーザーが店舗の場所に入ったときに開始し、有効なメールアドレスがあるかどうかに基づいてフォローアップするジャーニーを作成します。」
...
* **完全コンテンツ生成** – 電子メール、web、ランディングページ、プッシュ用に、完全なコンテンツエクスペリエンス（テキストと画像）を1つのフローで生成します。 [AI アシスタントで完全なコンテンツを生成](../content-management/generative-full-content.md)
...
* **ロジックを保持する簡素化** - AIは、シミュレートされたプロファイルに同じ結果を返す短い式を提案します。
* **検証レポート** – 変更を適用する前に、シミュレートされた各プロファイルが両方のバージョンに対してどのように評価されるかを示す分析（TSV）をダウンロードします。
* **ワンクリックで適用** – 元のPQLを&#x200B;**[!UICONTROL 最適化]** ウィンドウから直接最適化されたバージョンに置き換えます。

### 実施要件

PQL式が&#x200B;**2 KB** （UTF-8 エンコード）より大きいルールとランキング式のみが分析の対象となり、小さい式は分析されません。
...
* **[ジャーニーCreate Agent](#journey-create-agent)** -ジャーニーの管理、ジャーニーイベント/データソース/アクションの表示、セグメントの表示、およびセグメントの管理権限が必要です
...
>[!MORELIKETHIS]
>
>* [Journey Optimizerとは何ですか？](get-started.md)  – 主な機能、ユースケース、アーキテクチャの概要。
