...
1. **管理者**：環境を設定します\
   ...
   * チャネル設定（メール、SMS、プッシュ、web プッシュ、アプリ内、web、ダイレクトメール、コンテンツカード）を設定
...
* **Journey Orchestration**：様々なチャネルをまたいだ行動やイベントをトリガーとして、各ユーザーが自分のペースで顧客エンゲージメントを進めていく、1 対 1 のリアルタイムのカスタマーエンゲージメントを作成します。 すべてのチャネルアクションの統合アクションアクティビティ、ジャーニーにオファーを統合するコンテンツ決定アクティビティおよび自然言語プロンプトからジャーニーを作成するJourney Agentを使用できます
* **キャンペーンオーケストレーション**：視覚的なキャンバスを使用して、複雑でマルチステップのバッチキャンペーンを大規模にデザインおよび自動化します。季節のプロモーション、製品ローンチ、アカウントベースのコミュニケーションなど、ブランドのキャンペーンに最適です。マルチエンティティのセグメント化を活用して、顧客データを関連するエンティティ（アカウント、購入、予約）と結び付けることで、正確なオーディエンスを作成します。 ウェーブ送信を使用して、制御されたバッチでメッセージを配信します
* **最新のメッセージデザイナー**：ドラッグ＆ドロップインターフェイスで、メールやモバイルメッセージをデザインおよびパーソナライズします。標準のテンプレートを編集して、マーケット投入までの時間を高速化します
* **意思決定管理**：メールおよび顧客タッチポイントに埋め込むことができる一元化されたライブラリで、オファー、実施要件ルールおよびその他のコンポーネントを作成および管理します。 プッシュと SMS のパーソナライゼーションに意思決定を使用する
...
* **配信制御**：顧客の好みを尊重し、過度の通信を防ぐために、待ち時間（時間ベースの除外）と競合管理を使用します

**開始：** ユースケーステンプレートとウィザードを使用して、新しいカスタマージャーニーを簡単に作成およびデプロイします。 Journey Agentを使用して、自然言語プロンプトからジャーニーを作成します。
...
* **配信品質**：サブドメインをデリゲートし、必要に応じてサブドメインをカスタムデリゲーションに移行し、IP プールを作成して、抑制リストと許可リストを管理します
* **ジャーニー設定**：チームのジャーニー要素と設定を指定します
* **チャネル設定**：必要に応じて、web プッシュ通知、ダイレクトメール、メッセージエクスポート（メール/SMS）を設定します
