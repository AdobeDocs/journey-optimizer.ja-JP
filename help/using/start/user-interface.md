...
title: インターフェイスの操作
description: Adobe Journey Optimizer インターフェイスを操作する方法と、上部バー、ホームページ、左側のナビゲーションなどの主要機能にアクセスする方法について説明します。
...
**ヒント：** 新規ユーザーは、**ホーム** ページから開始して最近の項目やユースケースにアクセスしたり、上部バーの **ヘルプ** アイコンを使用してコンテキストガイダンスを得たりできます。

## 上部バーの機能 {#top-bar}

### ユニバーサル検索 {#search}

検索アイコンを使用すると、Journey Optimizer をまたいでジャーニー、キャンペーン、アセット、他のオブジェクトをすばやく検索できます。キーワードを入力すると、プラットフォームのすべての領域から関連性の高い結果が表示されます。 [ ユニバーサル検索の詳細 ](search-filter-categorize.md#unified-search)。
...
コンテキストヘルプには、表示している画面に関連するガイダンスと、対応するドキュメントに直接リンクが表示されるので、ワークフローを離れることなく、適切な情報を取得できます。

![コンテキストヘルプの例](assets/do-not-localize/Context-help.gif)

#### サポートチケットガイドライン {#support-ticket-guidelines}

Adobe サポートに連絡する際は、根本原因の分析と解決を迅速に行うために、以下を含めてください。

* **環境の詳細** - サンドボックス名、組織 ID および製品領域（ジャーニー、キャンペーン、決定など）
* **影響レベル** – 問題の重大度（実稼動のブロック、機能の制限、表面的な問題など）
* **レプリケーション手順** – 問題を再現するための明確な手順
* **ログまたはスクリーンショット** – 問題を示す関連するエラーメッセージ、リクエスト ID、スクリーンショット
* **関連する ID** -ジャーニーID、キャンペーン ID、オーディエンス ID、または問題に関連するその他のオブジェクト ID
...
1. プロファイルメニューで「**[!UICONTROL 環境設定]**」をクリックします
2. 優先言語を選択します
...
### ユースケースとクイックスタートワークフロー {#use-cases}
