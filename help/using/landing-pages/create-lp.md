...
**[!UICONTROL ランディングページ]** リストには、既存のすべてのランディングページが表示されます。 ステータス、変更日、タグに基づいてフィルタリングできます。

![](assets/lp_access-list-filter.png)

このリストから、ランディングページの複製、削除、非公開や、ランディングページレポートへのアクセスも可能です。 それには、ランディングページの横にある 3 ドットアイコンをクリックし、目的のアクションを選択します。
...
* 公開済みのランディングページの **ランディングページレポート** および [ 過去 24 時間のライブレポート [](../reports/lp-report-global-cja.md) のレポート ](../reports/lp-report-live.md) にアクセスします。

* ランディングページを **削除** または **複製** します。 [公開済み](#publish-landing-page)のランディングページは削除できません。削除するには、まず **非公開** にする必要があります。

* **非公開** ランディングページ。

  >[!CAUTION]
  >
  >メッセージで参照されているランディングページを非公開にした場合、そのランディングページへのリンクが壊れ、ユーザーがアクセスしようとするとエラーページが表示されます。

* ランディングページに関連付けられている [ タグ ](../start/search-filter-categorize.md#tags) を編集します。
...
ランディングページを作成するには、プリセットを選択し、プライマリページとサブページを設定してから、公開前にページをテストする必要があります。 これらの手順について以下で詳しく説明します。


1. 左メニューから **[!UICONTROL コンテンツ管理]**/**[!UICONTROL ランディングページ]** を参照します。
...
>[!CAUTION]
>
>ランディングページを公開するには、ユーザーに **[!UICONTROL メッセージを公開]** 権限が必要です。
