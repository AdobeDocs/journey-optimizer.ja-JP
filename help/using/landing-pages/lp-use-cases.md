...
**このページでは、** フォーム送信からの購読の促進、オプトアウトの取得、トリガーフォローアップ ジャーニーの実行にランディングページを使用する実例について説明します。
...
イベント登録用のランディングページを設定するには、サブスクリプションリストを作成し、登録フォームを含むランディングページをデザインして、必要なページと設定を指定します。 次の手順に従います。
...
1. ジャーニーを公開して実行します。 [学習を増やす](../building-journeys/journey.md)。
