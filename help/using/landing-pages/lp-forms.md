...
description: Journey Optimizerでランディングページ用のフォームを作成および使用する方法について説明します
...
[!DNL Journey Optimizer]のランディングページでプロファイルデータを取得し、[!DNL Experience Platform]のデータセットを強化するには、ランディングページでフォームを活用します。 データキャプチャのシナリオと要件の概要については、[&#x200B; ランディングページを使用したデータキャプチャ &#x200B;](get-started-lp.md#data-capture-lp)を参照してください。
...
1. フォームにリンクする&#x200B;**[!UICONTROL データセット]**&#x200B;を選択します。ここにフォームの応答が保存および反映されます。 特定のデータセットを検索するには、入力するか、リストから選択できます。
   ...
   >[!NOTE]
   >
   >現在、**プロファイルが有効な**&#x200B;と&#x200B;**プロファイルが有効でない** [!DNL Adobe Experience Platform]個のデータセットのみを選択できます。 一度に 1 つのデータセットを選択できます。フォームデータの保存にシステムデータセットを使用することはできません。 [詳しくは、データセットを参照してください。](../data/get-started-datasets.md)
...
フォームの詳細に戻り、「**[!UICONTROL ありがとうございましたページ]**」セクションで、ユーザーがフォームに入力した際の動作を設定します。
...
1. 「**[!UICONTROL 埋め込みフォーム]**」セクションで、作成したフォームを選択します。
   ...
   >[!NOTE]
   >
   >「**[!UICONTROL フォームを編集]**」ボタンを使用して、選択したフォームを更新できます。フォームが新しいタブで開きます。フォームコンテンツの編集手順について詳しくは、[この節](#edit-form)を参照してください。
...
ランディングページが[公開](create-lp.md#publish-landing-page)され、ジャーニーで使用されると、ユーザーがフォームに入力すると、入力された情報が選択したデータセットに取り込まれます。 データを処理して使用可能にするには、最大15分かかります。
...
+++フォームを使用するには、どの種類のランディングページが必要ですか？

ランディングページを作成する際に、ランディングページタイプとして「**[!UICONTROL データキャプチャ]**」を選択します。 これは、データ収集用のフォームの埋め込みをサポートする唯一のランディングページタイプです。

+++

+++フォームプリセットで任意のデータセットを使用できますか？

選択できるデータセットは、**プロファイルが有効な**&#x200B;と&#x200B;**プロファイルが有効でない** [!DNL Adobe Experience Platform]のみです。 フォームデータの保存にシステムデータセットを使用することはできず、プリセットごとに1つのデータセットのみを選択できます。

+++

+++複数のフォームプリセットで同じデータセットまたはストリーミング接続を再利用できますか？

はい。 同じデータセットまたはストリーミング接続を複数のプリセットで再利用できます。 また、異なるデータセットとストリーミング接続の組み合わせを使用して複数のプリセットを設定し、異なるデータキャプチャシナリオをサポートすることもできます。

+++

+++ランディングページでドラフトフォームを使用できますか？

いいえ。 ランディングページで選択して埋め込むことができるのは、**公開された** フォームのみです。

+++

+++ライブランディングページに既に埋め込まれているフォームを編集して再公開する場合、ランディングページはどのバージョンを使用しますか？

ランディングページでは、常にフォームの&#x200B;**最新の公開済みバージョン**&#x200B;が使用されます。 ドラフトの変更は、更新されたフォームが公開されるまで有効になりません。

+++

+++フォームの入力方法に応じて、ユーザーを異なるページにリダイレクトできますか？

はい。 サンキューページで&#x200B;**[!UICONTROL 条件付きリダイレクト]**&#x200B;を設定し、フォームの応答に基づいて異なるフォローアップアクションを表示するルールを定義できます。例えば、ある国の訪問者をあるページに、別の国の訪問者を別のページにリダイレクトし、他の全員にデフォルトのフォールバックを適用することができます。 [詳細情報](#thank-you-page)

+++

+++フォームフィールドには、既存のプロファイルデータを事前に入力できますか？

はい。 フィールドを設定する際に、**[!UICONTROL 事前入力済み有効]** オプションを有効にすると、フィールドが使用可能になったときに、既知のプロファイルの情報からフィールドが自動的に入力されます。

+++

+++ユーザーがスキップできないように、フィールドを必須にできますか？

はい。 フィールドの設定の「**[!UICONTROL フィールドの動作]**」セクションで、**[!UICONTROL 必須]**&#x200B;を選択します。 すべての必須フィールドが完了するまで、ユーザーはフォームを送信できません。

+++

+++ユーザーがフォームを送信した後、フォーム送信データはどこに送られますか？

フォームが送信されると、フォームプリセットで定義された&#x200B;**ストリーミング接続**&#x200B;にデータが送信されます。 その後、マッピング、変換、検証を処理する自動生成ソースフローを通じて、リンクされた&#x200B;**[!DNL Experience Platform]データセット**&#x200B;にプッシュされます。
