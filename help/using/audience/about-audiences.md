...
セグメント定義、カスタムアップロードまたはコンポジションワークフローを使用して生成されたオーディエンスをキャンペーンやジャーニーで選択できます。

>[!AVAILABILITY]
>
>オーディエンス構成からのオーディエンスおよび属性の使用は、現在、Healthcare Shield またはプライバシーとセキュリティシールドで使用することはできません。 [詳しくは、Journey Optimizer でのオーディエンスのエンリッチメント属性の使用方法を参照してください](../audience/about-audiences.md#enrichment)
...
コンポジションワークフローまたはカスタム（CSV ファイル）オーディエンスを使用して生成されたオーディエンスをターゲティングする場合、これらのオーディエンスのエンリッチメント属性を活用してジャーニーを作成し、メッセージをパーソナライズできます。

>[!NOTE]
>
>2024 年 10 月 1 日（PT）より前に CSV ファイルのカスタムアップロードで作成されたオーディエンスは、パーソナライゼーションの対象となりません。 これらのオーディエンスの属性を使用し、この機能を最大限に活用するには、この日付より前に読み込んだ外部 CSV オーディエンスを再作成および再アップロードしてください。
>
...
>[!IMPORTANT]
>
>コンポジションワークフローを使用して作成されたオーディエンスのエンリッチメント属性を使用するには、属性が「ExperiencePlatform」の Data Source内のフィールドグループに追加されていることを確認します。
>
...
## カスタムアップロード（CSV ファイル）オーディエンス {#csv}

この節では、カスタムアップロード（CSV ファイル）オーディエンスを使用する際に留意すべき重要な情報を示します。

* **CSV オーディエンスのプレビューと配達確認のサポート：** 現在、CSV アップロードを使用して作成されたオーディエンスでは、プレビューと配達確認はサポートされていません。 キャンペーンを計画する際には、このことに注意してください。

* **高速なアクティベーションと ID ステッチの遅延：** Adobe Experience Platform アーキテクチャでは、カスタムアップロードオーディエンスをJourney Optimizerで直ちにアクティブ化できるようにするために、ID ステッチが遅延します。次のような影響があります。

   * オーディエンスは、取り込みが完了するとすぐに、Journey Optimizerで使用できるようになります。 これは通常 1 時間以内ですが、何らかの変動が生じる可能性があります。
   * アクティブ化されたレコードの数は、ID ステッチ後のプロファイルの数と異なる場合があります。
   * 重複を含め、CSV ファイル内のすべてのレコードがアクティベートされます。 次回の UPS プロファイルの書き出し時に、これらのレコードは ID ステッチを経ます。

* **CSV アップロードからの新しいプロファイルのターゲット設定：** CSV レコードと UPS プロファイルの間に一致するものが見つからない場合、新しい空のプロファイルが作成されます。 このプロファイルは、データレイクに保存されるエンリッチメント属性にリンクされています。 この新しいプロファイルは空なので、Journey Optimizerで通常使用されるターゲティングフィールド（personalEmail.address、mobilePhone.number など）は空なので、ターゲティングに使用できません。

  これを解決するには、チャネル設定の「実行フィールド」（またはチャネルに応じて「実行アドレス」）を「identityMap」として指定します。 これにより、CSV アップロード時に ID として選択した属性が、Journey Optimizerでのターゲティングに使用される属性になります。
...
コンポジションワークフローを使用して作成されたオーディエンスのエンリッチメント属性を使用するには、それらを「ExperiencePlatform」データSource内のフィールドグループに追加する必要があります。 エンリッチメント属性をフィールドグループに追加する方法について詳しくは、[この節](#enrichment)を参照してください
