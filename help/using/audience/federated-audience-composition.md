...
Adobe [!DNL Journey Optimizer]で使用できる連合オーディエンス構成機能は、データまたはプロファイルのデータウェアハウスを照会できます
オーディエンスの構成で使用する属性。機密データをウェアハウスに残し、データの重複を回避します。 これには、プロファイルベースのデータと非プロファイルデータの両方が含まれる場合があります。 たとえば、クイックサービスのレストラングループは、顧客のプロファイルを充実させることができます
これにより、各顧客が好むレストランの場所でオンラインで注文を行い、レストラングループがその好みの場所で使用できるオファーを顧客に送信できるようになりました。
...
* **効率的なデータ管理** - データのコピーを最小限に抑え、機密データを管理
データ運用を最適化し、Adobeのリアルタイムのチャネルをまたいだユースケースを補完できます
