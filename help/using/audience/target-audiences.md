...
バッチセグメント化ジョブから生成されたオーディエンスの場合、バッチ取り込みの変動によりアクティベーションが遅延する場合があります。毎日スケジュールされる「オーディエンスを読み取り」ジャーニーの場合、ジャーニーの実行前に新しいオーディエンスデータを使用できるように、ジャーニープロパティで時間枠を定義できます。定義された期間内にセグメント化ジョブが完了しない場合、ジャーニーは次に発生するまでスキップされます。 [オーディエンスを読み取りジャーニーのスケジュール方法の詳細情報](../building-journeys/read-audience.md)
