...
1. コンポーネントのプロパティパネルが右側に開きます。**編集可能フィールド** タブを選択してから、「**編集を有効にする** オプションを切り替えます。
...
➡️ [ このビデオでは、編集可能なフラグメント内でHTML コンポーネントにリッチテキストを追加して使用する方法を説明 ](#video) ます。

### リッチテキストを含むフラグメントの作成 {#add-rich-text}

1. ビジュアルフラグメントを作成し、コンポーネントの追加を開始します。
...
1. `render_content` を、必要なデフォルトのコンテンツに対応するHTML コードに置き換えます。
...
1. キャンペーンまたはジャーニーにメールを作成し、作成したフラグメントを追加します。

   右側のパネルに作成された 2 つの編集可能フィールドを表示できます。

   ![](assets/fragment-use-rich-editable-fields.png)

1. **[!UICONTROL コンテンツをシミュレート]** をクリックして、編集可能なコンテンツとスタイル設定がどのようにレンダリングされるかを確認できます。

1. 編集可能フィールドの 1 つの横にある **[!UICONTROL パーソナライゼーションを追加]** アイコンを選択し、CSS スタイル設定やコンテンツを必要に応じて編集します。
