...
**このページ：** Adobe Journey Optimizerのコンテンツ実験で、メッセージのバリエーションをテストするためにランダム化試用版を使用する方法、結果を計算する方法、キャンペーンを最適化するためにレポートを解釈する方法を説明します。
...
テストを長く実行すると、処理間の目標指標の違いがより小さくなることを検出できます。ただし、目標指標のベースライン値が小さい場合は、より大きなサンプルサイズが必要になります。
実験に含める必要があるユーザーの数は、検出するエフェクトサイズ、目標指標の分散または広がり、偽陽性および偽陰性エラーに対する許容値によって異なります。従来の実験では、[&#x200B; サンプルサイズ計算機](https://experienceleague.adobe.com/tools/calculator/testcalculator.html?lang=ja){_blank}を使用して、テストを実行する必要がある期間を決定できます。
+++ 

+++統計的不確実性について

1,000人のユーザーが1回の処理を行った実験を実行している場合、コンバージョン率は5%に設定されます。すべてのユーザーを含めれば、これは実際のコンバージョン率でしょうか？真のコンバージョン率はどのくらいでしょうか？
統計的手法は、その不確実性を形式化する方法を提供します。オンライン実験を実施する際に理解すべき最も重要な概念の1つは、観察されたコンバージョン率が基礎となる真のコンバージョン率の範囲と一致しているということです。つまり、結論を出す前に、これらの見積もりが十分に正確になるまで待つ必要があります。信頼区間、および信頼性は、この不確実性を定量化するのに役立ちます。
+++
...
### &#x200B;2. 信頼区間を重視する {#confidence-intervals}
...
### &#x200B;3. 信頼性の把握 {#understand-confidence}
