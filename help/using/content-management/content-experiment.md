...
1. まず、要件に従って[キャンペーン](../campaigns/create-campaign.md)や[ジャーニー](../building-journeys/journey-action.md)を作成し、設定します。
...
* **[!UICONTROL A/B 実験]**：テストの開始時に処理間のトラフィック分割を定義します。パフォーマンスは、選択したプライマリ指標に基づいて評価され、その後、実験アクセラレーターにより、処理間で確認された上昇率が報告されます。

* **[!UICONTROL マルチアームバンディット]**：処理間のトラフィック分割は自動的に処理されます。7 日ごとに、プライマリ指標のパフォーマンスが確認され、それに応じて重み付けが調整されます。実験アクセラレーターのレポートには、A/B テストと同様に引き続き上昇率が表示されます。

* **[!UICONTROL 独自に導入したマルチアームバンディット]**：処理間のトラフィック分割は自動的に処理されます。Experiment API を使用して配分をリアルタイムで調整して、変更するタイミングと方法を柔軟に決定できます。

➡️ [A/B 実験とマルチアームバンディット実験の違いの詳細情報](mab-vs-ab.md)

>[!BEGINTABS]

>[!TAB A/B 実験]

1. メッセージをパーソナライズしたら、「**[!UICONTROL アクション]**」タブから「**[!UICONTROL 実験を作成]**」をクリックして、コンテンツ実験の設定を開始します。
...
1. 「**[!UICONTROL 処理を追加]**」をクリックして、新しい処理を必要な数だけ作成します。
   ...
   >[!CAUTION]
   >
   >コードベースの実験を作成する際は、次の制限に注意してください。
   >* **処理数**：3～5 を超える処理を作成すると、パフォーマンスとインターフェイスの問題が発生する場合があります。 処理を追加する際にエラーが発生した場合は、処理の数を減らすか、問題が解決するまで増分的に追加してみてください。
   >* **予約済みのキーワード**：処理名に「除外」などの予約済みのキーワードを使用しないでください。決定ノードマッピングエラーが発生し、実験が正しく機能しない場合があります。
...
>[!TAB 独自に導入したマルチアームバンディット]

独自に導入したマルチアームバンディットマルチアームバンディット実験は、次の場合にのみ使用できます。
...
1. API トリガーキャンペーンを作成した場合は、**[!UICONTROL 実験タイプ]**&#x200B;ドロップダウンから「**[!UICONTROL 独自に導入したマルチアームバンディット]**」を選択します。
