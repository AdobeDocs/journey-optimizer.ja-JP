...
1. 「**[!UICONTROL 処理を追加]**」をクリックして、新しい処理を必要な数だけ作成します。
   ...
   >* **処理数**:3～5 個を超える処理を作成すると、パフォーマンスとインターフェイスの問題が発生する場合があります。 処理を追加するときにエラーが発生した場合は、処理の数を減らすか、問題が解決するまで処理を増分的に追加してみてください。
   >* **予約済みのキーワード**：処理名に「除外」などの予約済みのキーワードを使用しないでください。これにより、決定ノードマッピングエラーが発生し、実験が正しく機能しない可能性があります。
