...
1. 自動スケール実験を有効にして、実験の勝者バリエーションを自動的にロールアウトします。 [ 勝者のスケール方法の詳細を説明します ](#scale-winner)
...
## 勝者のスケーリング {#scale-winner}

>[!AVAILABILITY]
>
>現在、勝者の尺度設定機能は、次のチャネルでサポートされています。
>
>* すべてのジャーニーまたはキャンペーンのインバウンドチャネル（web、アプリ内メッセージ、コードベースのエクスペリエンスなど）。
>* API トリガートランザクションキャンペーンのアウトバウンドチャネル（メール、プッシュ通知、SMS など）。

勝者のスケールを使用すると、実験の勝者バリエーションを自動または手動でオーディエンス全体にロールアウトできます。 この機能により、勝者が決定されると、実験を常に監視することなく、そのリーチと有効性を増幅させることができます。

次の 2 つのモードから選択できます。

* **自動スケーリング**：勝者の処理をスケーリングするタイミングと条件を選択するか、勝者が現れない場合はフォールバックオプションを選択して、実験を作成する際に自動スケーリング設定を構成します。
...
自動スケーリングを使用すると、実験の結果に基づいて、勝者の処理またはフォールバックをロールアウトするタイミングを事前定義したルールに設定できます。

自動スケーリングが発生すると、手動スケーリングは使用できなくなります。
...
1. 勝者のスケールを設定するタイミングを選択します：

   * 勝者が見つかったらすぐに。
   * 実験後は、選択した時間のみ有効になります。

     自動スケール時間は、実験の終了日より前にスケジュールする必要があります。 終了日より後の時間に設定されている場合、検証警告が表示され、キャンペーンまたはジャーニーは公開されません。

   ![](assets/scale-winner-2.png)

1. スケール時間で勝者が見つからない場合のフォールバック動作を選択します。

   * 計画どおりに終了するまで実験を続けます。
   * 指定した時間の経過後に代替処理のスケールを設定します。

すべてのパラメーターが満たされると、勝者または代替処理がオーディエンスに送信されます。

### 手動スケーリング {#manual-scaling}

手動スケーリングを使用すると、実験結果を確認し、勝者の処理を独自のスケジュールで展開するタイミングを決定できます。

スケジュールされた自動スケール時間より前に手動で推奨結果のスケールを設定すると、自動スケールはキャンセルされることに注意してください。

実験の勝者のスケールを手動で設定するには：
...
1. 「**[!UICONTROL 処理をスケール]**」をクリックして、勝者の処理を残りのオーディエンスにプッシュします。

   ![](assets/scale-winner-campaign.png)

1. スケールする処理をドロップダウンメニューから選択し、「**[!UICONTROL スケール]**」をクリックします。

   ![](assets/scale-winner-3.png)

処理の拡大・縮小には最大 1 時間かかる場合があります。 手動スケールプロセスが完了すると、通知が届きます。
