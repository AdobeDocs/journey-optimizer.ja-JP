...
1. **[!UICONTROL キャンペーン]**&#x200B;メニューに移動し、「**[!UICONTROL オーケストレーション]**」タブを選択します。
...
調整されたキャンペーンをスケジュールする際は、次のベストプラクティスを考慮することで、最適なパフォーマンスと期待される動作を確保します。

* システム全体のパフォーマンスが落ちたり、データベースにブロックが作成されたりする可能性があるので、調整されたキャンペーンを 15 分毎以上で実行するようにスケジュールしないでください。
...
次の例では、2025年10月1日から 2026年1月1日まで毎日、調整されたキャンペーンが 1 日に 2 回（午前 9 時と 12 時に）実行されるようにアクティビティが設定されています。

![キャンペーンが 1 日に 2 回（午前 9 時と午後 12 時に）実行されるように設定されているスケジューラー](assets/scheduler-sample.png){width="50%"}
