...
調整されたキャンペーンのサポート **メール、SMS、プッシュ通知**。
...
**例** – 次の 3ad-h0 日で有効期限が切れる購読を持つすべての受信者をターゲットにする：Campaign オーケストレーションでは、クエリを購読スキーマから開始し、そのスキーマの有効期限列のみを検索して、有効期限が切れるすべての購読を返します。次に、これらの特定の購読 ID に関連する受信者データにロールアップすると、受信者レベルで各クエリを開始するデータモデルよりも速く効率的に結果を返します。
