...
>abstract="**ファイルの読み込み** アクティビティは&#x200B;**データ管理** アクティビティです。 オーケストレーションされたキャンペーンキャンバス上の外部ファイルに保存されているプロファイルとデータを操作し、キャンペーンオーディエンスを定義するために使用します。 ファイルデータは実行時に消費され、Adobe Experience Platform データセットとして保持されません。 行は、ID列とターゲティングディメンションを使用して、既存の受信者に紐付けられます。 Adobeの担当者に連絡してアクセス権をリクエストしてください。"
...
2つの部分でアクティビティを設定します。サンプルファイルを使用して予想されるファイル構造を定義し、キャンペーンの実行時に読み込むファイルと、行をターゲティングディメンションに紐付ける方法を指定します。

次の手順に従って、**[!UICONTROL ファイルの読み込み]** アクティビティを設定します。
...
1. **[!UICONTROL 列]** セクションで、各列を展開し、そのプロパティを設定します。
   ...
   以下のプロパティは、各列で使用できます。 **[!UICONTROL データタイプ]**&#x200B;を選択すると、そのタイプに追加のオプションが表示されます。 以下のセクションを展開して、データタイプごとに完全なリストを表示します。
   ...
   * **[!UICONTROL 区切り記号]** – 列のUIに示すように、日付と時刻のコンポーネント間の文字。
...
1. 「**[!UICONTROL ターゲットファイル]**」セクションで、ファイルの提供方法を選択します。例えば、このリリースの手動アップロード用に&#x200B;**[!UICONTROL ローカルマシン]**&#x200B;からファイルをアップロードします。

1. アップロードするCSV ファイルまたはTXT ファイルを選択します。

   >[!CAUTION]
   >
   > ターゲットファイルが、サンプルファイルと同じ形式、列構造、列数に従っていることを確認します。 不一致は、実行中にエラーを引き起こす可能性があります。

1. ファイルのID列（各行を既存の受信者に一致させるために使用されるフィールド）を選択します（電子メールアドレスや顧客IDなど）。

1. 調整する&#x200B;**[!UICONTROL ターゲティングディメンション]**&#x200B;を選択します。

1. 設定が完了したら、UIが提供する場合は、マッピングされた行のサンプルをプレビューしてから確認します。
...
1. アウトバウンドトランジションを下流アクティビティに接続します。

既存の受信者に紐付けできない行は、オーディエンスから除外されます。 除外された行はキャンペーン実行ログに記録されます。キャンペーンは、一部の行が一致しなかったためだけに失敗するわけではありません。

## 配信でのファイルオーディエンスの使用 {#downstream}

**[!UICONTROL ファイルを読み込み]**&#x200B;がオーディエンスを解決したら、標準のオーケストレーションされたキャンペーンアクティビティを使用できます。

* **[チャネルアクティビティ](channels.md)** – 電子メール、SMS、プッシュ通知、ダイレクトメール。

* **[Enrichment](enrichment.md)**&#x200B;または&#x200B;**[Reconciliation](reconciliation.md)** – 必要に応じてワークテーブルデータをさらに調整またはリンクします。

[キャンペーンアクティビティのオーケストレーション方法を学ぶ](../orchestrate-activities.md)
...
* ダウンストリームアクティビティに渡されるオーディエンスから受け入れられる行。

* 拒否された行または調整されていない行は除外されます。カウントと理由は&#x200B;**実行ログ**&#x200B;に表示されます（例えば、読み込まれた合計行、受け入れられた行、拒否された行など）。

オーディエンスの解決は、標準のオーケストレーションされたキャンペーンインフラストラクチャの下で&#x200B;**100,000 – 行** CSVについて、約&#x200B;**60秒**&#x200B;以内に完了するように設計されています。

## 関連コンテンツ {#related}

* [ターゲティングディメンションの作成](../target-dimension.md)
* [オーディエンスを作成アクティビティ](build-audience.md)
* [オーディエンスを読み取りアクティビティ](read-audience.md)
* [紐付けアクティビティ](reconciliation.md)
