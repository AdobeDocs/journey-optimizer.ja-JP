...
**このページ：**&#x200B;では、オーケストレーションされたキャンペーンで重複排除ターゲティングアクティビティを使用して、ターゲットデータを使用する前に、インバウンドアクティビティの結果から重複したレコードを削除する方法を説明します。
...
>abstract="「**重複を識別するフィールド**」セクションで、「**属性を追加**」ボタンをクリックして、メールアドレス、名、姓など、同一の値によって重複を識別できるフィールドを指定します。フィールドの順序により、最初に処理するフィールドを指定できます。"
...
1. 「**[!UICONTROL 重複を識別するフィールド]**」セクションで、「**[!UICONTROL 属性を追加]**」ボタンをクリックして、メールアドレス、名、姓など、同一の値によって重複を識別できるフィールドを指定します。フィールドの順序により、最初に処理するフィールドを指定できます。
