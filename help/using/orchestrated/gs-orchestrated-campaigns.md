...
* **[参照アクティビティによる顧客との関与](engage-customers-uc.md)** - マルチステップのオーケストレーションキャンペーンを使用して、参照したが購入しなかったプロファイルに再び関与します。
* **[製品の可用性に関するユーザーへの通知](product-availability-uc.md)** - 顧客が興味を示した製品が再入荷した際に顧客に通知します。
* **[ウィッシュリスト項目の更新の送信](wishlist-uc.md)** - ウィッシュリスト項目が販売中または使用可能になった際に、パーソナライズされたメッセージをトリガーします。
