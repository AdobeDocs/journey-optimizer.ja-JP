...
**[!UICONTROL オーケストレーションキャンペーン]**&#x200B;は、次の2種類のターゲティングディメンションをサポートしています。

* **プロファイルターゲティングディメンション**: **プロファイル** スキーマを使用して直接受信者をターゲットにする、組み込みのデフォルト。
* **カスタムターゲティングディメンション**：リレーショナルスキーマを通じて受信者をターゲットにします（例：`Recipients`、`Visitors`、`Customers`）。 プロファイルレベルではなく、サブスクリプションや契約ごとに、エンティティレベルでメッセージを配信する必要がある場合に使用します。

カスタムターゲティングディメンションを設定する場合は、次の2つの重要な側面を定義します。
...
* **ターゲット可能スキーマ**
  ...
  >[!IMPORTANT]
  >
  > オーケストレーションされたキャンペーンでは、**プロファイル** スキーマと直接または関連する関係を持つ任意のスキーマをターゲットにできます。 主に 1:1 の関係で使用することを目的としていますが、関係パスがデータモデルで適切にモデル化されている限り、アカウント `>` 受信者などの 1:N の関係もサポートします。 これにより、メッセージ配信に対する正しいプロファイル ID を解決しながら、アカウントレベルのデータに基づいてターゲティングが可能になります。
