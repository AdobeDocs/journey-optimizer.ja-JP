...
**このページでは、** DDL ファイルをアップロードし、テーブル間の関係を定義し、オーケストレーションされたキャンペーンで使用するためにスキーマを既存のエンティティにリンクして、Adobe Experience Platformでリレーショナルスキーマを作成する方法を説明します。
...
1. 各スキーマとその列を設定し、プライマリキーとバージョン記述子が指定されていることを確認します。
