...
**[!UICONTROL Orchestrated Campaign]** には、そのための 2 つの主なスキーマが用意されています。
...
フィードバックベースのリターゲティングルールを使用すると、「メッセージフィードバック **[!UICONTROL スキーマでキャプチャされたメッセージ配信イベントに基づいて受信者をリターゲティングでき]** す。 これらのイベントには、メッセージの送信済み、開封済み、バウンス済み、スパムとしてマーク済みなどの結果が含まれます。

このデータを使用すると、以前にメッセージを受信した受信者を識別するルールを定義することで、特定の配信ステータスに基づいてフォローアップのコミュニケーションが可能になります。

1. 新しい&#x200B;**[!UICONTROL 調整されたキャンペーン]**&#x200B;を作成します。

1. **[!UICONTROL オーディエンスを作成]** アクティビティを追加し、ターゲティングディメンションを **[!UICONTROL 受信者（caas）]** に設定します。 「**[!UICONTROL 続行]**」をクリックします。

1. すぐに使い始めるには、組み込みの **[!UICONTROL キャンペーンフィードバック]** フィルターを使用して、メッセージ配信イベントに基づいて受信者をターゲットに設定できます。

   +++ 詳細な手順

   1. **[!UICONTROL ルールビルダー]** で「**[!UICONTROL フィルターを選択または保存]** をクリックし、リストから **[!UICONTROL キャンペーンフィードバック]** を選択します。

   1. フィルタールールを選択し、「**[!UICONTROL 送信済みメッセージ]** など、ターゲットにする **[!UICONTROL 動作]** を選択します。

   1. ![ フォルダーアイコン ](assets/do-not-localize/folder-search.svg) をクリックして、再ターゲットする特定のキャンペーンを選択します。 次の 2 つのオプションがあります。

      * **[!UICONTROL 特定のキャンペーンを選択]**：リストから特定のキャンペーンを選択して、そのキャンペーンで操作した受信者をリターゲットします。

      * **[!UICONTROL トランジションからキャンペーン]**：オーケストレーションされたキャンペーンの以前のアクティビティからキャンペーンを参照します。

   +++

1. または、カスタムルールを手動で作成することもできます。 **[!UICONTROL ルールビルダー]**&#x200B;で「**[!UICONTROL 条件を追加]**」をクリックし、**[!UICONTROL 属性ピッカー]**&#x200B;から「**[!UICONTROL メッセージフィードバック]**」を選択します。「**[!UICONTROL 確認]**」をクリックして、**メッセージフィードバックが存在するなど**&#x200B;の条件を作成します。
...
トラッキングベースのリターゲティングルールは、**[!UICONTROL メールトラッキング]** スキーマからのデータを使用して、メッセージとのインタラクションに基づいて受信者をターゲットに設定します。 メールの開封数やリンクのクリック数など、ユーザーのアクションをキャプチャします。
...
1. すぐに使い始めるには、組み込みの **[!UICONTROL キャンペーンフィードバック]** フィルターを使用して、メッセージ配信イベントに基づいて受信者をターゲットに設定できます。
   ...
   1. フィルタールールを選択し、ターゲットにする **[!UICONTROL 動作]** （**[!UICONTROL メッセージが開封されました]**、**[!UICONTROL メッセージがクリックされました]** など）を選択します。
...
1. または、カスタムルールを手動で作成することもできます。 **[!UICONTROL ルールビルダー]**&#x200B;で「**[!UICONTROL 条件を追加]**」をクリックし、**[!UICONTROL 属性ピッカー]**&#x200B;から「**[!UICONTROL メールトラッキング]**」を選択します。
