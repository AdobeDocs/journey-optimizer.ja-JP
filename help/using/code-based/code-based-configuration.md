...
**このページ：**&#x200B;では、アプリケーションまたはweb ページ内でコンテンツが配信および消費されるサーフェス、プラットフォーム、場所を定義するコードベースのエクスペリエンスチャネル設定を作成する方法について説明します。
...
>[!NOTE]
>
>アプリ実装チームは、明示的な API または SDK 呼び出しを行って、選択したコードベースのエクスペリエンス設定で定義されたサーフェスのコンテンツを取得する責任があります。 様々な顧客の実装について詳しくは、[この節](code-based-implementation-samples.md)を参照してください。
...
1. デバイスでのプレビューを有効にするには、「**[!UICONTROL プレビュー URL]**」フィールドに入力します。 この URL は、デバイスでプレビューをトリガーする際に使用する特定の URL をプレビューサービスに通知します。 [詳細情報](test-code-based.md#preview-on-device)
   ...
   * Android の場合：
      ...
      * [アプリのコンテキストへのディープリンクの作成](https://developer.android.com/training/app-links/deep-linking)

   * iOS の場合：

      * [アプリのカスタム URL スキームの定義](https://developer.apple.com/documentation/xcode/defining-a-custom-url-scheme-for-your-app)

      * [アプリでのユニバーサルリンクのサポート](https://developer.apple.com/documentation/xcode/supporting-universal-links-in-your-app)
