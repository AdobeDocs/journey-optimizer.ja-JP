...
クエリは、定義した期間に対して、1 日あたりにジャーニーにエントリしたプロファイルの数を返します。 プロファイルが別の ID を使用してエントリした場合は、2 回カウントされます。再エントリを有効にすると、別の日にジャーニーに再エントリした場合、プロファイル数が複数日にわたって重複する場合があります。

## 読み取りオーディエンスに関連するクエリ {#read-segment-queries}

**オーディエンス書き出しジョブの完了に要した時間**
...
クエリは、オーディエンス書き出しジョブがキューに入れられた時刻と最終的に終了した時刻との間の時間差（分単位）を返します。
...
**特定のジャーニーバージョンに関するオーディエンスの読み取りの概要**
...
* トピックまたは書き出しジョブ作成のエラー（オーディエンス書き出し API 呼び出しのタイムアウトを含む）
...
**特定のジャーニーバージョンのオーディエンス読み取りエラーを取得する**
...
**すべての書き出しジョブの集計指標（オーディエンスの書き出しジョブと破棄）の取得**
...
**設定されたものとは異なるオーディエンス適合のため、プロファイルが破棄されました**
...
このクエリは、オーディエンスの適合が間違っているので、ジャーニーバージョンによって破棄されたすべてのプロファイル ID を返します。

**特定のプロファイルに対する他の理由で破棄されたオーディエンスの選定イベント**
...
このクエリは、プロファイルに対する他の理由で破棄されたすべてのイベント（外部イベント/オーディエンスの選定イベント）を返します。
