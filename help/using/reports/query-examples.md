...
このページでクエリを実行する前に、次を確認します。

* **Adobe Experience Platform クエリサービスへのアクセス** - Adobe Experience Platform サンドボックスで [ クエリサービス ](https://experienceleague.adobe.com/docs/experience-platform/query/home.html?lang=ja){target="_blank"} にアクセスできる必要があります。
* **データセット利用可能** - クエリは、`journey_step_events` のデータセットをターゲットにします。 **Experience Platform/データセット** を使用して、データセットが存在し、サンドボックスにデータが含まれていることを確認します。
* **ジャーニーバージョン ID を修正** – ほとんどのクエリには `journeyVersionID` が必要です。 **ジャーニー/[ ジャーニー ]/プロパティ** の下のJourney Optimizerで検索するか、`journeyVersionName` を使用して最初にデータセット内で検索します。
* **スキーマフィールドの値** - クエリで使用するフィールドに、対応するスキーマに関連付けられた値があることを確認します。 空のフィールドは、エラーのない結果を返しません。

>[!TIP]
>
>**クエリサービスを初めて使用する場合は、** Adobe Experience Platform[ を開く ](https://experience.adobe.com/)、**クエリサービス/クエリ** に移動し、以下の例を貼り付け、プレースホルダー値（例：`<journeyVersionID>`、`<last x hours>`）を置き換え、「**実行**」を選択します。

## 適切なクエリを見つける {#find-query}

| 私は… | に移動 |
|---|---|
| ジャーニーにエントリしたプロファイルのカウント | [ 基本的なユースケース ](#common-queries) |
| 特定のプロファイルのジャーニーパスのデバッグ | [ プロファイルベースのクエリ ](#profile-based-queries) |
| オーディエンスを読み取りの実行またはエラーを調査 | [ オーディエンスクエリの読み取り ](#read-segment-queries) |
| メッセージまたはアクションエラーのトラブルシューティング | [ メッセージおよびアクションエラー ](#message-action-errors) |
| オーディエンスの選定の破棄を分析 | [ オーディエンスの選定クエリ ](#segment-qualification-queries) |
| 外部イベントまたはビジネスイベントのデバッグ | [ イベントベースのクエリ ](#event-based-queries) |
| カスタムアクションエンドポイントのパフォーマンスの監視 | [ カスタムアクションクエリ ](#query-custom-action) |
| エンゲージメント可能なプロファイルとライセンス使用状況の追跡 | [ エンゲージメント可能なプロファイルクエリ ](#engageable-profiles-queries) |
...
* **eventType** - 破棄の原因となったビジネスルールのタイプを指定します。
   ...
   * `forcedDiscardDueToQuietHours`：クワイエットアワーに保持されるプロファイルのガードレール制限に達したので、プロファイルは強制的に破棄されました
