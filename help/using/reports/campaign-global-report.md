...
The **[!UICONTROL アプリ内概要]** グラフには、該当する期間中のアプリ内インプレッション数およびインタラクション数の変化が表示されます。
...
* **[!UICONTROL インタラクション率]**:Web ページに対するアクションの割合。 これには、クリックやその他のインタラクションなど、ユーザーが実行したすべてのアクションが含まれます。

The **[!UICONTROL Web サマリ]** グラフには、該当する期間中の web エクスペリエンス（インプレッション数、個別インプレッション数およびインタラクション数）の変化が表示されます。

The **[!UICONTROL 要素別インタラクション数]** 表には、Web ページ上の様々な要素に対する訪問者のエンゲージメントに関する主な情報の詳細が示されます。
+++
