...
* **[!UICONTROL 送信数]**：アプリがアプリ内キャンペーンをリクエストした回数。 キャンペーンデータがキャッシュされていない場合、ユーザーセッションごとに複数のリクエスト（起動時や再読み込み時など）があると、この値が一意のユーザー数を超える可能性があります。

* **[!UICONTROL インバウンドトリガー]**：アプリがアプリ内メッセージの表示を考慮した回数。 この数は、アプリ側のルールによってメッセージが表示されない場合、合計送信数よりも少ない可能性があります。
