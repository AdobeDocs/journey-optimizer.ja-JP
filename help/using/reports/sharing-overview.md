...
>[!NOTE]
>
>プロファイルレベルのステップイベントに加えて、システムでは&#x200B;**オーディエンスを読み取り**&#x200B;アクティビティの内部イベントも生成されます。 `segmentExportJob` イベントと呼ばれるこれらのイベントは、オーディエンスを読み取りノードのライフサイクル（書き出しジョブの作成、キューへの登録、完了、エラーなど）を記録し、個々のプロファイルごとではなく、オーディエンスを読み取りアクティビティごとに生成されます。 その結果、これらのイベントには関連付けられたプロファイル識別子（UPMID）がない場合があります。 これらの内部イベントは、オーディエンスを読み取りのパフォーマンスのモニタリングとトラブルシューティングに役立ち、[serviceEvents の節](../reports/sharing-field-list.md#servicevents-field)の説明に従ってフィールドを使用してクエリを実行できます。 segmentExportJob イベントの操作方法に関するクエリの例について詳しくは、[オーディエンスを読み取りに関連するクエリ](../reports/query-examples.md#read-segment-queries)を参照してください。
