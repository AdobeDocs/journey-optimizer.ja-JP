...
title: 除外リスト
description: 送信中に発生する除外の詳細を説明します
feature: Reporting
...
キャンペーンレポートを表示する場合、*除外* 指標は次のように計算されます。

**除外=一意の除外+重複除外イベント**

つまり、1 つのプロファイルが複数回除外されている場合（同じプロファイルに対する複数の除外イベントが原因など）、各イベントが除外数の合計にカウントされます。 その結果、*配信済み* と *除外* の合計が、元のターゲットオーディエンスサイズを超える場合があります。 この動作は予期される動作で、システムでの除外イベントの追跡方法を反映しています。
...
重複する除外イベントが除外数に含まれるので、合計はターゲットオーディエンスを超えています。

具体的な除外理由の詳細については、次の表を参照してください。
