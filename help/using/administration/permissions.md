...
- **TL;DR:**&#x200B;このページでは、製品管理者とシステム管理者が、「権限」製品の3つのロール管理タスク（既存のロールのユーザーへの割り当て、ロールの権限の編集、特定の権限とサンドボックスを使用した新しいカスタムロールの作成）について説明します。

**インテント：**

- Journey Optimizerのユーザーへのビルトインロールまたはカスタムロールの割り当て
- 既存の役割の権限の編集（権限の追加または削除）
- 特定の権限とサンドボックスの割り当てを使用して、新しいカスタムロールを作成します
- 誰が役割と権限管理を実行する権限を持っているかを把握します

**用語集：**

- **役割**：組織&#x200B;*（製品固有）*&#x200B;内のアクセスを管理するために使用される、同じ権限とサンドボックスを共有するユーザーのコレクション
- **権限製品**: ロール、権限、およびサンドボックスが設定されている&#x200B;*（製品固有）* Adobe CX Enterprise インターフェイス（[!DNL Permissions]経由でアクセス）
- **組み込みの役割**：定義された権限セットを持つ既存の役割で、カスタム設定なしで即時に割り当てることができます&#x200B;*（製品固有）*

**ガードレール：**

- 役割を割り当て、編集、または作成できるのは、製品またはシステム管理者のみです（ページの重要なメモに記載されているように、ハード前提条件）
- 組み込みロールまたはカスタムロールに対して行われた変更は、そのロールに割り当てられたすべてのユーザーに影響します（ページの重要なメモに記載されているように）

**用語：**

- 正規名：権限製品 – バリアント：Adobe権限、権限UI、Adobe CX Enterprise権限
- 混乱しないでください：「役割の割り当て」（既存の役割へのユーザーの追加）≠「役割の作成」（独自の権限とサンドボックスをゼロから定義して新しい役割を定義）
- 「既存の役割を編集」（既存の役割に対する権限またはサンドボックスの変更）≠「新しい役割を作成」（既存の役割またはそのユーザーに影響を与えずに新しい役割を作成）

**FAQ:**

- **Q: Journey Optimizerのユーザーにロールを割り当てることができるのは誰ですか？**  – 製品またはシステム管理者のみ。
- **Q：組み込みロールの権限を編集するとどうなりますか？**  – 変更は、現在その役割に割り当てられているすべてのユーザーに影響します。
- **Q: ロールを管理するには、製品のどの部分に行けばよいですか？**  – 権限製品で、「役割」タブに移動します。
- **Q：役割が割り当てられた後、ユーザーは通知を受け取りますか？**  – はい。ユーザーは、インスタンスにリダイレクトするメールを自動的に受信します。
