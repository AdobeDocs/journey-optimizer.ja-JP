...
* **TL;DR:** Journey Optimizer ロールは、上位レベルの権限から構築されており、各ロールには、ジャーニー、キャンペーン、決定、チャネル設定などをまたいで、ユーザーがリソースを読み取り、書き込み、公開または削除するために必要な特定の下位レベルのAPI権限がバンドルされています。

**インテント：**

* 上位レベルの権限と下位レベルの権限の違いを理解する
* 各上位レベルの権限によって付与される下位レベルの権限を特定します
* ジャーニー、キャンペーン、意思決定管理、チャネル設定、オーケストレーションされたキャンペーンに合わせて役割を正確に設定できます
* コンテンツ生成にAI アシスタントのアクセス権を付与する
* ジャーニーの管理権限と比較したジャーニーの公開権限について説明します

**用語集：**

* **高レベル権限**:1つ以上の低レベル権限&#x200B;*（製品固有）*&#x200B;を含む役割（ジャーニーの管理、ジャーニーの公開など）に割り当てられた名前付き権限
* **低レベル権限**：高レベル権限&#x200B;*（製品固有）*&#x200B;に派生し、含まれる詳細なAPI レベル権限（例：journeys.read、journeys.write）
* **役割**：組織&#x200B;*（製品固有）*&#x200B;内で同じ権限とサンドボックスを共有するユーザーのコレクション

**用語：**

* 「高レベルの権限」（ロールに割り当て可能な名前の権限）≠「低レベルの権限」（基礎となる詳細なAPI権限、直接割り当て可能ではない）を混同しないでください
* 混同しないでください。「ジャーニーの管理」（作成、編集、削除、停止（ライブ、テストモード、ドライランを含む）≠「ジャーニーの公開」（公開、テストモードの開始、ドライランの開始、一時停止、ジャーニーの再開を可能にする）
* 混同しないでください。「ジャーニーイベント、データソース、アクションの管理」（イベント、ソース、アクションに関する完全なCRUD）≠「ジャーニーイベント、データソース、アクションの表示」（これらのオブジェクトへの読み取り専用アクセス）
* 「コンテンツを生成」（Journey OptimizerのAI アシスタントへのアクセス）≠他のジャーニーまたはキャンペーンの権限を混同しないでください
* 混同しないでください。「テストモード」（ジャーニーの公開およびジャーニーの管理で開始または停止できるジャーニー実行モードとして参照）≠「ドライ実行」（同じ権限でも参照される別のジャーニー実行モード）

**FAQ:**

* **Q: ジャーニーの管理権限を使用すると、ユーザーはジャーニーを公開できますか？**  – いいえ。ジャーニーを公開するには、ジャーニーを公開するための高度な権限が別途必要です。
* **Q: 「コンテンツを生成」権限は何を付与しますか？** — Journey OptimizerのAI アシスタントへのアクセス。
* **Q: ユーザーはジャーニー管理権限を持たずにジャーニーイベントを設定できますか？**  – はい。ジャーニーイベント、データソース、アクションの管理は、イベント、データソース、アクション設定をカバーする個別の高レベル権限です。
* **Q: ジャーニーレポートの表示にはどのような低レベルの権限が含まれていますか？** — journeys_report.readおよびmessages_report.readに加え、Adobe Experience Platformのdatasets.read、queries.read、queries.write、およびqueries.deleteが含まれます。
