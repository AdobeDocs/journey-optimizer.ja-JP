...
>
>* 実行時のパーソナライゼーションに大きく依存するメール（`#each` ループ、ネストされたオブジェクト、大きなペイロードデータなど）を作成する場合、特に、ペイロードから大量のコンテキストデータを使用する API トリガーキャンペーンでは、ミラーページ URL が過度に大きくなる可能性があります。 これは、ブラウザーまたはメールクライアントで HTTP エラー（404、422、502）を引き起こす可能性があります。 Adobeでは、動的フィールドの幅と深さを制限し、複雑なフラグメントへの依存を減らし、パーソナライゼーション構造をフラット化してリンクのエラーを防ぐことをお勧めします。
>
