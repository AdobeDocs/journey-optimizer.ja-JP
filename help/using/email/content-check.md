...
>title="メールコンテンツの検証"
>abstract="コンテンツチェックでは、メールを送信する前に、HTML や CSS に関する問題を自動的に検出します。 Gmail や Microsoft Outlook でレンダリングが壊れる場合がある、サポートされていないタグ、空の div、サイズ制限にフラグを設定します。 問題は、エラー、警告または情報通知として表示され、コンテキストの詳細や、使用可能な場合はワンクリック修正も含まれます。"
...
* **条件付きブロック （if-else）** - **すべての分岐**&#x200B;は、送信時間まで条件が評価されないため、オーサリング時のサイズ推定値に含まれます
...
* **HTML サイズの最適化**:「**[!UICONTROL HTML サイズの最適化]**」オプションが有効になっている場合、送信時に空白、コメント、および冗長な文字が削除され、最終的なペイロードが削減されます。 [詳細情報](create-email.md#optimize-html-size)
