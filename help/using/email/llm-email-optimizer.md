...
この機能を利用することで、メッセージの専用テキストバージョンを絞り込むことができ、AIが支援する受信トレイの体験では、自動生成されたテキストや無関係なコンテキストではなく、オファー、CTA、詳細が表示される可能性が高まります。
...
これらの問題を回避するために、[!DNL Journey Optimizer]はメッセージの追加のテキストバージョンを作成し、クーポン、割引範囲、コールトゥアクション、その他の優先事項が明確な線形コピーで前面に表示されるようにします。

>[!NOTE]
>
>この専用テキストバージョンは、メッセージのデフォルトまたはカスタムプレーンテキストバージョンとは異なります。 [詳細情報](text-version-email.md)
...
>
>「AI受信トレイ用にメールテキストを最適化」機能は、Journey Optimizerで専用のテキストバージョンを生成するだけです。サードパーティアシスタントがメッセージをどのように解釈または表示するかを保証するものではありません。 サードパーティの受信トレイ AIの[制限とリスクについて詳しくは、こちらを参照してください](#inbox-ai-risks)。
...
* **受信トレイのQ&amp;A**&#x200B;の制御 – 受信者が&#x200B;*電子メールの内容*&#x200B;または&#x200B;*オファーの内容*&#x200B;をアシスタントに尋ねることを想定している場合、強力なai受信トレイのバージョンは、部分的な要約を削減し、承認済みコピーに関連付けられていないwebで補完された回答への依存を回避します。
...
1. 生成されたバージョンが表示されます。
   ...
   >[!NOTE]
   >
   >**AI インボックス用にメール テキストを最適化**&#x200B;しても、HTMLのデザイン、レイアウト、画像は変更されません。

1. 自動的に生成されたコンテンツに変更を加えるには、**[!UICONTROL 編集を有効にする]** トグルを選択し、必要に応じてコンテンツを手動で編集します。

1. バージョンに問題がなければ、「**[!UICONTROL メールを最適化]**」ボタンをクリックして確認します。

1. これで、電子メールがAI受信トレイ用に最適化されました。

1. 最適化されたバージョンにアクセスまたは編集するには、「**[!UICONTROL AI インボックス向けに最適化]**」ボタンをクリックします。

   ![電子メールDesignerの「再最適化」ボタン ](assets/optimize-ai-inbox-optimized-button.png){zoomable="yes" width="80%"}

1. 最適化バージョンが表示されます。 **[!UICONTROL 最適化を削除]**&#x200B;するか、**[!UICONTROL 再最適化]**&#x200B;をクリックして新しいバージョンを生成できます。

   ![以前に電子メール Designerで最適化されたバージョン ](assets/optimize-ai-inbox-optimized-version.png){zoomable="yes" width="80%"}

   >[!NOTE]
   >
   >HTMLの元のコンテンツに変更を加えた場合は、AI受信箱のバージョンを再最適化する必要があります。
...
* **ブランドおよび法的な露出** – 不正確または不完全なAIによる要約でも、プロモーション、条件、オプトアウト言語に関する顧客の混乱や紛争が生じる可能性があります。 **AI受信箱に合わせて電子メールを最適化**&#x200B;しても、最適化されたバージョンの電子メールがサードパーティのモデルによって正確に再現されるとは限りません。
