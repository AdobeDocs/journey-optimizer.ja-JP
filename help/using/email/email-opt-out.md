...
を切り替えた場合 **[!UICONTROL リストの購読解除を有効にする]** オプション リスト購読解除ヘッダーに関しては、両方の方法を有効にすることをお勧めします。 **宛先（購読解除）** および **ワンクリック登録解除 URL**. すべてのメールクライアントが HTTP メソッドをサポートしているわけではありません。 宛先のリスト – 購読解除機能を別の受信者を選択する機能として提供すると、送信者のレピュテーションを保護し、すべての受信者が購読解除機能にアクセスできるようになります。 [詳細情報](email-settings.md#list-unsubscribe)
