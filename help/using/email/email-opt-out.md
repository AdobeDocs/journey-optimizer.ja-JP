...
>[!NOTE]
>
>すべてのマーケティングのメッセージには、オプトアウトリンクを含める必要があります。トランザクションメッセージの場合、これは必須ではありません。メッセージカテゴリ **[!UICONTROL マーケティング]** または **[!UICONTROL トランザクション]** は、[ チャネル設定 ](../configuration/channel-surfaces.md#email-type) レベルで、またメッセージの作成時に定義されます。

メールコンテンツに購読解除リンクを挿入するには、次の操作を実行します。

* ワンクリック登録解除 URL をメールヘッダーに追加します。チャネル設定レベルで **[!UICONTROL List-Unsubscribe Header]** オプションを有効にすると、メールヘッダーにオプトアウトリンクが追加されます。 [詳しくは、メールヘッダーのオプトアウトを参照してください](#unsubscribe-header)
...
[List-Unsubscribe を有効にする ](email-settings.md#list-unsubscribe) 切替スイッチをチャネル設定レベルで有効にして、この設定を使用するメールにメールヘッダーのワンクリック購読解除 URL が含まれるようにする必要があります。
...
Adobe Journey Optimizerでは、自動生成されたワンクリック購読解除 URL と宛先アドレスをメールヘッダーに含めたり、メール本文にワンクリックオプトアウト URL を含めたりして、メール設定を指定できます。受信者がワンクリックオプトアウトリンクをクリックすると、受信者の購読解除リクエストが適切に処理されます。
...
E メールクライアントと [E メール設定の購読解除設定 ](email-settings.md#list-unsubscribe) によっては、E メールヘッダーの購読解除リンクをクリックすると、次のような影響が出ることがあります。
...
1. 登録解除後にユーザーがリダイレクトされるランディングページの URL を入力します。このページは、オプトアウトが成功したことを確認できるように表示されます。
   ...
   >[!NOTE]
   >
   >[ チャネル設定レベル ](email-settings.md#list-unsubscribe) で **[!UICONTROL List-Unsubscribe]** オプションを有効にし、デフォルトのワンクリックオプトアウト URL オプションをオフにした場合、ユーザーがメールヘッダーの購読解除リンクをクリックしたときに、この URL が使用されます。 [詳細情報](#unsubscribe-header)
