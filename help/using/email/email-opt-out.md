...
**このページ：**&#x200B;では、メールのヘッダーまたは本文のワンクリックのオプトアウトリンクや、ランディングページを通じた2段階のオプトアウトなど、購読解除オプションをメールに追加して、受信者が今後のコミュニケーションを停止できるようにする方法について説明します。
...
### 登録解除フローについて {#send-message-unsubscribe-link}

ランディングページへの登録解除リンクを設定したら、メッセージを入力してサブスクライバーに送信できます。

ランディングページのオプトアウトフロー全体が正常に完了するには、次のイベントが順番に発生することが予想されます。

1. **クリック** - メッセージを受信した受信者は、メールの登録解除リンクをクリックします。

1. **訪問** - ランディングページが読み込まれ、受信者に表示されます。

   ![](assets/opt-out-lp-example.png)

1. **送信** - 受信者は、ランディングページの登録解除ボタンをクリックしてオプトアウトフォームを送信します。

   >[!WARNING]
   >
   >メール内で登録解除リンクをクリックした場合にのみ、ランディングページが開きます。 購読解除を完了し、プロファイルの同意を更新するには、受信者は、**ランディングページのオプトアウトボタンをクリックしてフォームを送信**&#x200B;する必要があります。

1. **登録解除** - 登録解除リクエストが処理されます。 オプトアウトした受信者は、オプトアウトが成功したことを示す確認メッセージ画面にリダイレクトされます。
...
このイベントシーケンスにより、登録解除プロセスが適切に追跡され、プロファイルの同意環境設定がシステムに正確に反映されます。 このフローのいずれかの手順が欠落しているか、順序どおりに実行されない場合は、オプトアウトの実装に問題があることを示すことがあり、調査する必要があります。
