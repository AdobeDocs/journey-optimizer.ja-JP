...
title: 電子メールおよびSMS メッセージでのディープリンクの使用と設定
description: IOSおよびAndroid アプリで電子メールやSMS コンテンツにディープリンクを追加する方法と、ディープリンク処理を実装する方法について説明します。
feature: Email, SMS
...
モバイルアプリの電子メールやSMSでディープリンクを使用するには、以下の設定手順を完了します。
...
この節では、一般的な&#x200B;**HTTPS**&#x200B;設定（ユニバーサルリンクとアプリリンク）で、単一のURLで次のことが可能になるように、[!DNL Adobe Journey Optimizer]とのモバイルディープリンクを実装する方法について説明します。
...
* **アプリ側ルーティングの検証**: ディープリンクが予想される画面を開かない場合は、アプリ側ルーティングとURL形式（ホスト/パス/クエリおよびURL エンコーディング）を検証します。
...
* ディープリンクを使用してプルーフを作成し、iOSとAndroidのデバイス（インストール済みおよびインストールされていないシナリオ）でプルーフをクリックします。
* 検証：
...
はい。 デリゲートされたすべてのサブドメインにディープリンクを設定する場合は、機能をサポートする必要がある各サブドメインに対してAASAと`assetlinks.json`設定をリクエストします。
