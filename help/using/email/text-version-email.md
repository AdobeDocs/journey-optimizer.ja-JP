...
セキュリティの観点から見ると、HTML E メールには悪意のあるスクリプト、トラッキングピクセル、豊富な形式やリンクに依存するフィッシング攻撃などのリスクがあるので、プレーンテキスト版を提供することが重要です。 プレーンテキストは攻撃対象を減らし、セキュリティを重視する受信者や、HTMLを制限または削除する企業の電子メールシステムで好まれることが多いです。 両方のバージョンを指定すると、受信者はセキュリティとプライバシーの要件に合った形式を選択できます。
...
## カスタムテキストバージョンを使用 {#plain-text-default-custom}
...
1. その後、必要に応じてカスタムのプレーンテキストバージョンを編集できます。
