...
さらに、 **[!UICONTROL 返信先（E メール）]** アドレスの場合は、有効な MX レコード設定を持つサブドメインを使用してください。使用しない場合、E メールの表面処理が失敗します。

E メールサーフェスの送信時にエラーが発生した場合は、入力したアドレスのサブドメインに対して MX レコードが設定されていないことを意味します。 対応する MX レコードを設定する場合は管理者に問い合わせるか、有効な MX レコード設定を持つ別のアドレスを使用してください。

>[!NOTE]
>
>入力したアドレスのサブドメインが [完全に委任された](../configuration/delegate-subdomain.md#full-subdomain-delegation) Adobeについては、担当のAdobeアカウント担当者にお問い合わせください。
...
さらに、 **[!UICONTROL Bcc 電子メール]** アドレスの場合は、有効な MX レコード設定を持つサブドメインを使用してください。使用しない場合、E メールの表面処理が失敗します。
...
>[!NOTE]
>
>テキスト値の入力と、式エディターからのコンテキスト属性の使用を組み合わせることができます。各 **[!UICONTROL 値]** フィールドには、5 KB までの文字数を含めることができます。
