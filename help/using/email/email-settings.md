...
* **[!UICONTROL エラーメール]**：メールを配信してから数日後に ISP で発生したすべてのエラー（非同期バウンス）は、このアドレスで受信されます。また、このアドレスには、不在通知とチャレンジ応答も受け取られます。

  Adobeにデリゲートされていない特定の電子メールアドレスに対して不在通知とチャレンジ応答を受け取る場合は、 [前進過程](#forward-email). その場合は、このインボックスにランディングする E メールを処理するために、手動または自動のソリューションを用意してください。
...
返信に使用されるインボックスは、不在通知とチャレンジ応答を除くすべての返信メールを受信します。これは、 **[!UICONTROL エラーメール]** 住所。
...
で受信したすべての電子メールを特定の電子メールアドレスに転送するには [!DNL Journey Optimizer] デリゲートされたサブドメインについては、Adobeカスタマーケアにお問い合わせください。

>[!NOTE]
>
>サブドメインが **[!UICONTROL 返信先（E メール）]** アドレスはAdobeに委任されていないので、このアドレスでは転送は機能しません。

以下を指定する必要があります。

* 選択した転送メールアドレス。転送メールアドレスドメインは、アドビにデリゲートされたサブドメインと同じにはできないことに注意してください。
* サンドボックス名。
* 転送 E メールアドレスを使用するサブドメインのサーフェス名。
...
転送メールアドレスは、Adobeが設定します。 これには 3～4 日かかる場合があります。

完了すると、 **[!UICONTROL 返信先（E メール）]** および **[!UICONTROL エラーメール]** アドレスは、指定した特定の電子メールアドレスに転送されます。
