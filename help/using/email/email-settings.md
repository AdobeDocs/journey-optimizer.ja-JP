...
* **[!UICONTROL 送信元のメールプレフィックス]**：コミュニケーションに使用するメールアドレス。
* **[!UICONTROL 返信先名]**：受信者がメールクライアントソフトウェアの **返信** ボタンをクリックしたときに使用する名前。
* **[!UICONTROL メールへの返信]**：受信者がメールクライアントソフトウェアの **返信** ボタンをクリックしたときに使用するメールアドレス。 [詳細情報](#reply-to-email)
* **[!UICONTROL エラーのメールプレフィックス]**：メールを配信してから数日後に ISP が生成したすべてのエラー（非同期バウンス）は、このアドレスで受信します。 また、このアドレスでは、不在通知とチャレンジ応答も受信されます。
...
>[!NOTE]
>
>**[!UICONTROL 送信元メールのプレフィックス]** および **[!UICONTROL エラーメールのプレフィックス]** アドレスは、現在選択されている [ デリゲートされたサブドメイン ](../configuration/about-subdomain-delegation.md) を使用してメールを送信します。 例えば、デリゲートされたサブドメインが *marketing.luma.com* の場合は次のようになります。
>* *連絡先* を **[!UICONTROL 送信者のメールプレフィックス]** として入力します – 送信者のメールは *contact@marketing.luma.com* です。
>* *エラー* を **[!UICONTROL エラーのメールアドレスのプレフィックス]** として入力します – エラーアドレスは *error@marketing.luma.com* です。
...
**[!UICONTROL メールに返信]** アドレスを定義する場合、有効なアドレスで、正しい形式でタイプミスがない限り、任意のメールアドレスを指定できます。
...
さらに、**[!UICONTROL メールへの返信]** アドレスを定義する場合は、有効な MX レコード設定を持つサブドメインを使用します。使用しない場合、メール設定の処理が失敗します。
...
>[!NOTE]
>
>**[!UICONTROL 返信先メール]** アドレスに使用されるサブドメインがAdobeにデリゲートされていない場合、転送はこのアドレスでは機能しません。
...
完了すると、**[!UICONTROL 返信先メール]** および **エラーメール** アドレスで受信したすべてのメッセージ、および **送信者メール** アドレスに送信されたすべてのメールが、指定した特定のメールアドレスに転送されます。

>[!NOTE]
>
>デフォルトでは、転送が有効になっていない場合、**送信元メール** アドレスに直接送信されたメールは破棄されます。
