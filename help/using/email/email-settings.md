...
## 配信停止済みメールアドレスへの送信 {#send-to-suppressed-email-addresses}

>[!CONTEXTUALHELP]
>id="ajo_surface_suppressed_addresses"
>title="抑制リストの優先順位を上書き"
>abstract="スパムが原因で E メールアドレスがAdobe Journey Optimizer抑制リストに登録されている場合でも、プロファイルにトランザクションメッセージを送信することもできます。 このオプションはデフォルトでは無効です。"
>additional-url="https://experienceleague.adobe.com/docs/journey-optimizer/using/configuration/monitor-reputation/manage-suppression-list.html" text="抑制リストの管理"

>[!IMPORTANT]
>
>このオプションは、 **[!UICONTROL トランザクション]** 電子メールのタイプ。 [詳細情報](#email-type)

In [!DNL Journey Optimizer]、ハードバウンス、ソフトバウンスおよびスパムの苦情としてマークされたすべての E メールアドレスは、 [抑制リスト](../configuration/manage-suppression-list.md) ジャーニーやキャンペーンでの送信から除外されました。

ただし、 **transactional** ユーザーによるスパムの苦情により E メールアドレスが抑制リストに登録されている場合でも、プロファイルにタイプします。

実際、トランザクションメッセージには、通常、注文の確認やパスワードのリセット通知など、有用で期待される情報が含まれます。 したがって、顧客がマーケティングメッセージの 1 つをスパムとして報告した場合でも、ほとんどの場合は、このタイプの非商用 E メールを顧客に受け取ってもらいたいと考えます。

スパムの苦情によって抑制された E メールアドレスをトランザクションメッセージオーディエンスに含めるには、 **[!UICONTROL 配信停止済みメールアドレスに送信]** 」セクションに入力します。
...
配信品質のベストプラクティスとして、このオプションは、オプトアウトした顧客が連絡を受けないように、デフォルトで無効になっています。 ただし、このデフォルトオプションを変更すると、で顧客にトランザクションメッセージを送信できるようになります。

このオプションを有効にすると、顧客がマーケティング E メールをスパムとしてマークしても、そのような顧客は現在の表面を使用してトランザクションメッセージを受け取ることができます。 配信品質のベストプラクティスに従って、必ずオプトアウト設定を管理してください。
...
>additional-url="https://experienceleague.adobe.com/docs/journey-optimizer/using/configuration/seed-lists.html#create-seed-list" text="シードリストを作成"
