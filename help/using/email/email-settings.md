...
**このページでは：**&#x200B;電子メールの種類、サブドメイン、IP プール、リストの登録解除、ヘッダーパラメーター、BCC、シードリスト、再試行、URL トラッキング、実行アドレスなど、チャネル設定で電子メール設定を定義する方法について説明します。
...
>abstract="スパムの苦情が原因でメールアドレスが Adobe Journey Optimizer の抑制リストに登録されている場合でも、プロファイルへのトランザクションメッセージの送信を決定することができます このオプションはデフォルトでは無効です。"
