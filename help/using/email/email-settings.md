...
が含まれる **E メール タイプ** セクションで、サーフェスのメッセージのタイプを選択します。 **[!UICONTROL Marketing]** または **[!UICONTROL トランザクション]**.

* を選択 **Marketing** プロモーションメール用（小売店の毎週のプロモーションなど）。 これらのメッセージには、ユーザーの同意が必要です。

* を選択 **トランザクション** 注文確認、パスワードリセット通知、配信情報など、非商用メールの場合。 これらのメールは、アドビからのお知らせの&#x200B;**登録解除**&#x200B;したプロファイルに送信できます。これらのメッセージは、特定のコンテキストでのみ送信できます。
...
このオプションはデフォルトで有効です。有効にしたままにすると、次のような登録解除リンクが電子メールヘッダーに自動的に含まれます。

![](assets/preset-list-unsubscribe-header.png)

このオプションを無効にした場合、メールヘッダーに購読解除リンクは表示されません。

から同意レベルを選択できます **同意レベル** ドロップダウンリスト。 チャネルまたはプロファイル ID に固有です。 この設定に基づいて、ユーザーがメールの list unsubscribe header リンクを使用して購読解除すると、Adobe Journey Optimizerでは、チャネルレベルまたは ID レベルで同意が更新されます。
