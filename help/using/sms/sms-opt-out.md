...
SMS API 認証情報にカスタムのオプトアウトキーワードを定義すると、上記のデフォルトの受信キーワードが上書きされます。 デフォルトのキーワード（STOP、QUIT、CANCEL、END、UNSUBSCRIBE など）が機能するように保持するには、カスタムキーワードと共に、SMS 設定の「オプトアウトキーワード」フィールドに明示的に含めます。 そうしないと、カスタムキーワードのみが認識され、デフォルトのキーワードではオプトアウト操作がトリガーされなくなります。
