...
1. 「**[!UICONTROL ファジーオプトアウト]**」オプションを有効にすると、オプトアウトキーワード（「CANCIL」など）に似たメッセージが検出され、「**[!UICONTROL ファジー自動返信]**」フィールドで確認返信がカスタマイズされます。

   **[!UICONTROL ファジーオプトアウト]** は、メッセージが定義済みのオプトアウトキーワードと完全には一致しない場合でも、ユーザーが購読解除を希望していることを示す SMS メッセージを識別します。 一般的なオプトアウトフレーズや特定の攻撃用語を検出できるため、キャンペーンでユーザーの好みを尊重し、コンプライアンスを維持するのに役立ちます。
