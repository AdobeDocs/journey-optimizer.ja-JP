...
1. 以下で説明するように、SMS API 資格情報を設定します。
   ...
   | カスタム受信キーワード | バッチベースのアクションに対して、同意に関連しない一意のキーワード（割引、オファー、登録など）を定義します。 これらのキーワードはプロファイル内の属性として取得され、保存されるので、ユーザーはジャーニー内でストリーミングセグメントの選定をトリガーし、カスタマイズされた応答やアクションを提供できます。 |
