...
1. 以下で説明するように、SMS API 資格情報を設定します。
   ...
   | ダブルオプトインキーワード | ダブルオプトイン処理をトリガーするキーワードを入力します。ユーザープロファイルが存在しない場合は、確認が成功すると作成されます。複数のキーワードの場合は、コンマ区切り値を使用します。[詳しくは、SMS ダブルオプトインを参照してください](https://video.tv.adobe.com/v/3440273/?captions=jpn&learn=on)。 |
   ...
   | カスタム受信キーワード | バッチベースのアクションに対して、同意に関連しない一意のキーワード（割引、オファー、登録など）を定義します。 これらのキーワードは取得され、プロファイルに属性として保存されるので、バッチ化されたセグメントの選定をジャーニー内でトリガー設定し、カスタマイズされた応答やアクションを提供できるようになります。 |
   | デフォルトのインバウンド返信メッセージ | 定義されたどのキーワードにも一致しない受信 SMS をエンドユーザーが送信した場合に送信されるデフォルトの返信を入力します。 |
...
>[!AVAILABILITY]
>
> 高度な RCS メッセージは、Sinch が管理する直接アカウントでのみ使用できます。
