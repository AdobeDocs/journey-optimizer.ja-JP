...
1. **[!UICONTROL プロバイダーペイロード]**&#x200B;を追加して、リクエストペイロードを検証およびカスタマイズします。
   ...
   >[!NOTE]
   >
   >カスタム SMS プロバイダーを基本認証またはベアラー認証で設定する場合、JSON ペイロードに `authOption` パラメーターを含める必要があります。 さらに、**プロバイダーペイロード** は、テンプレート変数 `{{fromNumber}}`、`{{toNumber}}`、`{{message}}` を参照する必要があります。
...
1. 以下で説明するように、Webhook 設定を指定します。
   ...
   * **[!UICONTROL API 資格情報]**：[以前に設定した API 資格情報](#api-credential)をドロップダウンから選択します。
...
1. 「![](assets/do-not-localize/Smock_Add_18_N.svg)」をクリックしてキーワードカテゴリを追加し、次のように設定します。
   ...
   * **[!UICONTROL インバウンドキーワードカテゴリ]**：キーワードカテゴリを **[!UICONTROL オプトイン]**、**[!UICONTROL オプトアウト]**、**[!UICONTROL ダブルオプトイン]**、**[!UICONTROL ヘルプ]** または **[!UICONTROL カスタム]** から選択します。
   ...
   * **[!UICONTROL キーワードを入力]**：メッセージを自動的にトリガーするデフォルトまたはカスタムのキーワードを入力します。「![](assets/do-not-localize/Smock_Add_18_N.svg)」をクリックして複数のキーワードを追加します。
     ...
     **[!UICONTROL カスタムキーワード]** については、ジャーニー内のバッチベースのアクションに同意以外の関連キーワードを使用します。
