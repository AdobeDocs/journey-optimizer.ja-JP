...
ファイルのインポート先 [!DNL Assets Essentials]を参照する場合、まず保存先のフォルダーを参照または作成する必要があります。 その後、メールコンテンツに挿入できます。
...
メッセージを作成し、パーソナライズしたら、パブリッシュして実行できるようにします。 [詳細情報](../messages/publish-manage-message.md)
