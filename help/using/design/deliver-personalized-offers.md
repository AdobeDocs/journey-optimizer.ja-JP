...
[!DNL Journey Optimizer] のメールメッセージでは、オファー決定エンジンを利用して、顧客に提供する最適なオファーを選択するための決定を挿入できます。
...
>[!NOTE]
>
>公開済みメッセージで直接または間接的に参照されるオファー、フォールバックオファー、オファーコレクション、オファー決定を更新すると、その更新は、対応するメッセージに自動的に反映されるようになり、再公開する必要がなくなりました。
