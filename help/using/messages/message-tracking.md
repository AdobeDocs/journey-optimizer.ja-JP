...
1. 作成するリンクのタイプを選択します。
   ...
   * **[!UICONTROL ワンクリックのオプトアウト]**:リンクを挿入すると、ユーザーはオプトアウトを確認する必要なく、通信を素早く登録解除できます。 詳しくは、[この節](consent.md#one-click-opt-out)を参照してください。

   * **[!UICONTROL 外部のオプトイン/購読]**:ブランドからの受信コミュニケーションを受け入れるためのリンクを挿入します。

   * **[!UICONTROL 外部のオプトアウト/購読解除]**:ブランドからの通信の受信を停止するリンクを挿入します。 オプトアウト管理の詳細については、[この節](consent.md#opt-out-management)を参照してください。
