...
* 変数は、パラメーターの値が変化することを意味します。ジャーニーでこのカスタムアクションを使用するマーケターは、望む値を自由に渡したり、このパラメーターの値を取得する場所 ( イベントから、Adobe Experience Platformからなど ) を指定したりできます。 この場合、トグル定数/変数の右側のフィールドは、ジャーニーでこのパラメーターに名前を付ける際にマーケターに表示されるラベルです。
