...
すべてのカスタムアクションに対して、5,000 個の呼び出し/秒の上限が定義されています。 この制限は、カスタムアクションの対象となる外部エンドポイントを保護するために、顧客の使用に基づいて設定されました。 適切な読み取り率（カスタムアクションを使用する場合は 5,000 個のプロファイル）を定義して、オーディエンスベースのジャーニーでこの点を考慮する必要があります。 必要に応じて、キャッピング/スロットリング API で上限またはスロットル制限を大きく定義することで、この設定を上書きできます。 [このページ](../configuration/external-systems.md)を参照してください。

次の様々な理由により、カスタムアクションを使用してパブリックエンドポイントをターゲット設定しないでください。

* 適切な制限やスロットルがないと、パブリックエンドポイントに対して多くの呼び出しが送信され、そのボリュームをサポートしていない可能性があります。
* プロファイルデータは、カスタムアクションを通じて送信される可能性があるので、パブリックエンドポイントをターゲットに設定すると、誤って個人情報を外部で共有する可能性があります。
* パブリックエンドポイントから返されるデータを制御できません。 エンドポイントの API を変更した場合や、誤った情報の送信を開始した場合は、送信された通信でそれらの情報が使用可能になり、悪影響が出る可能性があります。
...
カスタムアクションを設定する場合、次の項目を定義する必要があります **[!UICONTROL エンドポイントの設定]** パラメータ：
...
1. 呼び出しを選択 **[!UICONTROL メソッド]**: **[!UICONTROL POST]**, **[!UICONTROL GET]** または **[!UICONTROL PUT]**.
...
1. Adobe Analytics の **[!UICONTROL リクエスト]** セクションで、外部サービスに送信する JSON ペイロードの例を貼り付けます。 このフィールドはオプションで、POSTおよびPUT呼び出しメソッドでのみ使用できます。

1. Adobe Analytics の **[!UICONTROL 応答]** セクションで、呼び出しで返されたペイロードの例を貼り付けます。 このフィールドはオプションで、すべての呼び出しメソッドで使用できます。 お客様のアクションで API 呼び出し応答を活用する方法について詳しくは、 [このページ](../action/action-response.md).
