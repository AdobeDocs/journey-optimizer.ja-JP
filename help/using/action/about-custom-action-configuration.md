...
>[!NOTE]
>
>1 分あたりの上限 300,000 回の呼び出しは、応答時間が 0.75 秒未満のエンドポイントのサンドボックスごと **エンドポイントごとに** スライディングウィンドウ」として適用されます。 スライディングウィンドウは任意のミリ秒で開始できます。つまり、クロック分に合わせてレートが 300 k/分を下回っても、キャッピングエラーが発生する可能性があります。 応答時間が 0.75 秒を超えるエンドポイントの場合、30 秒あたり 150,000 回の呼び出しという別の制限（スライディングウィンドウも含む）が適用されます。 処理に時間のかかるエンドポイントについて詳しくは、[&#x200B; このページ &#x200B;](../configuration/external-systems.md#response-time) を参照してください。

1 分あたりのデフォルトの 300,000 回の呼び出しの制限は、ドメインレベル（example.comなど）で適用されます。 より高い制限が必要な場合は、使用状況の証拠を添えてAdobe サポートに連絡し、エンドポイントのスループットを確認します。 キャッピングの増加をリクエストするには、予想される通話量とエンドポイントの処理能力の詳細を入力します。 処理能力テストでエンドポイントがより高いスループットを処理できることが示された場合、Adobeはキャッピングをカスタマイズできます。 ベストプラクティスとしては、ジャーニーの再構築や待機アクティビティの実装を検討し、アウトバウンド呼び出しを分散させ、キャッピングエラーを回避します。

この制限は、カスタムアクションの対象となる外部エンドポイントを保護するために、顧客の使用状況に基づいて設定されています。必要に応じて、キャップ／スロットリング API でキャップまたはスロットルキャップを大きく定義することで、この設定を上書きできます。[このページ](../configuration/external-systems.md)を参照してください。
