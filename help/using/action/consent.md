...
**このページ：** Journey Optimizer で Adobe Experience Platform の同意ポリシーを適用します。これにより、各顧客の同意選択を尊重し、データ使用状況ルールに準拠した通信を実現できます。
...
1. Adobe Experience Platform で、関連付けられたマーケティングアクションと同意ポリシーを作成します。 [同意ポリシーの作成方法の詳細情報](https://experienceleague.adobe.com/docs/experience-platform/data-governance/policies/user-guide.html?lang=ja#consent-policy){target="_blank"}
...
1. Journey Optimizer で、メールチャネル設定を作成します。 [方法についてはこちらを参照](../configuration/channel-surfaces.md#create-channel-surface)
...
**追加のマーケティングアクション**&#x200B;を定義して、カスタムアクションのタイプを設定できます。 これにより、このジャーニーでのカスタムアクションの目的を定義できます。 必須のマーケティングアクション（通常はチャネルに固有）に加えて、この特定のジャーニーのカスタムアクションに固有の追加のマーケティングアクションを定義できます。 例：ワークアウト通信、ニュースレター、フィットネス通信など。必須のマーケティングアクションと追加のマーケティングアクションの両方が適用されます。
