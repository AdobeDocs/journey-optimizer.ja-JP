...
* 最後のプロファイルがジャーニーを終了すると、繰り返されないオーディエンスジャーニー&#x200B;**が自動的に停止**&#x200B;します。 [詳細情報](#auto-stop-non-recurring)
...
**非繰り返しオーディエンスジャーニー**&#x200B;は、最後のプロファイルがジャーニーを終了すると、自動的に&#x200B;**[!UICONTROL 停止]** ステータスに移行します。 これにより、プロファイルがアクティブに流れていなかったにもかかわらず、91日間のグローバルタイムアウトが期限切れになるまで、繰り返し繰り返されないオーディエンス読み取りジャーニーが&#x200B;**ライブ**&#x200B;状態のままだった以前の動作が削除されます。
...
|--------------|------------|---------------|--------------------------|
| オーディエンスを読み取り | × | 該当なし | 最後のプロファイルが終了したとき（自動停止） |
...
* 待機、反応、またはイベントがトリガーされたノードを長時間実行しない非定期的な「オーディエンスを読み取り」ジャーニーは、最後のプロファイルが終了したときに自動的に「停止」に遷移します。 これらのノードを持つジャーニーは、手動でクローズしない限り、91日間のグローバルタイムアウトの対象となります。
...
* **Q: オーディエンスジャーニーの「読み取り」が「完了」ステータスに達するのはいつですか？**  – 非繰り返しの「オーディエンスを読み取る」ジャーニーの場合：最後のプロファイルが終了すると（待機、反応、またはイベントノードがプロファイルをアクティブに保っている場合は91日後に）自動停止します。 クローズしたジャーニーが91日間のグローバルタイムアウトに達した場合、または完了定義テーブルの繰り返しジャーニーのルールごとに達した場合、完了に到達します。
