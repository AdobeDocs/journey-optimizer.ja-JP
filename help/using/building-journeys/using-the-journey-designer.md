...
次のジャーニーアクティビティでエラーやタイムアウトが発生した場合のフォールバックアクションを定義できます。 **[!UICONTROL 条件]** および **[!UICONTROL アクション]**.

アクティビティにフォールバックアクションを追加するには、 **[!UICONTROL タイムアウトまたはエラーの場合に代替パスを追加]** 」ボックスを使用して、次の操作をおこないます。アクティビティの後に別のパスが追加されます。 タイムアウト時間は、管理者ユーザーが [ジャーニーのプロパティ](../building-journeys/journey-gs.md#change-properties). 例えば、E メールの送信に時間がかかりすぎる場合やエラーが発生した場合は、プッシュ通知を送信することにします。
