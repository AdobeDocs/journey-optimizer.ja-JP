...
このセクションから、ジャーニーの名前を定義し、説明を追加して、ジャーニーのグローバルプロパティを設定します。
...
* ジャーニー指標を選択します。 [ ジャーニー指標を設定およびトラッキングする方法を学ぶ ](success-metrics.md)
* [ エントリおよび再エントリ ](#entrance) を管理します。 プロファイルエントリ管理は、ジャーニーのタイプに応じて異なります。詳しくは、[ このページ ](entry-management.md) を参照してください。
* 管理 [ データへのアクセス ](#manage-access)
* ジャーニーとプロファイル [ タイムゾーン ](#timezone) を選択します
* カスタム [ 開始日と終了日 ](#dates) を選択
* ジャーニーアクティビティで [ タイムアウト時間 ](#timeout) を定義します（管理者ユーザーのみ）
* [ 競合管理ツール ](#conflict) を使用して、競合を監視し、ジャーニーに優先順位を付ける
...
[タイムゾーン管理の詳細](../building-journeys/timezone-management.md)
...
>title="終了日"
>abstract="ジャーニーが終了する日付を設定します この日付に、アクティブなプロファイルがジャーニーを自動的に終了し、新しいエントリは許可されません。"

デフォルトでは、プロファイルは公開され次第ジャーニーにエントリし、[ グローバルジャーニータイムアウト ](#global_timeout) に達するまでジャーニーを継続できます。 唯一の例外は、「**繰り返し時に再エントリを強制する**」が有効になっている繰り返しの「オーディエンスを読み取り」ジャーニーで、これは次回の実行の開始日に終了します。

必要に応じて、カスタムの **開始日** と **終了日** を定義できます。 これにより、プロファイルは特定の日付にジャーニーにエントリし、終了日に達すると自動的に終了します。
...
>title="タイムアウトまたはエラー"
>abstract="ジャーニーがアクションの実行または条件の評価をタイムアウトと見なすまでに試行する時間を指定します。 推奨値は 1～30 秒です。"
...
Adobe Journey Optimizer は、ジャーニー全体を通じて使用される結合ポリシーを適用します。したがって、1 つのジャーニーで複数のオーディエンスが使用されている場合（例えば、[`inAudience` 関数で ](functions/functioninaudience.md) を使用している場合）、これによってジャーニーで使用される結合ポリシーとの不一致が生じ、エラーが発生して公開がブロックされます。 ただし、メッセージのパーソナライゼーションで一貫性のないオーディエンスが使用されると、一貫性がないにもかかわらず、アラートは発生しません。このため、このオーディエンスをメッセージのパーソナライゼーションに使用する場合は、オーディエンスに関連付けられた結合ポリシーを確認することを強くお勧めします。
...
「**[!UICONTROL スケジュール]**」セクションは、**[!UICONTROL オーディエンスを読み取り]** アクティビティがキャンバスにドロップされた場合にのみ使用できます。 ジャーニーを実行する特定の日時および頻度を定義できます。 [ オーディエンスを読み取りジャーニーをスケジュールする方法を学ぶ ](../building-journeys/read-audience.md)
