...
**プロファイルを更新** アクティビティには、更新を保存するための専用のデータセットが必要です。 このアクティビティは（データレイクではなく）プロファイルストアのみを更新するので、すべての更新を、**プロファイルを更新** アクション専用に指定されたプロファイル対応データセットに保存する必要があります。 バッチまたはストリーミング取得に使用するデータセットを使用すると、新しくオンボードされたデータによって、「**プロファイルを更新** アクションで行われた変更が上書きされます。

また、「**プロファイルを更新**」アクティビティ設定には ID 名前空間は必要ありません。そのため、選択したデータセットで、ジャーニーを開始したアクションで使用したものと同じ **ID 名前空間** が、これらの更新で使用される名前空間として使用されていることを確認してください。 また、選択したデータセットで ID マップを使用することもできます。正しい名前空間のデータセットか ID マップを使用するデータセットを選択しないと、プロファイルを更新アクティビティが失敗します。
