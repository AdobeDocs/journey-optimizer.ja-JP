...
## ジャーニーがいつ一時停止または再開されたかを表示 {#view-pause-resume-info}

ジャーニーの最終一時停止または再開日時を確認するには、ジャーニーを開いて **プロパティ** に移動します（ジャーニー名の横にある鉛筆アイコンをクリック）。 「**技術的な詳細をコピー**」ボタンを使用して、次のテクニカル情報をコピーします。

* 最後の一時停止と再開の日時
* 最後の一時停止と最後の再開を実行したユーザーの表示名と識別子
* ジャーニーの一時停止の設定（一時停止動作、最大一時停止時間、自動再開状態、一時停止 ID）

この情報は、トラブルシューティング、監査、またはサポートとの共有に役立ちます。 コピーされたフィールドの完全なリストについては、[&#x200B; ジャーニーのプロパティへのアクセス &#x200B;](journey-properties.md#access-properties) を参照してください。
