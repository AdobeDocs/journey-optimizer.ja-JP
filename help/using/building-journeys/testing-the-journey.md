...
1. 「**[!UICONTROL イベントをトリガー]**」ボタンを使用してイベントを設定し、ジャーニーに送信します。
   ...
   ![ テスト モードでイベント ボタンをトリガーする ](assets/journeyuctest1.png)
...
1. 必要に応じて、様々なフィールドを設定します。**プロファイル ID** フィールドに、テストプロファイルの識別に使用するフィールドの値を入力します。これには、メールアドレスなどを使用できます。テストプロファイルに関連するイベントを送信します。[この節](#firing_events)を参照してください。
   ...
   ![ プロファイル識別子を入力できるイベント設定フィールド ](assets/journeyuctest1-bis.png)

1. イベントを受け取ったら、「**[!UICONTROL ログを表示]**」ボタンをクリックし、テスト結果を表示して確認します。[この節](#viewing_logs)を参照してください。

   ![ テスト結果を表示するための「ログを表示」ボタン ](assets/journeyuctest2.png)
...
![ イベント選択用のフィールドとドロップダウンを備えたイベント設定インターフェイス ](assets/journeytest4.png)
...
![ 詳細設定のための JSON 形式のイベントペイロードのコードビュー ](assets/journeytest5.png)
...
![ プロファイルの進行状況とエラーを示すジャーニーテストの視覚的なフロー ](assets/journeytest6.png)
...
![ ジャーニーの実行結果を JSON 形式で表示するテストログ ](assets/journeytest3.png)
