...
* 反応イベントを含むジャーニーには、ドライランモードは使用できません
...
* Adobe Experience Platform クエリサービスを使用してジャーニーレポート指標を分析する際は、ドライランで生成されたステップイベントを除外する必要があります。これを実行するには、`inDryRun` フラグを `false` に設定します。