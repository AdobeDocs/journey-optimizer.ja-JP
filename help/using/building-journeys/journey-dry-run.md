...
1. **オーディエンスインサイト**:ジャーニー実務担当者は、ジャーニー条件に基づくオプトアウトや除外を含む、様々なジャーニーノードでオーディエンスの到達可能性を予測できます。
1. **リアルタイムフィードバック**：指標は、ライブレポートと同様に、ジャーニーキャンバスに直接表示され、ジャーニー実務担当者はジャーニーのデザインを調整できます。

## ドライランの実行ロジック {#journey-dry-run-exec}

ドライラン中、ジャーニーはシミュレーションモードで実行され、実際のアクションをトリガーせずに、各ジャーニーアクティビティに次の特定の動作を適用します。

* メール、SMS、プッシュ通知を含む&#x200B;**チャネルアクション**&#x200B;ノードは実行されません。
* **カスタムアクション**&#x200B;は、ドライラン中は無効になり、その応答は null に設定されます。

  読みやすくするために、ドライランの実行中、カスタムアクションとチャネルアクティビティはグレー表示されます。

  ![ ドライランのジャーニーでグレー表示されているアクションアクティビティ ](assets/dry-run-greyed-activities.png){width="80%" align="left"}

* 外部データソースを含む **データソース** と **待機** アクティビティは、ドライラン中にデフォルトで無効になります。 ただし、この動作 [ ドライランモードを有効にする場合 ](#journey-dry-run-start) を変更できます。

* **反応** ノードが実行されません：このノードに入力したすべてのプロファイルは成功して終了します。 ただし、次の優先度のルールが適用されます。
   * **反応** ノードを 1 つまたは複数の **単一イベント** ノードと並行して使用する場合、プロファイルは常に反応イベントを通過します。
   * **反応** ノードを 1 つまたは複数の **反応イベント** ノードと並行して使用する場合、プロファイルは常にキャンバスの最初のノード（上部のノード）を通過します。
...
1. **待機** アクティビティと **外部データソース** 呼び出しを有効または無効にする場合に選択し、ドライランの公開を確認します。

   ![ ジャーニーのドライランの公開を確認する ](assets/dry-run-publish.png){width="50%" align="left"}
...
ドライランのジャーニーは手動で停止することもできます。 ドライラン モードを非アクティブにするには、次の手順に従います。

1. 停止するドライラン ジャーニーを開きます。
1. 「**閉じる**」ボタンを選択してテストを終了します。
過去 24 時間およびすべての時間レポートへのリンクが、確認画面で利用できます。

   ![ ジャーニーのドライランの実行を停止 ](assets/dry-run-stop.png){width="50%" align="left"}

1. 「**ドラフトに戻る** をクリックして確認します。
...
* **ジャンプ** アクションは、ドライランでは有効になっていません。
ソースジャーニーが宛先ジャーニーに **ジャンプ** イベントをトリガー付けする場合、そのジャンプイベントは、ドライランのジャーニーバージョンには適用されません。 例えば、ジャーニーの最新バージョンがドライランで、以前のバージョンが **ライブ** の場合、ジャンプイベントはドライランのバージョンを無視し、**ライブ** バージョンにのみ適用されます。

## ジャーニーステップイベントとドライラン {#journey-step-events}

ジャーニードライランが生成します **stepEvents**。 これらの stepEvents には、特定のフラグとドライラン ID （`inDryRun` および `dryRunID`）があります。

![ジャーニードライランのスキーマ属性 ](assets/dry-run-attributes.png)
...
stepEvent データを **外部システム** に書き出す場合は、`inDryRun` フラグを使用してドライランの実行をフィルタリングできます。
