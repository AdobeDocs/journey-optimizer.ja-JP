...
**クエリサービスを使用して** ジャーニーレポート指標[!DNL Adobe Experience Platform]を分析する場合、ドライラン生成ステップイベントを除外する必要があります。 これを行うには、`inDryRun`が`true`のステップイベントを除外します（つまり、`inDryRun`が`null`または`false`のイベントのみを含めます）。
