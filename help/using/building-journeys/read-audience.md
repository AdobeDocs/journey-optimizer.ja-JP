...
>
>* **繰り返し以外**&#x200B;最後にアクティブなプロファイルが終了すると、オーディエンスジャーニーは自動的に&#x200B;**停止** ステータスに移行します。ただし、ジャーニーに待機期間を引き起こすノード（待機ノード、リアクションノード、イベントトリガーの移行）が含まれている場合は、標準の[91日のグローバルタイムアウト ](journey-properties.md#global_timeout)が適用されます。 [詳細情報](end-journey.md#auto-stop-non-recurring)
...
>[!NOTE]
>
>**オーディエンススナップショットの使用方法**
>
>オーディエンスの読み取りの各実行では、実行の実行時に使用可能なオーディエンスメンバーシップが使用されます。 バッチオーディエンスの場合、[!DNL Journey Optimizer]は使用可能な最新のバッチオーディエンスのスナップショットから読み取ります。 ジャーニーの開始時にリアルタイムでオーディエンスが再計算されることはありません。
>
>繰り返しジャーニーの場合、各オカレンスは、そのオカレンスで使用可能なスナップショットを使用します。 最新のバッチオーディエンスの評価を待ってからジャーニーを実行する場合は、バッチオーディエンスの評価&#x200B;]**後に**[!UICONTROL &#x200B;トリガーを有効にします。
...
>[!CAUTION]
>
>[ カスタムアップロードオーディエンス ](../audience/custom-upload.md) （CSV アップロード）およびその他の外部オーディエンス（Federated Audience Compositionなど）の場合、**[!UICONTROL 増分読み取り]**&#x200B;は現在、機能としてサポートされていません。 繰り返しごとに、**オーディエンス全体**&#x200B;が、増分読み取り切り替え設定に関係なく処理されます。
>
>繰り返しエントリを制御するには、[繰り返し](#schedule)に強制的に再エントリを使用します。
...
+++**増分読み取り[!UICONTROL と[!UICONTROL 繰り返し]の強制的な再エントリの仕組み**]

これら2つのオプションは、ジャーニー実行の異なる部分を制御します。

* **[!UICONTROL 増分読み取り]**&#x200B;は、次回の定期的な実行でオーディエンス **から選択されるプロファイルを**&#x200B;制御します。
* **[!UICONTROL 繰り返し時に再エントリを強制]**&#x200B;すると、**次回の繰り返し実行が開始されたときに、ジャーニーでアクティブなプロファイル**&#x200B;に何が起こるかを制御できます。

以下の表を使用して、次の実行時の組み合わせ動作を把握します。

| [!UICONTROL 増分読み取り] | [!UICONTROL 繰り返し時に再エントリを強制する] | 次の実行での動作 |
| ------------------------------ | ------------------------------------------- | ------------------------ |
| オフ | オフ | [!DNL Journey Optimizer]はその実行の完全なオーディエンスを読み取ります。 ジャーニーでアクティブなプロファイルは、自動的にリセットされません。 |
| 実施日 | オフ | [!DNL Journey Optimizer]は、前回の実行以降にオーディエンスに追加されたプロファイルのみを読み取ります。 ジャーニーでアクティブなプロファイルは、自動的にリセットされません。 |
| オフ | 実施日 | [!DNL Journey Optimizer]は、次の実行を開始する前に、現在のジャーニー実行からアクティブな参加者を削除し、その後、完全なオーディエンスを再度読み取ります。 これにより、プロファイルは新しいオカレンスで新しく開始できます。 |
| 実施日 | 実施日 | [!DNL Journey Optimizer]は、次の実行を開始する前に、現在のジャーニー実行からアクティブな参加者を削除し、最後の実行以降にオーディエンスに追加されたプロファイルのみを読み取ります。 強制的に再エントリすると、アクティブなジャーニーの参加がリセットされますが、増分読み取りは新しく追加されたオーディエンスメンバーに選択を制限します。 |

つまり、**[!UICONTROL 繰り返し]に強制的に再エントリしても、[!UICONTROL 増分読み取り]**&#x200B;は無効になりません。 両方のオプションが有効になっている場合、プロファイルは、次のオカレンスが開始される前にアクティブなジャーニーインスタンスから削除されますが、次のオカレンスでは、前回の実行以降に新規と見なされたオーディエンスメンバーのみが選択されます。

>[!IMPORTANT]
>
>**[!UICONTROL 繰り返し]**&#x200B;に強制的に再エントリすると削除されたプロファイルは、**[!UICONTROL 増分読み取り]**&#x200B;の新しいオーディエンスメンバーとして自動的に扱われません。 オーディエンスの選択は、前回の実行以降にプロファイルが新しくオーディエンスに追加されたかどうかによって異なります。
...
* **TL;DR:**&#x200B;このページでは、スケジュール、スループット、トラブルシューティング、ベストプラクティスに関するガイダンスを使用して、Adobe Journey Optimizerで「オーディエンスを読み取り」アクティビティを設定および使用して、Adobe Experience Platform オーディエンスからプロファイルを1回または定期的にジャーニーに追加する方法について説明します。

**インテント：**
* オーディエンスの読み取りアクティビティをジャーニーのエントリポイントとして設定します
* ジャーニーのAdobe Experience Platform オーディエンスとID名前空間を選択します
* 読み取り率を設定して、1秒間に入力するプロファイル数を制御します
* ジャーニーを1回、毎日、毎週、またはカスタムの繰り返しで実行するようにスケジュールできます
* 繰り返し実行で新しいオーディエンスメンバーのみを処理する増分読み取りを有効にする
* オーディエンスサイズの不一致、プロファイルなし実行、遅延エントリのトラブルシューティング
* バッチまたはリアルタイムのニーズにもとづいて、「オーディエンスを読み込む」と「オーディエンスの選定」を決定

**用語集：**
* **オーディエンスアクティビティの読み取り**：選択したAdobe Experience Platform オーディエンスからすべての適格プロファイルを読み取り、ジャーニー&#x200B;*（製品固有）*&#x200B;に追加するジャーニーエントリポイントアクティビティ
* **読み取り率**: 1秒あたりのジャーニーにエントリできるプロファイルの最大数（500 ～ 20,000、デフォルトは5,000） *（製品固有）*
* **増分読み取り**：前回のジャーニー実行以降にオーディエンスに新たに追加されたプロファイルのみを処理する定期的なジャーニーオプション *（製品固有）*
* **繰り返し時に再エントリを強制**: プロファイルが新しい&#x200B;*（製品固有）*&#x200B;に再入力できるように、新しい実行の前にすべてのアクティブなジャーニー参加者を削除するスケジューリングオプション
* **バッチオーディエンス評価後のトリガー**：新しいバッチオーディエンススナップショットが利用可能になるまでジャーニーの実行を遅らせるスケジューリングオプション（最大6時間） *（製品固有）*
* **補足識別子**：識別子が異なる場合に、同じプロファイルがジャーニーに複数回入力できるようにするセカンダリ識別子（注文IDなど） *（製品固有）*

**ガードレール：**
* ジャーニーごとに1つのオーディエンス読み取りアクティビティのみが許可され、最初のアクティビティである必要があります。
* オーディエンスの読み取りアクティビティごとに1つのオーディエンスのみを選択できます。
* 組織ごとに最大5つの同時オーディエンス読み取り実行。
* 最大読み取り率は、サンドボックスあたり1秒あたり20,000 プロファイルです（すべての同時読み取りオーディエンスアクティビティの合計）。
* 補足識別子を使用する場合、読み取り速度は1秒あたり500 プロファイルに制限されます。
* 「オーディエンス参加を実現」ステータスのプロファイルのみがジャーニーにエントリします。
* 個人ベースのID名前空間のみが使用できます。選択した名前空間のないプロファイルは入力できません。
* 12時間のジョブタイムアウトは、オーディエンスの読み取り書き出しジョブに適用されます。
* 失敗した書き出しジョブの再試行は、10分ごとに最大1時間行われます。
* 増分読み取りが有効になっているカスタムアップロードオーディエンスの場合、プロファイルは最初の繰り返しでのみ取得されます（これらのオーディエンスは修正されます）。
* 勝者の拡大・縮小は、オーディエンスの読み取りジャーニー（パス実験）では使用できません。

**用語：**
* 正規の名前：読み取りオーディエンス – 頭字語：なし – バリアント：セグメントトリガー、オーディエンスベースのジャーニーエントリ、セグメントの読み取り（従来のAPI名）
* 同義語：「オーディエンスを読み取る」 = 「セグメントトリガー」 = 「オーディエンストリガージャーニー」
* 「オーディエンスの読み取り」≠「オーディエンスの選定」（オーディエンスの読み取りはバッチ/スケジュール化、オーディエンスの選定はリアルタイムストリーミング化）は混同しないでください

**FAQ:**
* **Q: Audience QualificationではなくRead Audienceを使用するタイミングを教えてください。** — バッチのスケジュールされたユースケース（例：週次ニュースレター、リエンゲージメントキャンペーン）に「オーディエンスを読む」を使用します。 プロファイルがリアルタイムで選定された時点でジャーニーに即座にエントリする必要がある場合は、オーディエンスの選定を使用します。
* **Q: ジャーニーに参加するプロファイルがオーディエンスサイズよりも少ないのはなぜですか？**  – 一般的な原因には、選択した名前空間を持たないプロファイル、ジャーニーの実行前にまだ完了していないバッチセグメンテーションジョブ、またはプロファイルが実現ステータスになっていないことがあります。 「バッチオーディエンス評価後のトリガー」を有効にし、名前空間設定を確認します。
* **Q：増分読み取りは最初の実行時に何をしますか？**  – 最初の実行では、すべてのオーディエンスプロファイルが入力されます。 後続の実行では、最後の実行以降にオーディエンスに新たに追加されたプロファイルのみが処理されます。
* **Q：書き出しジョブが失敗した場合はどうなりますか？** — システムは10分ごとに最大1時間再試行します。 エラーはアラートで報告されます。 成功せずに1時間後、実行は失敗したと見なされます。
* **Q：同じプロファイルでオーディエンスの読み取りジャーニーを複数回入力できますか？**  – はい、補足識別子が設定され、エントリ間で異なる場合、または「繰り返し時に再エントリを強制」が有効になっている場合。 これらがなければ、プロファイルを同時に複数回表示することはできません。
* **Q: 1回限りのオーディエンス読み取りジャーニーの有効期間はどれくらいですか？** — ジャーニーに待機、反応、またはイベントトリガーのトランジションが含まれていない限り、最後のプロファイルが終了すると、自動的に停止します。この場合、91日間のグローバルタイムアウトが適用されます。 デフォルトでは、91日で完了するまでライブのままになりません。
