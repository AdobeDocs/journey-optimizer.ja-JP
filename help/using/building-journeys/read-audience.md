...
1. 「**[!UICONTROL スロットルレート]**」を設定します。これは、1 秒あたりのジャーニーにエントリできるプロファイルの最大数です。 この率は、このアクティビティにのみ適用され、ジャーニー内の他のアクティビティには適用されません。 例えば、カスタムのアクションに対してスロットルレートを定義する場合は、Throttling API を使用する必要があります。この[ページ](../configuration/throttling.md)を参照してください。
