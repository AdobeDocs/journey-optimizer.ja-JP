...
* 組織ごとの同時読み取りオーディエンス制限：各組織は、最大 5 つの読み取りオーディエンスインスタンスを同時に実行できます。 これには、すべてのサンドボックスとジャーニーにわたって、スケジュールされた実行と、ビジネスイベントによってトリガーされる実行の両方が含まれます。 この制限は、すべての組織にわたって公平でバランスの取れたリソース割り当てを確実にするために適用されます。

* サンドボックススループット管理：システムは、すべてのオーディエンスを読み取りアクティビティで共有される 1 秒あたりの最大プロファイル数 20,000 個を使用して、サンドボックスあたりの処理スループットを動的に管理します。 個々のオーディエンスを読み取りアクティビティは、1 秒あたり 500 プロファイル以上の速度で設定できます。 サンドボックスレベルのスループット制限に達した場合、公正なリソース割り当てを確保するためにジョブをキューに入れることができます。

* ジョブ処理タイムアウト：ガードレールの制限により 12 時間以内に処理できないオーディエンスジョブは、自動的にクリーンアップされ、実行されなくなります。 これにより、ジョブの蓄積を防ぎ、システムの安定性を確保します。
