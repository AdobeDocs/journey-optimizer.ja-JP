...
* を使用するジャーニーの場合： **オーディエンスの閲覧** アクティビティの場合、同時に開始できるジャーニーの数は最大です。 再試行はシステムによって実行されますが、ジャーニーが 5 つ ( **オーディエンスの閲覧**、スケジュール済み、または「可能な限り早く」開始 )、まったく同時に開始されます。 ベストプラクティスは、時間の経過と共に、5 ～ 10 分の間隔で分散することです。

* エクスペリエンスイベントフィールドグループは、 **オーディエンスの閲覧** アクティビティ **[オーディエンスの選定](audience-qualification-events.md)** アクティビティ、またはビジネスイベントアクティビティ。
