...
>
>**実例**
>* **週刊ニュースレター** → オーディエンスを読む。 オーディエンスは日次バッチスナップショットで。 あなたは毎週月曜日の午前9時に旅行を予約します。 すべての適格なプロファイルが一緒に入力されます。
...
### アクティビティを追加し、オーディエンスを選択 {#add-activity-and-select-audience}
...
>title="読み取り率"
>abstract="1 秒あたりにジャーニーにエントリするプロファイルの最大数（500 ～ 20,000）。 デフォルトは 5,000 です。"
...
>
>* **繰り返し以外**&#x200B;最後にアクティブなプロファイルが終了すると、オーディエンスジャーニーは自動的に&#x200B;**停止** ステータスに移行します。ただし、ジャーニーに待機期間を引き起こすノード（待機ノード、リアクションノード、イベントトリガーのトランジション）が含まれている場合は、標準の91日間のグローバルタイムアウトが適用されます。 [詳細情報](end-journey.md#auto-stop-non-recurring)
...
オーディエンスベースのジャーニーは常に、**オーディエンスを読み取り**&#x200B;アクティビティで始まり、[!DNL Adobe Experience Platform] オーディエンスに属する個人を取得します。 これらのプロファイルは、1回または定期的に読み取られます。
