...
description: 汎用アクションアクティビティを追加して、ジャーニーキャンバス内で単一アクションとマルチアクションのインバウンドアクショングループを設定する方法と、組み込みのチャネルアクションを追加する方法について説明します。
...
**このページ：**&#x200B;では、「アクション」アクティビティを使用して、電子メール、プッシュ、SMS、アプリ内、web、コードベースのエクスペリエンス、コンテンツカードをカバーする単一の統合アクティビティを通じて、ジャーニーキャンバスからコンテンツを配信する方法を説明します。
...
>title="アクションアクティビティ"
>abstract="**アクション**&#x200B;アクティビティでは、1 つのネイティブチャネルアクションと複数のインバウンドアクティビティを設定でき、任意のビルトインチャネルアクションに最適化を追加することもできます。"

**アクション** アクティビティは、ジャーニーキャンバスから顧客にコンテンツを配信するための単一のエントリポイントです。 各チャネルの個別のアクティビティから選択する代わりに、単一の&#x200B;**[!UICONTROL アクション]** アクティビティをキャンバスにドラッグし、使用するチャネルを選択します。

メール、プッシュ通知、SMS、アプリ内メッセージ、web、コードベースのエクスペリエンス、コンテンツカードなど、従来のネイティブに組み込まれたあらゆるチャネルを、統合されたアクティビティタイプに統合することで、これまで使用していた個々のチャネルアクティビティを置き換えることができます。
...
従来のネイティブチャネルアクティビティ（電子メール、プッシュ、SMS、アプリ内、Web、コードベースのエクスペリエンス、コンテンツカード）は、2026年3月リリース **で非推奨（廃止予定）となりました。**

これらのアクティビティを使用する既存のジャーニーは、変更なしで引き続き機能します。移行は必要ありません。

従来のネイティブチャネルアクティビティも、次の場合に保持されます。

* **ジャーニーの複製** – 複製されたジャーニーは引き続きレガシーアクティビティを使用します。 そのまま編集して公開できます。移行は必要ありません。
* **新しいジャーニーのバージョンを作成** – 新しいバージョンでは、引き続きレガシーアクティビティが使用されます。 そのまま編集して公開できます。移行は必要ありません。
...
1. 選択したチャネルに従ってアクティビティを設定します。 設定ガイドラインについて詳しくは、以下のリンクを参照してください。
   ...
   * アウトバウンドアクションを作成する方法を次に説明します。
     ...
     <table style="table-layout:fixed">
      ...
      <div>
      <a href="../mobile/create-mobile-message.md"><strong> モバイルメッセージの作成（SMS/RCS/MMS） </strong></a>
      </div>
      <p>
      </td>
      </tr>
   ...
   * インバウンドアクションを作成する方法を次に説明します。
   ...
   >[!NOTE]
   >
   >* 各インバウンドエクスペリエンスアクションには、3日間の&#x200B;**待機** アクティビティが付属しています。 [詳細情報](wait-activity.md#auto-wait-node)
   >
...
迅速配信モードを使用する際のパフォーマンスについて詳しくは、[[!DNL Adobe Journey Optimizer] 製品の説明](https://helpx.adobe.com/jp/legal/product-descriptions/adobe-journey-optimizer.html){target="_blank"}を参照してください。
...
コンテンツに加えた変更は、アクションのプロパティを保存するまでジャーニーに反映されません。 [詳細情報](about-journey-activities.md#advanced-parameters)
