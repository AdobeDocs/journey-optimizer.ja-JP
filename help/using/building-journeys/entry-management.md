...
* **TL;DR:**&#x200B;このページでは、スループット制限、再エントリ設定、処理速度に対する待機およびアクションアクティビティの動作など、Adobe Journey Optimizerの4つのジャーニータイプでプロファイル エントリ管理がどのように機能するかを説明します。

**インテント：**

* 各ジャーニータイプ（単一イベント、ビジネスイベント、オーディエンスの読み取り、オーディエンスの選定）のエントリ動作とスループットの制限について説明します
* プロファイルの再エントリを有効または無効にし、再エントリの待機期間を設定します
* 1つのビジネスジャーニーに対して複数のビジネスイベントの実行を許可
* 待機アクティビティとアクションアクティビティがジャーニーの処理率にどのように影響するかを特定します
* プロファイルが同時に同じジャーニーに存在していないことを確認します

**用語集：**

* **再エントリ**: プロファイルが以前に終了した後に同じジャーニーを再度入力する機能。待機期間&#x200B;*（製品固有）*&#x200B;で設定可能
* **再入場待機期間**: プロファイルがジャーニーを再入力するまでの最小時間です。デフォルトは5分、ジャーニープロパティの最大値は90日です&#x200B;*（製品固有）*
* **TPS （Transactions Per Second）**: ジャーニーにプロファイルを入力または処理できるスループット *（製品固有）*
* **単一イベントジャーニー**:1つのプロファイル *（製品固有）*&#x200B;に関連付けられた1つのイベントによってトリガーされるジャーニー
* **オーディエンスジャーニーを読み取る**：定義されたオーディエンスに属するプロファイルのバッチを1回または定期的なスケジュール *（製品固有）*&#x200B;で処理するジャーニー
* **ビジネスイベントジャーニー**: オーディエンスをターゲットとするビジネスイベントによってトリガーされるジャーニー。プロファイル *（製品固有）*&#x200B;ごとに1つのジャーニーインスタンスを作成します
* **オーディエンス選定ジャーニー**: プロファイルがリアルタイム *（製品固有）*&#x200B;でストリーミングオーディエンスに出入りしたときにトリガーされるジャーニー

**ガードレール：**

* すべてのアクティブなバージョンで、プロファイルを同じジャーニーに同時に複数回存在させることはできません。
* オーディエンスジャーニーの読み取り：最大20,000 TPS （サンドボックスレベルの割り当て。同じサンドボックス内のすべての同時読み取りオーディエンスジャーニーで共有）
* オーディエンスの選定と単一イベントジャーニー：最大5,000 TPS （組織レベルの割り当て量、組織内のすべてのサンドボックス間で互いに共有）
* ビジネスイベントは5,000 TPS組織レベルの割り当てにカウントされます。後続の「オーディエンスを読み取り」アクティビティは、20,000 TPS サンドボックスレベルの割り当てを共有します
* デフォルトの再エントリ待機期間は5分です。ジャーニープロパティの設定可能な最大値は90日です
* 固定時間待機アクティビティは、プロファイルの急増が20,000 TPSを超える可能性があるため、お勧めしません。
* カスタムアクションのデフォルトのキャッピングは1分あたり300,000回です。
* ビジネスジャーニーの場合、最初の実行のオーディエンスデータは1時間再利用されます。

**用語：**

* 正式名称：プロファイル入口管理 – 頭字語：n/a – 変種：プロファイル入力管理、ジャーニーエントリ
* 同義語：&quot;reentrance&quot; = &quot;re-entry&quot;
* 「単一イベントジャーニー」≠「オーディエンスの選定ジャーニー」を混同しないでください。どちらも単一シナリオですが、トリガーが異なります（イベントの送信とオーディエンスメンバーシップの変更の違い）

**FAQ:**

* **Q: プロファイルは同じジャーニーを2回同時に開始できますか？**  – いいえ、システムはプロファイル IDをキーとして使用し、同じプロファイルが同じジャーニーの異なる場所に同時に存在するのを防ぎます。
* **Q: デフォルトの再エントリ待機期間は何ですか？** — ジャーニープロパティで最大90日間まで設定可能な5分。
* **Q: オーディエンスジャーニーを読み取り処理できる1秒あたりのプロファイル数は？** - サンドボックスレベルでは最大20,000 TPSですが、同じサンドボックスで複数のジャーニーを同時に実行する場合、この最大値は達成できない場合があります。
* **Q：一定時間の待機アクティビティ後のスループットはどうなりますか？**  – 複数のプロファイルが同時に待機を終了し、20,000 TPSを超える可能性があります。これを回避するには、相対時間の待機アクティビティを使用することをお勧めします。
* **Q: プロファイルは、ビジネス ジャーニーに複数回同時に表示できますか？**  – はい、ただし、異なるビジネスイベントのコンテキストでのみ可能です。
