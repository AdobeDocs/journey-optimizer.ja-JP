...
* **再入場待機期間**: プロファイルがジャーニーを再入力する前に通過する必要がある最小時間。デフォルトは5分、最大は91日&#x200B;*（製品固有）*&#x200B;です
...
* オーディエンスジャーニーを読む：サンドボックスレベルで最大20,000 TPS。
* オーディエンスの選定と単一イベントジャーニー：組織レベルで最大5,000 TPS共有。
* ビジネスイベントは5,000 TPSの割り当てにカウントされ、その後の「オーディエンスを読み取り」アクティビティは20,000 TPSの制限に従います。
* デフォルトの再エントリ待機期間は5分です。最大は91日（グローバル タイムアウト）です。
...
* **Q: デフォルトの再エントリ待機期間は何ですか？** — 5分、最大91日間まで設定可能。
