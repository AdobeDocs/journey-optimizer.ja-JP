...
* **TL;DR:**&#x200B;このページでは、URL設定、ヘッダー/クエリパラメーターマッピング、アクションパラメーターマッピング、データガバナンスおよび同意ポリシーの適用など、JSON ペイロードを使用してサードパーティ REST APIを呼び出すために、ジャーニーでカスタムアクションアクティビティを追加および設定する方法について説明します。

**インテント：**

* カスタムアクションアクティビティをジャーニーに追加して、REST APIを介してサードパーティシステムにデータを送信します
* 式エディターでフィールドと静的テキストを連結して、動的URL パスを設定します
* ジャーニーイベントまたはデータソースからの動的ヘッダー値とクエリパラメーター値のマッピング
* アクションパラメーター（変数として定義）をイベントフィールド、データソースフィールド、または静的値にマッピングします
* データガバナンスと同意ポリシーを適用して、カスタムアクションを通じて書き出すデータを制御できます

**用語集：**

* **カスタムアクション**: JSON形式のペイロードを使用して外部REST API エンドポイントを呼び出し、サードパーティシステム *（製品固有）*&#x200B;を統合するジャーニーアクションアクティビティ
* **動的パス**: ジャーニーコンテキスト *（製品固有）*&#x200B;のフィールドを使用して、実行ごとに定義されるカスタムアクション URLの変数部分
* **アクションパラメーター**：カスタムアクション設定で「変数」として定義されたメッセージペイロードフィールド。ジャーニーレベル *（製品固有）*&#x200B;のジャーニーデータにマッピングされます

**ガードレール：**

* URLの静的部分は、ジャーニーで変更できません。グローバルカスタムアクション設定で設定する必要があります。
* 動的ヘッダーフィールドとクエリパラメーターフィールドは、ジャーニーではなく、アクション設定画面で変数として定義されます。
* データガバナンスおよび同意ポリシーを適用することで、特定のフィールドがエクスポートされないようにしたり、同意のない顧客を除外したりできます。

**用語：**

* 正式名称：カスタムアクション – 頭字語：none – 変種：カスタムアクション、サードパーティアクション
* 同義語：「アクションパラメーター」 = 「変数として定義されたメッセージパラメーター」
* 混同しないでください。「静的URL部分」（グローバルアクション設定で設定され、ジャーニーでは編集できません）≠「動的パス」（ジャーニーごとに設定）

**FAQ:**

* **Q: ジャーニー内のカスタムアクションのベース URLを変更できますか？**  – いいえ、ジャーニーでは動的パス部分のみを設定できます。URLの静的部分は、グローバルカスタムアクション設定で設定されます。
* **Q: プロファイル IDを含む動的URL パスを作成するにはどうすればよいですか？**  – 高度な式エディターでパスフィールドを使用して、ID フィールドを静的文字列で連結します（例：`_id + '/messages'`）。
* **Q：同意ルールをカスタムアクションに適用するにはどうすればよいですか？** - カスタムアクションで同意ポリシーを設定して、関連するコミュニケーションの受信に同意しない顧客を除外します。詳しくは、同意ページを参照してください。
* **Q：動的ヘッダーの値をどこでマッピングしますか？** — アクティビティペインの「URL設定」セクションで、動的ヘッダーフィールド内をクリックするか、鉛筆アイコンを使用して、イベントまたはデータソースから目的のフィールドを選択します。
* **Q: アクションパラメーターに割り当てることができる値のタイプは何ですか？** — イベントフィールド、データソースフィールドにパラメーターをマッピングしたり、値を手動で渡したり、データ操作に高度な式エディターを使用したりできます。
