...
* **TL;DR:**&#x200B;このページは、ジャーニーの高度な式エディターで使用できる60以上の組み込み関数の分類されたリファレンスです。集計、コンバージョン、日付/時刻、リスト、計算、文字列、およびAdobe Experience Platform オーディエンス関数について説明します。

**インテント：**

* 分類された関数テーブルを参照して、タスクに適した関数を特定します
* 変換関数を使用して、文字列、整数、小数、ブール値、日付、期間の間でデータ型を変換します
* `inLastDays`、`inNextHours`、`nowWithDelta`などの関数を使用して、日付ベースのフィルタリングを実行します
* `contain`、`replace`、`split`、`trim`などの関数を使用して、文字列値を操作および検証します
* `count`、`avg`、`sum`、`distinctCount`などの集計関数を使用して、コレクションの統計計算を実行します
* `inAudience`関数を使用して、ジャーニー条件のオーディエンスメンバーシップを確認します

**用語集：**

* **集計関数**：値のコレクション *（製品固有）*&#x200B;から1つの値（count、sum、average、min、max）を計算する関数
* **変換関数**：あるデータタイプから別のデータタイプに値をキャストする関数（例：`toString`、`toDateTime`、`toDuration`） *（製品固有）*
* **日付関数**: ジャーニー式&#x200B;*（製品固有）*&#x200B;で日付、時刻、およびタイムゾーンの値を操作するための関数
* **リスト関数**：配列/収集データのフィルタリング、並べ替え、分析のための関数&#x200B;*（製品固有）*
* **inAudience**: プロファイルが指定されたAdobe Experience Platform オーディエンスセグメント *（product-specific）*&#x200B;に属しているかどうかを確認する関数

**ガードレール：**

* 関数は一貫した構文に従います：`functionName(param1, param2, ...)`
* 関数には、異なるユースケースを処理するための複数の署名（異なるパラメーターセット）が含まれている場合があります
* 各関数には固定の戻り値の型があります。戻り値の型が式のコンテキストで想定される型と一致することを確認します
* ジャーニー式エディターで使用できる関数は、パーソナライゼーションエディターの関数とは異なります

**用語：**

* 正規名：関数 – 頭字語：なし – バリアント：組み込み関数、式関数
* 同義語：「集計関数」 = 「統計関数」; 「変換関数」 = 「型鋳造関数」
* 混同しないでください：ジャーニー式の関数≠パーソナライゼーションエディターの関数（異なるセット）

**FAQ:**

* **Q: ジャーニー式エディターで使用できる関数の数は？** — aggregation、conversion、date、list、math、string、Adobe Experience Platformなど、60以上の関数をカテゴリ別に整理しました。
* **Q: プロファイルがジャーニー条件のオーディエンスに属しているかどうかを確認するにはどうすればよいですか？** — オーディエンス IDで`inAudience`関数を使用します。
* **Q：現在の日時を日数でオフセットするには、どの関数を使用すればよいですか？** — `nowWithDelta(N, "days")`を使用して、現在の時刻から日時オフセットを取得します。
* **Q：関数は、呼び出し方法に応じて異なる型を返すことができますか？**  – 関数は署名ごとに特定の戻り値タイプを持ちますが、1つの関数名に異なるパラメーターセットと戻り値タイプを持つ複数の署名を持つことができます。
* **Q: `count`と`countWithNull`の違いは何ですか？** — `count`はnull以外の要素のみをカウントし、`countWithNull`はnullを含むすべての要素をカウントします。
