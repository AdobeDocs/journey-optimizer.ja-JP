...
| 目標 | 関数 |
|------|----------|
| 文字列または Epoch を日付 **with** タイムゾーンに変換する | [toDateTime](#toDateTime) |
| 文字列または日付を日時 **タイムゾーンなし** に変換する | [toDateTimeOnly](#toDateTimeOnly) |
| 日付のみを抽出（年 – 月 – 日、時間なし） | [toDateOnly](#toDateOnly) |
| 整数に変換 | [toInteger](#toInteger) |
| 10 進数に変換 | [toDecimal](#toDecimal) |
| true/false に変換 | [toBool](#toBool) |
| 値を文字列に変換します | [toString](#toString) |
| 期間に変換（ISO-8601、例：PT10H） | [toDuration](#toDuration) |

>[!TIP]
>
>**toDateTime vs. toDateTimeOnly:** タイムゾーンが重要な場合（メッセージのスケジュール、地域間のイベントのタイムスタンプの比較など）に `toDateTime` を使用します。 ローカルの日時のみが関連し、タイムゾーンは無視できる場合（例えば、条件でカレンダー日付を比較する場合）は、`toDateTimeOnly` を使用します。

## よくある落とし穴 {#pitfalls}

* **Timezone は、文字列定数である必要があります** — `toDateTime` の timezone 引数は、フィールド参照または動的式にすることはできません。 `"UTC"` や `"Europe/Paris"` などのリテラル文字列を常に渡します。
* **文字列入力には ISO-8601 形式が必要** – 文字列を `toDateTime` または `toDateTimeOnly` に渡す場合、ISO-8601 形式（例：`"2023-08-18T23:17:59.123Z"`）に従っていることを確認します。 正しくない形式の文字列は、エラーなしで null を返します。
* **エポック値はミリ秒単位です** — ミリ秒 `toDateTime(1560762190189)` 想定します。 ソースが Unix のタイムスタンプを秒単位で提供する場合は、最初に 1000 を乗算します（例：`toDateTime(myField * 1000)`）。
* **toBool with unexpected strings** – 文字列値 `toBool` 正確に `true` の場合にのみ `"true"` を返します。 その他の文字列（`"1"`、`"yes"`、`"TRUE"` を含む）は `false` を返します。
