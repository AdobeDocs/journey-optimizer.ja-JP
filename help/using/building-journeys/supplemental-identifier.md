...
1. **属性をイベントスキーマの識別子としてマーク**
   ...
   1. 補足 ID に関連付ける名前空間を選択します。これは、ユーザー以外の識別子の名前空間にする必要があります。
      ...
      非人物 ID 名前空間をスキーマに適用した後、追加の識別子を使用するには、新しいイベントを作成する必要があります。 既存のエンティティを更新して新しい識別子を認識することはできません。
...
1. **属性を和集合／プロファイルスキーマの識別子としてマーク**
   ...
   1. 補足 ID に関連付ける名前空間を選択します。これは、ユーザー以外の識別子の名前空間にする必要があります。
      ...
      非人物 ID 名前空間をスキーマに適用した後、追加の識別子を使用するために、新しいフィールドグループを作成する必要があります。 既存のエンティティを更新して新しい識別子を認識することはできません。
