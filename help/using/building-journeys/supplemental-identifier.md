...
* **オーディエンスを読み取りジャーニー**
   ...
   * 補助 ID を使用するオーディエンスを読み取りジャーニーの場合、各ジャーニーインスタンスのオーディエンスを読み取りアクティビティの読み取り率は、1 秒あたり最大 500 プロファイルに制限されます。
   * 補助 ID を含むオーディエンスを読み取りジャーニーを使用する場合、統合プロファイルサービスオーディエンスのみがサポートされます。
...
1. **属性をイベントスキーマの識別子としてマーク**
   ...
   1. 識別子を **[!UICONTROL ID]** としてマークします。
      ...
      ![補足識別子フィールドグループを含むスキーマ設定](assets/supplemental-ID-schema.png)
...
1. **イベントに補足 ID を追加**
   ...
   1. イベント設定画面で、「**[!UICONTROL 補助識別子を使用]**」オプションをオンにします。
      ...
      ![補足識別子の名前空間選択を含むイベント設定](assets/supplemental-ID-event.png)

   1. 式エディターを使用して、補助 ID としてマークした属性を選択します。
...
1. **属性を和集合／プロファイルスキーマの識別子としてマーク**
   ...
   1. 識別子を **[!UICONTROL ID]** としてマークします。
      ...
      ![設定された補足識別子フィールドを含むプロファイルスキーマ](assets/supplemental-ID-schema-profile.png)
