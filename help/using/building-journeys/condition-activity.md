...
これにより、データソースのフィールドまたはジャーニー内で以前に配置されたイベントーに基づいて条件を定義できます。での式エディターの使用方法を説明します。 [この節](expression/expressionadvanced.md).

高度な式エディターを使用すると、コレクションを操作する、またはパラメーターを渡す必要があるデータソースを使用する、より高度な条件を設定できます。[詳細情報](../datasource/external-data-sources.md)。
...
* キャップが10000未満の場合、注入する個別プロファイルの数は、1,000 とキャップの数に一致する必要があります。
