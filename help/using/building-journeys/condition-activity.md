...
![&#x200B; 複数のパスオプションを持つジャーニーキャンバスの条件アクティビティ &#x200B;](assets/journey49.png)
...
![&#x200B; 条件アクティビティに「パス」ボタンを追加して、追加のパスを作成する &#x200B;](assets/journey47.png)
...
![&#x200B; フィールド選択と論理演算子 AND または OR を表示する式エディター &#x200B;](assets/journey64.png)
...
>[!IMPORTANT]
>
>**取り込まれていない属性または取り込まれていない属性の処理**
>
>プロファイルスキーマでスキーマフィールドが定義されていて、そのフィールドにデータが取り込まれていない場合、Journey Optimizerと基になるリアルタイム顧客プロファイルは、`null` のフィールドを解釈します。 その結果、`isEmpty()`、`isNull()` または類似の関数をチェックする条件は、属性が取り込まれなくても `true` と評価されます。 フィールドにデータがないことに気付いていない場合は、予期しないジャーニー動作につながる可能性があります。
>
>混乱を避けるために、条件式で使用する属性が、プロファイルがジャーニーにエントリする前に、実際のデータで取り込まれていることを確認してください。 [&#x200B; リアルタイム顧客プロファイル &#x200B;](https://experienceleague.adobe.com/docs/experience-platform/profile/home.html?lang=ja){target="_blank"} の属性値を確認して、条件で使用されるフィールドにデータが存在するかどうかを確認できます。

高度な式エディターを使用すると、コレクションを操作したり、パラメーターを渡す必要があるデータソースを使用したり、より高度な条件を設定できます。[詳細情報](../datasource/external-data-sources.md)。

![&#x200B; 式エディターを使用した Data Source条件の設定 &#x200B;](assets/journey50.png)
...
![&#x200B; 時間と曜日のフィルターを使用した時間条件設定 &#x200B;](assets/journey51.png)
...
![&#x200B; 複数のパスと配分を持つパーセンテージ分割設定 &#x200B;](assets/journey52.png)
...
![&#x200B; 最大プロファイル数制限設定を使用したプロファイルキャップ条件 &#x200B;](assets/profile-cap-condition.png)
...
1. ジャーニーを開いて&#x200B;**[!UICONTROL 条件]**&#x200B;アクティビティをドロップし、「**データソース条件**」を選択します。
   ...
   ![&#x200B; 条件アクティビティでの Data Source条件の選択 &#x200B;](assets/segment3.png)
...
1. 左側で、**[!UICONTROL オーディエンス]**&#x200B;ノードを展開します。条件に使用するオーディエンスをドラッグ＆ドロップします。デフォルトでは、オーディエンスの条件は true です。
   ...
   ![&#x200B; 式エディターのオーディエンスノードからのオーディエンス選択 &#x200B;](assets/segment4.png)
