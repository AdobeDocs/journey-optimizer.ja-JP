...
* **イベントが破棄されました – 認定条件が満たされていません** - ルールベースのイベントの場合、**認定条件** がイベントペイロードで満たされない（例えば、必須フィールドが空か見つからない、フィールドに `isNotEmpty` するなどの条件が失敗する）場合、イベントは **受信されますが破棄されます** ジャーニーはトリガーされません。 ログと Splunk トレースは、イベントが受信されたが、認定条件を満たさなかったことが理由で破棄されたことを、`notSuitableInitialEvent` などの破棄コードで示す場合があります。 これは想定されている動作です。認定条件が満たされない場合、イベントは破棄され、そのプロファイルに対してジャーニーはトリガーされません。 イベントペイロードに想定されるフィールドと値が含まれていること、およびイベント設定のルールが送信するデータと一致することを確認します。 イベントが別のジャーニーの **カスタムアクション** によってトリガーされる場合は、カスタムアクションのトラブルシューティングの [ 破棄イベントとアイドルタイムアウトの処理 ](../action/troubleshoot-custom-action.md#handling-discard-events-and-idle-timeouts) を参照してください。
...
1. 未解決のケースの場合は、[ サポートにお問い合わせください ](../start/user-interface.md#support-ticket-guidelines) ジャーニーログ、影響を受けるプロファイル ID、null トランジションの詳細を含めます
...
矛盾が解決しない場合は、[Adobe サポートにお問い合わせ ](../start/user-interface.md#support-ticket-guidelines)、「概要」タブと「参照」タブの両方のスクリーンショットを使用して調査を行ってください。

## クローズ済みジャーニーで空のプレースホルダーを示すトラッキングパラメーター {#tracking-parameters-closed-journeys}

送信済みメール内のトラッキング URL に `cid=em-acou-adob{}` などの空のプレースホルダーが含まれている場合、`context.system.source.actionId` などのコンテキストフィールドを解決できなかった可能性があります。 これは、通常、ジャーニーがクローズされ、関連する製品の変更後に再公開されていない場合に発生します。再公開されたジャーニーのみが、トラッキング URL のこれらのコンテキストフィールドに正しく入力されます。

これを解決するには、ジャーニーを再公開（[ 新しいバージョンを作成して公開する ](publish-journey.md#journey-create-new-version)）するか、チャネル設定またはメールコンテンツの [URL トラッキングパラメーター ](../email/url-tracking.md) から影響を受けるコンテキストフィールドへの参照を削除します。
