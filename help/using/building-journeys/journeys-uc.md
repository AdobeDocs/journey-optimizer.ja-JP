...
この使用例では、特定のオーディエンスに属するすべての顧客に最初の電子メールメッセージを送信します。

最初のメッセージへの反応に基づいて、特定のメッセージを送信するとします。

顧客が E メールを開いた場合は、購入を待ち、顧客に感謝のプッシュメッセージを送信します。
...
1. を追加します。 **反応** イベントと選択 **メール開封済み**. イベントは、オーディエンスに属する個人が E メールを開くとトリガーされます。

1. 次を確認します。 **イベントのタイムアウトを定義** ボックスで、期間（この例では 1 日）を定義し、 **タイムアウトパスを設定**. プッシュまたはメールでの最初のメッセージを開封しなかった人向けに、別のパスが作成されます。
...
1. 最初のパスに、前に作成した購入イベントを追加します。 個人が購入をおこなうと、このイベントがトリガーされます。
