...
この節では、セグメントの読み取り、イベント、反応イベント、電子メール/プッシュメッセージを組み合わせた使用例を示します。
...
1. 「CUSTOMER」メニューセクションで、「**[!UICONTROL Segments]**」を選択します。
...
セグメントが作成され、ジャーニーで使用できる状態になります。**セグメントを読み取り**&#x200B;アクティビティを使用して、セグメントに属するすべての個人をジャーニーにエントリさせることができます。
