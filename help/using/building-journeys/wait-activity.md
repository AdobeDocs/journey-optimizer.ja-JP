...
>id="ajo_journey_wait"
>title="「待機」アクティビティ"
>abstract="待機アクティビティを使用すると、パス内の次のアクティビティを実行するまで待機できます。 後に続くアクティビティを実行するタイミングを定義できます。 期間とカスタムの 2 つのオプションを使用できます。"
...
>title="自動待機ノードについて"
>abstract="**待機**&#x200B;ノードは、このインバウンドアクションの後に自動的に挿入されます。 デフォルトでは 3 日間に設定されているので、プロファイルがメッセージやエクスペリエンスを表示するのに十分な期間、ジャーニー内に留まります。 ユースケースに応じて、待機期間を更新するか、ノードを削除できます。"
