...
グローバルジャーニーのタイムアウトが 30 日である（つまり、プロファイルはジャーニーに入って最大 30 日後にジャーニーから常にドロップアウトする）ので、1 つのジャーニーで複数の待機アクティビティを使用する場合は注意が必要です。[このページ](../building-journeys/journey-gs.md#global_timeout)を参照してください。

ユーザーは、ジャーニーの 30 日間のジャーニータイムアウトまでの待機時間を完了するのに十分な時間がある場合にのみ、待機アクティビティを入力できます。 例えば、2 つの待機アクティビティをそれぞれ 20 日に設定した場合、2 番目の待機が 30 日のタイムアウトの後に終了することを検出します。 したがって、2 番目の待機は無視され、個人は開始する前にジャーニーを終了します。 この例では、顧客はジャーニーに合計 20 日間滞在します。

再入場をブロックするのを待たずにおくのがベストプラクティスです。 代わりに、 **再エントリを許可** オプションを使用します。 [このページ](../building-journeys/journey-gs.md#entrance)を参照してください。
