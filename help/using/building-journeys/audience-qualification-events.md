...
**このページでは、** プロファイルがAdobe Experience Platform オーディエンスに対して適格または離脱する際に、ジャーニーのエントリまたはプログレッションをトリガーするためにオーディエンス選定イベントを使用および設定する方法について説明します。
...
>title="結合ポリシー"
>abstract="結合ポリシーは、選択したオーディエンスから自動的に取得され、ジャーニー全体を通じて適用されます。"
>additional-url="https://experienceleague.adobe.com/ja/docs/journey-optimizer/using/orchestrate-journeys/create-journey/journey-properties#merge-policies" text="詳しくは、結合ポリシーを参照してください"
...
1. 「**[!UICONTROL オーディエンス]**」フィールドをクリックし、活用するオーディエンスを選択します。
   ...
   >[!TIP]
   >
   >オーディエンスの評価方法を使用する前に、オーディエンスの評価方法を特定するには、**[!UICONTROL オーディエンス]** メニューを開き、オーディエンスを選択して、**[!UICONTROL 評価方法]** フィールド **ストリーミング**、**バッチ**、または&#x200B;**Edge**&#x200B;を確認します。 **[!UICONTROL 評価方法]**&#x200B;列をこのアクティビティのオーディエンスリストに追加することもできます。 評価方法は、エントリタイミングと適用されるベストプラクティスに影響します。[ バッチオーディエンス ](#batch-speed-segment-qualification)および[ ストリーミングオーディエンス ](#streamed-speed-segment-qualification)を参照してください。
...
>[!NOTE]
>
>Edgeでストリーミングセグメントの対象となるプロファイルがプロファイルである場合、そのメンバーシップは、ジャーニーがアクションを実行する前にEdgeからHubに予測されます。 このEdgeからハブへの伝搬には、通常&#x200B;**15 ～ 30分かかります**。 プロファイルが期待通りにオーディエンス選定ジャーニーに入っていない場合は、さらに調査する前に、（必要に応じて待機アクティビティを追加して）この伝播ウィンドウを許可します。 真のリアルタイム入力を必要とするユースケースについては、代わりに単一イベントトリガーを検討してください。
