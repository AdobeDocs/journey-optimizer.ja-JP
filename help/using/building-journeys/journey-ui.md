...
ジャーニーのリストで様々なフィルターを使用すると、ジャーニーのリストを絞り込むことができます。

![](assets/filter-journeys.png)

**[!UICONTROL ステータスおよびバージョンフィルター [ から、ジャーニーを ](#journey-statuses) ステータス [、タイプ ](#journey-types)、[ バージョン ](#journey-versions) および割り当てられた [ タグ ](../start/search-filter-categorize.md#tags) に従ってフィルタリング]** きます。
...
## ジャーニータイプ {#journey-types}

ジャーニーのタイプは、そのジャーニーで使用されるアクティビティによって異なります。 これは、以下に対して実行できます。

* **[!UICONTROL 単一イベント]** – 単一イベントジャーニーは、特定のプロファイルにリンクされます。 イベントは、人物の行動や、人物にリンクして発生した事象（10,000 ロイヤルティポイントに到達など）に関連しています。 [詳細情報](../event/about-events.md)
* **[!UICONTROL ビジネスイベント]**。 ビジネスイベントジャーニーは、プロファイルに関連しないイベントで開始します。 イベントの設定は技術ユーザーが実行し、編集できません。 [詳細情報](../event/about-events.md)
* **[!UICONTROL オーディエンスの選定]** - オーディエンスの選定ジャーニーは、個人をジャーニーにエントリさせたり、ジャーニーを進行させたりするために、Adobe Experience Platform オーディエンスでプロファイルのエントリや離脱をリッスンします。 [詳細情報](audience-qualification-events.md)
...
## ジャーニーステータス {#journey-statuses}

ジャーニーのステータスはライフサイクルに応じて異なります。 これは、以下に対して実行できます。
...
### 新しいバージョンのジャーニーを作成します {#journey-create-new-version}
