...
[!DNL Journey Optimizer] では、ジャーニーリストに加えて、ジャーニーのカレンダービューが提供され、ジャーニーのスケジュールが明確に視覚的に表現されます。
...
カレンダーには、今週にスケジュールされているすべてのジャーニーが表示されます。 週間を移動するには、カレンダーの上にある矢印ボタンを使用します。

![ ライブジャーニーを表示するカレンダー表示 ](assets/timeline-journeys.png)

ジャーニーの表現方法：

* デフォルトでは、カレンダーグリッドには、選択した週のすべてのライブジャーニーとスケジュールされたジャーニーが表示されます。 追加のフィルターオプションで、完了、停止および完了したアクティベーションまたはアクティベーションを表示できます。
* ドラフトジャーニーとテストモードのジャーニーは表示されません。
* 複数日にわたるジャーニーは、カレンダーグリッドの上部に表示されます。
* 開始時間が指定されていない場合、最も近い手動アクティベーション時間を使用してカレンダーに配置されます。
* ジャーニーは 1 時間の期間として表示されますが、実際の送信時間や完了時間は反映されません。

ジャーニーの詳細については、ジャーニーのビジュアルブロックをクリックして開き、ジャーニーの詳細を確認します。
