...
>abstract="[!DNL Journey Optimizer]では、ジャーニーリストに加えて、ジャーニーのカレンダー表示が提供され、ジャーニーのスケジュールが明確に視覚的に表現されます。 これらのボタンは、リスト表示とカレンダービューをいつでも切り替えることができます。"
...
1. カレンダー表示にアクセスするには、ジャーニーリストを開き、![カレンダー表示に切り替えるカレンダーアイコン](assets/do-not-localize/timeline-icon.svg) アイコンをクリックします。
...
1. ![数日間のジャーニーの表示を切り替える設定アイコン](assets/do-not-localize/Smock_Gears_18_N.png) アイコンをクリックして、数日間または数週間にまたがる項目の表示を切り替えます。

   ![ライブキャンペーンを表示するカレンダー表示](assets/journey-calendar-1.png)

1. ![外部カレンダーを追加アイコン](assets/do-not-localize/Smock_CalendarAdd_18_N.svg) アイコンをクリックして、最大 3 つの外部カレンダーを管理および追加します。
...
![イベントまたはアクションを使用するジャーニーの数を示す「使用されている場所」フィールド](assets/journey3bis.png)

## ジャーニーのタイプ {#journey-types}

ジャーニーのタイプは、そのジャーニーで使用されるアクティビティによって異なります。 これには、以下のようなものがあります。

* **[!UICONTROL 単一イベント]** - 単一イベントジャーニーは、特定のプロファイルにリンクされます。 イベントは、ユーザーの行動またはユーザーとリンクして発生した事象（例えば、ユーザーが 10,000 ロイヤルティポイントに到達したなど）に関連しています。 [学習を増やす](../event/about-events.md)。
* **[!UICONTROL ビジネスイベント]**。 ビジネスイベントジャーニーは、プロファイルに関連しないイベントで開始します。 イベントの設定は技術系ユーザーが実行し、編集はできません。 [学習を増やす](../event/about-events.md)。
* **[!UICONTROL オーディエンスの選定]** - オーディエンスの選定ジャーニーは、個人がジャーニーに参加または進行できるように、[!DNL Adobe Experience Platform] オーディエンスのプロファイルの出入りをリッスンします。 [学習を増やす](audience-qualification-events.md)。
* **[!UICONTROL オーディエンスを読み取り]** - オーディエンスを読み取りジャーニーでは、オーディエンス内のすべての個人がジャーニーにエントリし、ジャーニーに含まれるメッセージを受け取ります。  [学習を増やす](read-audience.md)。
...
1. ジャーニーの名前を入力して確定します。 ジャーニーのプロパティ画面で名前を変更することもできます。 デフォルトでは、名前は `[JOURNEY-NAME]_copy` のように設定されます。
   ...
   ![複製したジャーニーのジャーニー名入力フィールド](assets/duplicate-jo2.png)
...
* **TL;DR:**&#x200B;このページでは、Journey Optimizer ダッシュボードからジャーニーを参照、フィルター、表示（リストとカレンダー）、複製、一括操作を実行する方法について説明します。

**インテント：**

* 「概要」タブと「参照」タブでジャーニーを参照および検索します
* ステータス、タイプ、バージョン、タグ、作成日、公開日でジャーニーをフィルタリング
* リスト表示とカレンダービューを切り替えて、ジャーニースケジュールを視覚化します
* CSV ファイルをアップロードして外部カレンダーを追加および管理する
* 既存のジャーニーを複製して、設定を再利用する
* 複数のライブジャーニーまたは一時停止したジャーニーを一度に一括一時停止または再開できます

**用語集：**

* **ジャーニーダッシュボード**：主要な指標を示す「概要」タブとすべてのジャーニーを示す「参照」タブを備えたメインジャーニーインターフェイス。 *（製品固有）*
* **破棄レート**：過去24時間にエントリを試行したプロファイルの合計数に対する、ジャーニーにエントリする資格がないプロファイルの割合（例：名前空間が間違っているか、再エントリのルールが原因）。 *（製品固有）*
* **ジャーニーカレンダービュー**: ジャーニーリストのカレンダーアイコンをクリックしてアクセスできる、ライブおよびスケジュールされたジャーニーの視覚的な週別カレンダー表現。 *（製品固有）*
* **一括一時停止**：複数のライブジャーニーを一度に一時停止する操作（1回の操作につき最大10）。 *（製品固有）*

**ガードレール：**

* ダッシュボード指標は、30分ごとに更新され、新しいデータが利用可能な場合にのみ更新されます。過去24時間のみ更新されます
* テストモードのドラフトジャーニーとジャーニーは、カレンダービューに表示されません
* 一括一時停止/再開は、1回の操作につき10 ジャーニーに制限されています
* 「再開」ボタンは、「一時停止」ジャーニーが選択されている場合にのみ有効です。「一時停止」ボタンは、「ライブジャーニー」が選択されている場合にのみ有効です
* カレンダーには、実際の送信時間または完了時間に関係なく、ジャーニーが1時間の時間間隔で表示されます

**用語：**

* 正規名：ジャーニーダッシュボード – 頭字語：なし – バリアント：ジャーニーリスト、ジャーニーの概要
* 同義語：「タブを参照」 = 「ジャーニーリスト」
* 混同しないでください：「Discard rate」 ≠ 「Error rate」 – 入力する資格のないプロファイルを破棄レート数えます。エラー率は、入力されたが処理エラーが発生したプロファイルをカウントします。

**FAQ:**

* **Q: ジャーニーの主要なパフォーマンス指標を一目でどこで確認できますか？** — ジャーニーダッシュボードの「概要」タブで、過去24時間の処理済みプロファイル、ライブジャーニー、エラー率、破棄レートを表示します。
* **Q：特定のイベントまたはアクションを使用するジャーニーを見つけるにはどうすればよいですか？** — ジャーニーリストのアクティビティフィルターとデータフィルターを使用して、特定のイベント、フィールドグループ、またはアクションを参照するジャーニーを表示します。
* **Q：複数のジャーニーを一度に一時停止できますか？**  – はい。リストで複数のライブジャーニーを選択し、下部のバーの「一時停止」ボタンをクリックします。 1回の操作につき最大10個のジャーニーを一時停止できます。
* **Q：外部イベントをジャーニーカレンダーに追加するにはどうすればよいですか？** — カレンダーの追加アイコンをクリックし、イベント名、開始日、終了日の列を含むCSV ファイルをドラッグ&amp;ドロップします。アップロードされたイベントは、組織内のすべてのユーザーに表示されます。
* **Q: カレンダーの実行時間が長くなっても、ジャーニーが1時間と表示されるのはなぜですか？** — カレンダーには、視覚的な一貫性のために、すべてのジャーニーが1時間のタイムスパンとして表示されます。これは、実際の送信時間や完了時間を反映するものではありません。
