...
1. **joai** セグメント ID が存在しない場合や **[!UICONTROL exited]** ステートの場合は、Edgeに（まだ）反映されていないことを意味します。 `segmentMembership` の値がハブからEdgeに反映されるまで、15～30 分待ちます。
...
1. **joai** セグメント ID が存在しない、または **[!UICONTROL 離脱]** 状態の場合は、対応するインバウンドジャーニーアクションへのエントリ時に、プロファイルが（まだ）特別な **joai** オーディエンスセグメントに正しく選定されていなかったことを意味します。
   ...
   >[!NOTE]
   >
   >`segmentMembership` 値が Hub 上のプロファイルに取り込まれるまでに、最大 15～30 分かかる場合があることに注意してください。
