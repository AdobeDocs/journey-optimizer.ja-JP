...
* 公開前に、合成テストプロファイルでテストモードを使用してジャーニーをテストします
...
* **テストモード**：合成テストプロファイルを使用して、ライブコミュニケーションをアクティブにせずにジャーニー実行をシミュレートする検証モード *（製品固有）*
...
* 混同しないでください：「テストモード」≠「ドライラン」 – テストモードでは合成プロファイルを使用します。ドライランでは、顧客に連絡せずに実際の実稼動データを使用します
...
* **Q: ジャーニーが公開される前にテストできますか？**  – はい。合成テストプロファイルでテストモードを使用してジャーニーをステップスルーするか、ドライランを使用して通信を送信せずに実際の実稼動データに対して実行します。
