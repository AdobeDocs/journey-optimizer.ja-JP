...
タイムゾーンは、ジャーニーレベルで定義されます。 固定タイムゾーンを入力するか、Adobe Experience Platformプロファイルを使用してジャーニータイムゾーンを定義できます。 Adobe Experience Platform プロファイルでタイムゾーンが定義されている場合は、ジャーニーでそのタイムゾーンを取得できます。
...
**終了日**&#x200B;も追加できます。これにより、日付に達したプロファイルは自動的に終了します。 終了日が指定されていない場合、プロファイルは [グローバルジャーニータイムアウト](#global_timeout) （通常は 30 日間で、ヘルスケアシールドとセキュリティおよびプライバシーシールドのアドオンが提供されるので、7 日間に短縮されます）。 唯一の例外は、「**繰り返し時に再エントリを強制する**」が有効になっている繰り返しの「オーディエンスを読み取り」ジャーニーで、これは次回の実行の開始日に終了します。
...
また、 [timeout](#timeout_and_error) ジャーニーアクティビティで使用される、グローバルジャーニータイムアウトもあり、インターフェイスに表示されず、変更できません。

このグローバルタイムアウトにより、ジャーニーの個人の進行が停止します **30 日** 彼らが入ってきた後 このタイムアウトは、 **7 日** ヘルスケア・シールドとプライバシーとセキュリティ・シールドのアドオン機能を備えています。 つまり、個人のジャーニーは 30 日（7 日）を超えることはできません。 このタイムアウト期間が過ぎると、個人のデータは削除されます。 タイムアウト期間が終了してもまだジャーニーに流入してくる個人がいる場合、それらは流入を停止され、レポートでは考慮されません。したがって、離脱する人物よりもジャーニーにエントリする人物の方が多くなる可能性があります。
