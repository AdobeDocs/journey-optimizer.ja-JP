...
>[!TIP]
>
>**どのジャーニータイプを選択すればよいですか？**
>
>Journey Optimizerを初めて使用する場合は、**[!UICONTROL オーディエンスを読み取り]** アクティビティを使用してオーディエンスベースのジャーニーを開始します。事前のイベント設定は必要なく、カンバスに慣れるには最も簡単な方法です。 リアルタイムのイベントトリガー型エクスペリエンス（購入やフォーム送信への反応など）の場合は、まずイベントを設定し、イベントベースのエントリを使用します。 さらなる詳細は、 [すべてのジャーニータイプとそのエントリルールを確認](entry-management.md#types-of-journeys)。
