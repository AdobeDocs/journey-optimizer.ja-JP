...
* **ライブジャーニー**：過去 24 時間のトラフィックを含むライブジャーニーの合計数。ライブジャーニーには、**単一ジャーニー**（イベントベース）と&#x200B;**バッチジャーニー**（セグメントの読み取り）が含まれます。
...
ジャーニーのリストの「**[!UICONTROL ステータスとバージョンフィルター]**」で、ステータス、タイプおよびバージョンに従ってジャーニーをフィルタリングできます。タイプは、**[!UICONTROL 単一イベント]**、**[!UICONTROL セグメントの選定]**、**[!UICONTROL セグメントを読み取り]**、**[!UICONTROL ビジネスイベント]**&#x200B;のいずれかになります。
...
アクションまたは条件アクティビティを編集する際には、エラーまたはタイムアウトが発生した場合の代替パスを指定するオプションがあります。サードパーティシステムに対するクエリを伴うアクティビティの処理が、タイムアウトとエラー処理に関するジャーニーのプロパティ（「**[!UICONTROL タイムアウトとエラー]**」フィールド）で指定されている期間を超えた場合、必要に応じて、2 番目のパスが選択されて代替アクションが実行されます。
...
ジャーニーアクティビティで使用される[タイムアウト](#timeout_and_error)のほかに、グローバルジャーニータイムアウトも存在します。こちらはインターフェイスに表示されず、変更もできません。ジャーニー内にいる人の進行は、エントリしてから 30 日経過すると、グローバルタイムアウトにより停止されます。つまり、各人のジャーニーは 30 日を超えることはできません。30 日間のタイムアウト期間が経過すると、個人のデータは削除されます。タイムアウト期間が終了してもまだジャーニーに流入してくる個人がいる場合、それらは流入を停止され、レポートでエラーとして記録されます。
