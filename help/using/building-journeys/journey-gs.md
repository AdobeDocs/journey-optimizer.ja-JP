...
* **破棄率**:過去 24 時間に入力したプロファイルの合計数に対する、破棄されたすべてのプロファイルの割合。 破棄されたプロファイルは、例えば、名前空間が正しくない場合や再入場ルールが原因で、ジャーニーにエントリする資格がないユーザーを表します。
...
アクションまたは条件アクティビティを編集する際に、エラーまたはタイムアウトが発生した場合に代替パスを指定することもできます。 サードパーティシステムに対するクエリを伴うアクティビティの処理が、タイムアウトとエラー処理に関してジャーニーのプロパティで指定されている期間 (**[!UICONTROL タイムアウトとエラー]** フィールド ) の場合、必要に応じて、フォールバックアクションを実行するために 2 番目のパスが選択されます。
...
ジャーニーアクティビティで使用される[タイムアウト](#timeout_and_error)のほかに、グローバルジャーニータイムアウトも存在します。こちらはインターフェイスに表示されず、変更もできません。ジャーニー内にいる人の進行は、エントリしてから 30 日経過すると、グローバルタイムアウトにより停止されます。つまり、各人のジャーニーは 30 日を超えることはできません。30 日のタイムアウト期間が経過すると、そのユーザーのデータは削除されます。 グローバルタイムアウト期間の終了時にまだジャーニーが進行中の人がいる場合、その進行は停止され、レポートでエラーとして記録されます。
