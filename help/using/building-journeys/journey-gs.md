...
ジャーニーを使用してメッセージを送信するには、次の設定が必要です。
...
>title="ジャーニーの構築"
>abstract="この画面には、既存のジャーニーのリストが表示されます。 ジャーニーを開くか「ジャーニーを作成」をクリックし、様々なイベント、オーケストレーション、アクションアクティビティを組み合わせて、複数手順のクロスチャネルシナリオを構築します。"
...
>title="ジャーニーのプロパティ"
>abstract="この節では、ジャーニーのプロパティについて説明します。 デフォルトでは、読み取り専用のパラメーターは非表示になっています。 使用可能な設定は、ジャーニーのステータス、権限および製品設定によって異なります。"

右上の鉛筆アイコンをクリックして、ジャーニーのプロパティにアクセスします。

ジャーニー名の変更、説明の追加、再入場の許可、開始日と終了日の選択をおこない、管理者ユーザーとして **[!UICONTROL タイムアウトとエラー]** 期間。 組織で有効にしている場合は、を有効にすることもできます [バーストメッセージ](#burst).
...
バーストモードは、大量のプッシュメッセージを高速に送信できるJourney Optimizerアドオンです。 これは、 **セグメントを読み取り** アクティビティとシンプルなプッシュメッセージ。 バーストは、メッセージ配信の遅延がビジネス上重要な場合、携帯電話に緊急のプッシュアラートを送信するときに使用します（ニュースチャンネルアプリをインストールしたユーザーにニュース速報を流すなど）。

バーストメッセージには、次の要件があります。

* ジャーニーは、 **セグメントを読み取り** アクティビティ。 イベントは許可されません。
* 次の手順は、プッシュメッセージにする必要があります。 他のチャネル、アクティビティ、ステップは許可されません（オプションを除く） **終了** アクティビティ )。
* プッシュメッセージではパーソナライゼーションは許可されていません。
* メッセージは小さい (&lt;2KB) 必要があります。

>[!CAUTION]
>
>要件のいずれかが満たされていない場合、バーストモードはジャーニーで使用できません。

有効化するには **バーストモード**&#x200B;ジャーニーを開き、右上の鉛筆アイコンをクリックして、ジャーニーのプロパティにアクセスします。 次に、「**バーストモードを有効にする**」トグルを有効化します。

![](assets/burst.png)

バーストジャーニーを変更し、電子メールメッセージ、その他のアクション、イベントなど、バーストメッセージに準拠していないアクティビティを追加すると、バーストモードは自動的に無効になります。

![](assets/burst2.png)

その後、通常どおりにジャーニーをテストして公開します。 テストモードでは、メッセージはバーストモードでは送信されません。

バーストメッセージの該当する使用例と、バーストメッセージのジャーニーを設定する方法については、このビデオを参照してください。

>[!VIDEO](https://video.tv.adobe.com/v/334523?quality=12)


## ジャーニーの終了、停止、または終了{#end-journey}

ジャーニーは、次の 2 つの特定のコンテキストで、1 人の個人に対して終了できます。

* その人物がアクティビティの最後のパスに到達した。この最後のアクティビティは、 **終了** アクティビティまたは別のアクティビティ。 の使用 **終了** アクティビティは必須ではありません。 [このページ](../building-journeys/end-activity.md)を参照してください。
* その人が **条件** アクティビティ ( **待機** アクティビティのデータバインディングを含む ) と一致し、いずれの条件とも一致しない。
...
（上記の理由のいずれかにより）ジャーニーをクローズした場合、ステータスは「**[!UICONTROL クローズ]**」になります。ジャーニーは、新しい個人がジャーニーにエントリするのをやめます。 既にジャーニーに参加しているユーザーは、通常どおりにジャーニーを完了できます。 デフォルトのグローバルタイムアウトである 30 日が経過すると、ジャーニーは&#x200B;**完了**&#x200B;ステータスに切り替わります。この[節](../building-journeys/journey-gs.md#global_timeout)を参照してください。
...
1. クリック **[!UICONTROL 新しい流入に対してクローズ]**&#x200B;をクリックし、ダイアログボックスで確定します。
...
停止すると、ジャーニーのステータスはに設定されます **[!UICONTROL 停止]**.
...
1. クリック **[!UICONTROL 停止]**&#x200B;をクリックし、ダイアログボックスで確定します。
