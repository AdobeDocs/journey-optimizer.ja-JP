...
* **TL;DR:**&#x200B;このページでは、リアルタイムのパーソナライゼーションと条件付きロジックのために、ジャーニーランタイムでAEP レコードデータセットデータを動的に取得するようにデータセット検索アクティビティを設定する方法について説明します。

**インテント：**

* 実行時に外部AEP レコードデータを取得するデータセットルックアップアクティビティをジャーニーに追加します
* 検索時に取得する特定のデータセットフィールド（リーフノード/プリミティブ値）を選択します
* 高度なモードでルックアップキーを定義して、ジャーニーコンテキストをデータセットレコードと結合します
* ジャーニー式エディターまたはパーソナライゼーションエディターでの強化されたデータセットのデータの使用
* ルックアップキーにシンプルモードを使用することで発生する「データセットのルックアップが見つかりません」エラーのトラブルシューティング

**用語集：**

* **データセット検索アクティビティ**：結合キー&#x200B;*（製品固有）*&#x200B;を使用して、実行時にAEP レコード データセットからデータを取得するジャーニーオーケストレーション アクティビティ
* **リーフノード**：プリミティブ値（文字列、数値、ブール値、日付）を保持するスキーマ階層の最下位レベルのフィールド *（製品固有）*
* **参照キー**：選択したデータセット *（製品固有）*&#x200B;のレコードに対してジャーニーコンテキストデータを照合するために使用される結合式（文字列または文字列のリスト）
* **強化されたデータ**: データセット検索アクティビティによって取得され、ジャーニーコンテキストに一時的に保存され、ダウンストリームアクティビティ *（製品固有）*&#x200B;で使用されます

**ガードレール：**

* ジャーニーごとに最大10個のデータセット検索アクティビティ。
* 検索アクティビティごとに選択された最大20個のフィールド。
* 参照キー配列内に最大 50 個のキー。
* 強化されたデータサイズは 10 KB に制限されています。
* データセットがアクティビティ設定に表示される前に、Adobe Experience Platformで検索できるようにデータセットを有効にする必要があります。
* 選択できるのはリーフノード（プリミティブ値）のみです。配列とマップは選択できません。
* 文字列または文字列のリストのみがルックアップキーとしてサポートされます。
* ルックアップキーは詳細モードで定義する必要があります。シンプルなモードを使用すると、アクティビティ出力はコンテキスト属性としてダウンストリームで使用できなくなります。
* 強化されたデータは一時的なもので、ジャーニーランタイムおよびアウトバウンドアクティビティのパーソナライゼーションでのみ使用できます。
* 最高のパフォーマンスを得るには、ジャーニーごとに5つのルックアップアクティビティに制限してください（推奨）。システムによって適用されるハード制限は、ジャーニーごとに10のアクティビティです。 ルックアップごとに最大20個の属性を使用することも推奨されます。

**用語：**

* 正式名称：データセット検索アクティビティ — Acronym: n/a — バリアント：AEP data lookup, data enrichment activity
* 同義語：「ルックアップキー」 = 「結合キー」
* 混乱しないでください：「データセット検索アクティビティ」 ≠ 「エクスペリエンスイベント検索」 – データセット検索は、時系列のエクスペリエンスイベントではなく、レコードデータセットのデータを取得します

**FAQ:**

* **Q：自分のデータセットがデータセットフィールドのドロップダウンに表示されないのはなぜですか？** — Adobe Experience Platformで検索するには、データセットを有効にする必要があります。 これを有効にするには、「必読」セクションの手順に従います。
* **Q: `@datasetLookup{}`が条件で「データセット参照が見つかりません」エラーを返すのはなぜですか？** — ルックアップキーは、アドバンスモードではなくシンプルモードを使用して定義されました。 詳細設定モードで定義し直して、ジャーニーを再公開します。
* **Q: データセットから配列またはマップフィールドを取得できますか？**  – いいえ、プリミティブなリーフノードフィールド（文字列、数値、ブール値、日付）のみを選択できます。
* **Q: メール内のエンリッチメントデータにアクセスするにはどうすればよいですか？**  – 構文`{{context.journey.datasetLookup.<activityId>.entities}}`を含むパーソナライゼーションエディターを使用します。
* **Q: エンリッチメントされたデータは、ジャーニーの終了後も保持されますか？**  – いいえ、強化されたデータは一時的なもので、ジャーニーランタイムセッション中にのみ使用できます。
