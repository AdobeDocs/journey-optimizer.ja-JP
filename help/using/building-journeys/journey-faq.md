...
description: ' [!DNL Adobe Journey Optimizer]のJourney Orchestrationに関するよくある質問'
...
**このページでは、** Adobe Journey OptimizerのJourney Orchestrationに関するよくある質問に対する回答を示します。一般的な概念、アクティビティ、イベント、トラブルシューティングについて説明します。
...
+++ [!DNL Adobe Journey Optimizer]のジャーニーとは何ですか？
...
[!DNL Adobe Journey Optimizer]は4種類のジャーニーをサポートしています：

* **単一ジャーニー**：イベント（例：購入、アプリのログイン）により個別にトリガーされます。 イベントが発生すると、プロファイルが 1 つずつジャーニーにエントリします。
* **オーディエンスジャーニーを読む**: [!DNL Adobe Experience Platform]のオーディエンスから開始し、そのオーディエンス内のすべてのプロファイルに一括でメッセージを送信します。
...
+++ ジャーニーでサポートされているオーディエンスのタイプとその制限は何ですか？

[!DNL Adobe Journey Optimizer]は、異なる特性とガードレールを持つ4種類のオーディエンスをサポートしています。
...
* **ジャーニーの使用状況**：オーディエンスを読み取りアクティビティと条件アクティビティでサポートされます。オーディエンスの選定ジャーニーでは&#x200B;**サポートされません**
...
* **ガードレール**：
   ...
   * プロファイルは[!DNL Adobe Experience Platform]に存在する必要があります

**4. 連合オーディエンス構成（FAC）オーディエンス**

* **説明**：連合データを使用して作成されたオーディエンス。データを[!DNL Adobe Experience Platform]にコピーすることなく、外部データウェアハウスからオーディエンスをクエリおよび構成できます
* **評価**：連合オーディエンス構成を実行すると更新される静的構成
* **ジャーニーの使用状況**：オーディエンスを読み取りアクティビティと条件アクティビティでサポートされます。オーディエンスの選定ジャーニーでは&#x200B;**サポートされません**（バックエンドの観点からのアップロードオーディエンスと同様です）
...
* 即時の応答が必要なリアルタイムのイベント駆動型ジャーニーには、**ストリーミングオーディエンス**&#x200B;を使用します
* 毎日の評価で十分なスケジュール済み通信には、**バッチオーディエンス**&#x200B;を使用します
* 外部リストを使用した 1 回限りのターゲットキャンペーンには、**アップロードオーディエンス**&#x200B;を使用します
* データの重複なしでエンタープライズデータウェアハウス機能を活用する必要がある場合は、**FAC オーディエンス**&#x200B;を使用します
* 大規模デプロイメントでのオーディエンスサイズと評価パフォーマンスの監視
* ジャーニーのタイミングとエントリ条件をデザインする際は、オーディエンスの更新率を考慮します

詳しくは、[オーディエンス](../audience/about-audiences.md)、[セグメントの作成](../audience/creating-a-segment-definition.md)、[カスタムアップロードオーディエンス](../audience/custom-upload.md)、[連合オーディエンス構成](../audience/federated-audience-composition.md)を参照してください。
...
* 個々の顧客の行動（カート放棄、新規会員のオンボーディングなど）にリアルタイムで対応する必要があります
...
* **オーディエンスベースのジャーニー**: [!DNL Adobe Experience Platform]でオーディエンスを作成する
...
* **データセット検索**：外部システムから[!DNL Adobe Experience Platform]にデータを読み込むことができる場合は、データセット検索機能を使用して、Experience Platform データセットに保存されている情報を取得します。
...
詳しくは、[条件](conditions.md)を参照してください。
...
詳しくは、[ジャーニーのメッセージ](journey-action.md)と[カスタムアクション](using-custom-actions.md)を参照してください。
...
はい。 ジャーニーイベントまたは条件に基づいて[!DNL Adobe Experience Platform]のプロファイル属性を変更するには、**プロファイルの更新** アクティビティを使用します。 これは、ロイヤルティポイントの更新、ジャーニーマイルストーンの記録、環境設定の変更、顧客エンゲージメントスコアのトラッキングに役立ちます。

詳しくは、[プロファイルの更新](update-profiles.md)を参照してください。

+++

+++ カートが放棄された直後にメールを送信するにはどうすればよいですか？

**単一イベントトリガージャーニー**&#x200B;を作成します。

1. 関連するカートの詳細を含む「カート放棄」イベントを設定します
2. イベントをジャーニーエントリポイントとして追加します
3. すぐにメールアクションに従います
4. パーソナライズされた商品詳細でカート回復メールをデザインする
5. ジャーニーの公開

買い物かごの放棄イベントを受け取ると、ジャーニーは自動的にトリガーし、回復用のメールをリアルタイムで送信します。

詳しくは、[イベント設定](../event/about-events.md)と[メールアクション](journey-action.md)を参照してください。

+++

+++ 他のユーザーがメッセージを開封しなかったりクリックしなかったりした場合に、メッセージを再送信できますか？

はい。 **タイムアウト**&#x200B;付きの&#x200B;**[!UICONTROL 反応イベント]**&#x200B;を使用します。

1. メッセージの送信後、チャネルアクションの&#x200B;**直**&#x200B;後に&#x200B;**[!UICONTROL 反応]**&#x200B;イベントを追加します（間に&#x200B;**[!UICONTROL 待機]**&#x200B;アクティビティを追加しません）。
2. メールの開封数またはクリック数をリッスンするには、**[!UICONTROL 反応]**&#x200B;イベントにタイムアウト期間（例：3 日）を設定します。
3. 2 つのパスを作成します。
...
タイムアウト付きの&#x200B;**[!UICONTROL 反応]**&#x200B;イベントを使用して、イベントトリガージャーニーを作成します。

1. **「買い物かごの放棄」イベントを設定**：項目を追加したが、期間内にチェックアウトが完了しない場合にトリガーされます
2. **最初のメッセージを送信**（オプション）：買い物かごの項目を確認するメール
3. **チャネルアクションの直後に[!UICONTROL 反応]イベントを追加**：購入イベントをリッスンするように設定します
4. **タイムアウト期間を設定**：顧客が自然に完了する時間を与えるために、**[!UICONTROL 反応]**&#x200B;イベントにタイムアウト（例：1～2 時間）を定義します
5. **2 つのパスを作成します**。
   * **購入イベントが発生した場合**：ジャーニーを終了するか、購入後のフローに進みます
   * **タイムアウトパス（購入なし）**：買い物かごの内容を含む放棄リマインダーメールを送信します
6. **オプション**：リマインダーメールの&#x200B;**直後**&#x200B;に、タイムアウト（24 時間）付きの別の&#x200B;**[!UICONTROL 反応]**&#x200B;イベントを追加し、インセンティブ（例：10％ディスカウント）付きの 2 回目のリマインダーを送信します

>[!IMPORTANT]
>
>**[!UICONTROL 反応]**&#x200B;イベントは[チャネルアクション](journey-action.md)の直後に配置する必要があります。 チャネルアクションと&#x200B;**[!UICONTROL 反応]**&#x200B;アクティビティの間に&#x200B;**[!UICONTROL 待機]**&#x200B;アクティビティを配置しないでください。
...
詳しくは、[条件](optimize.md#conditions)と[オーディエンスの選定](audience-qualification-events.md)を参照してください。
...
* **トリガーされたマーケティングメッセージ** （買い物かご放棄の回復）：すぐに送信
...
+++ ジャーニーフラグメントとは何か、いつ使用すればよいか？

**ジャーニーフラグメント**&#x200B;は、一度作成したジャーニーノードの再利用可能なセットであり、サンドボックス全体の任意のジャーニーに挿入できます。 ジャーニーキャンバスのオーケストレーションアクティビティとして使用できます。
...
詳しくは、[エントリ管理](entry-management.md)と[ストリーミングオーディエンスの選定のタイミングに関する考慮事項](audience-qualification-events.md#streaming-entry-caveats)を参照してください。
...
詳しくは、[条件](conditions.md)、[メッセージアクション](journey-action.md)、[チャネルの選択](../channels/gs-channels.md)を参照してください。
...
+++ ジャーニーで[!DNL Adobe Campaign]を使用するにはどうすればよいですか？

Journey Optimizerは、[!DNL Adobe Campaign]とネイティブに統合して、その高度な機能を活用します。

* **[!DNL Adobe Campaign]Standard**: Campaign Standard アクションを使用してトランザクションメッセージを送信します
* **[!DNL Adobe Campaign]v7/v8**: Campaign ワークフローをトリガーし、Campaignの配信インフラストラクチャを使用します
...
**結合ポリシー**&#x200B;は、[!DNL Adobe Experience Platform]が複数のソースからのデータを組み合わせて統合プロファイル ビューを作成する方法を決定します。 異なるデータセットをまたいでプロファイルフラグメントが存在する場合のデータの優先順位付けと ID ステッチのルールを定義します。
...
* 結合ポリシーは、ジャーニー条件、パーソナライゼーションおよびアクションでアクセスできる属性に影響を与えます
...
| | **条件アクティビティ** | **待機アクティビティ** |
|---|---|---|
| **目的** | ロジック（if／then）に基づいて異なるパスを作成します | 一定期間、ジャーニーを一時停止します |
| **関数** | データを評価し、それに応じてプロファイルをルーティングします | 続行する前に、プロファイルを特定のポイントで保持します |
| **ユースケース** | 顧客をセグメント化し、ステータスを確認し、行動に基づいて分岐します | メッセージ間のタイミング、営業時間の待機、遅延の発生 |
| **例** | 顧客が VIP の場合はプレミアムオファーを送信し、それ以外の場合は標準オファーを送信します | 次のメッセージを送信する前に、ウェルカムメールの送信後 3 日間待機します |
...
詳しくは、[条件](optimize.md#conditions)と[待機アクティビティ](wait-activity.md)を参照してください。
...
* 処理の複雑さを軽減するために、[条件ロジック](conditions.md)を最適化します
...
* **TL;DR:**&#x200B;このページでは、ジャーニーオーケストレーションの概念、ジャーニーの構築、テストと公開、実行モニタリング、高度な機能、Adobe Journey Optimizerのベストプラクティスについて包括的なFAQで説明します。

**インテント：**
* 4つのジャーニータイプ（単一、オーディエンスの読み取り、オーディエンスの選定、ビジネスイベント）と、それらを使用するタイミングについて説明します
* 特定のユースケースにおけるジャーニーとキャンペーンのどちらかを決定します
* プロファイルが同じジャーニーに入る頻度を制御するための再入力設定を構成します
* プロファイルが入力されない理由やメッセージが送信されない理由をトラブルシューティングします
* ジャーニーの上限設定ルールを適用して、複数のジャーニーをまたいでメッセージが疲労するのを防ぎます
* ジャーニーフラグメントを使用して、ジャーニーをまたいで共通のノードシーケンスを再利用できます

**用語集：**
* **単一ジャーニー**：購入やサインアップ *（製品固有）*&#x200B;などのリアルタイム イベントによって、ジャーニーが一度に1つのプロファイルをトリガーしました
* **オーディエンスジャーニー**&#x200B;を読み取る：バッチオーディエンスのすべてのプロファイルを一度にまたはスケジュールに従って処理するジャーニー&#x200B;*（製品固有）*
* **オーディエンス選定ジャーニー**: プロファイルがストリーミングオーディエンスセグメント *（製品固有）*&#x200B;に出入りしたときにトリガーされるジャーニー
* **ジャーニーの上限**: プロファイルが時間枠内にジャーニーに入れる回数、またはプロファイルが同時に入れるジャーニーの数を制限する設定&#x200B;*（product-specific）*
* **ジャーニーフラグメント**：一度作成された再利用可能な静的ジャーニーノードのセットで、設計時&#x200B;*（製品固有）*&#x200B;に複数のジャーニーに挿入されます
* **送信時間最適化（STO）**: エンゲージメントを最大化するために、個々のプロファイルごとに最適な送信時間を予測するAI主導の機能&#x200B;*（製品固有）*
* **補足識別子**: プロファイルが異なるエンティティ（別々の注文など）に対して同じジャーニーを複数回入力できる追加の識別子&#x200B;*（製品固有）*

**ガードレール：**
* 1 ジャーニーにつき最大50 アクティビティ
* ジャーニーの最大期間は91日です（グローバル タイムアウト）
* オーディエンスのアップロードと連合オーディエンス構成オーディエンスは、オーディエンス選定ジャーニーではサポートされていません
* リアクションイベントは、チャネルアクションの直後に、待機アクティビティを介さずに配置する必要があります
* ジャーニーフラグメント内でのジャンプ操作は許可されていません
* ジャーニーフラグメントは最大20個のノードをサポートし、サンドボックスは最大200個のアクティブフラグメントをサポートします
* ストリーミングオーディエンスの選定は、既にオーディエンスにあるプロファイルについて、ジャーニーの公開後から最大10分遅れる場合があります

**用語：**
* 正式名称：ジャーニー（Acronym）：なし（variants）：カスタマージャーニー、オーケストレーション、フロー
* 同義語：「新しい入り口に近い」 = 「優雅な停止」; 「停止」 = 「即時停止」
* 混同しないでください：「ジャーニー」≠「キャンペーン」 – ジャーニーは、マルチステップのイベントトリガーオーケストレーションをサポートします。キャンペーンは、1回限りまたはスケジュールされたオーディエンスベースの送信です
...
**FAQ:**
* **Q: ジャーニーのアクティビティの最大数を教えてください。** — 50のアクティビティ。ジャーニーをシンプルにすることで、メンテナンス性とパフォーマンスが向上します。
* **Q: プロファイルがジャーニーにエントリしなかったのはなぜですか？**  – 一般的な原因には、トリガーイベントが受信されない、オーディエンスの条件が満たされない、再エントリをブロックする再エントランス ルール、ジャーニーが非公開になる、名前空間の不一致などがあります。
* **Q: ライブジャーニーの構造を変更できますか？**  – いいえ。構造変更には、新しいジャーニーバージョンを作成する必要があります。 メッセージコンテンツは、新しいバージョンなしで更新できます。
* **Q: 「一時停止」、「新しいエントランスに近い」、「停止」の違いは何ですか？**  – 実行中のプロファイルを保持または破棄している間、ジャーニーを一時的に一時停止します。 新しいエントリに近い場合、新しいエントリは停止しますが、既存のプロファイルは終了します。 Stopはすべてのプロファイルを直ちに終了します。
* **Q: ジャンプ アクティビティの代わりにジャーニーフラグメントを使用するタイミングを教えてください。** - フラグメントを使用して、設計時に共通のノードロジックを再利用します（コピー&amp;ペースト動作）。 ジャンプを使用して、実行時にプロファイルを別のライブジャーニーにリダイレクトします。
* **Q：同じ顧客に対してメッセージを送信しすぎないようにするにはどうすればよいですか？** — ジャーニーの上限ルール（エントリの上限または同時実行の上限）を適用し、個々のチャネルアクションに対して頻度の上限ビジネスルールを使用します。
