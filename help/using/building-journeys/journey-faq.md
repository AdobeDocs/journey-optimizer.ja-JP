...
description: Adobe Journey Optimizer の Journey Orchestration に関するよくある質問
...
Adobe Journey Optimizer の Journey Orchestration に関するよくある質問への回答を見つけます。
...
Adobe Journey Optimizer では、それぞれ異なる特性とガードレールを持つ、次の 4 つのタイプのオーディエンスをサポートしています。
...
* **ガードレール**：
   ...
   * プロファイルは、Adobe Experience Platform に存在する必要があります

**4. 連合オーディエンス構成（FAC）オーディエンス**

* **説明**：連合データを使用して作成されたオーディエンス。Adobe Experience Platform にデータをコピーすることなく、外部データウェアハウスからオーディエンスのクエリを実行し、オーディエンスを作成できます
...
* **ストリーミングオーディエンスの選定のタイミング**：ストリーミングオーディエンスでオーディエンスの選定を使用するジャーニーでは、ジャーニーを公開する前にプロファイルがオーディエンスに既に含まれている場合や、ジャーニーのアクティブ化期間（公開後、最大 10 分）が完了していない場合、プロファイルがエントリされないことがあります。
