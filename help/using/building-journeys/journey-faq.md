...
description: のJourney Orchestrationに関するよくある質問  [!DNL Adobe Journey Optimizer]
...
[!DNL Adobe Journey Optimizer] のJourney Orchestrationに関するよくある質問への回答を示します。

さらに詳細が必要ですか？このページの下部にあるフィードバックオプションを使用して、質問を上げます。 [[!DNL Adobe Journey Optimizer]  コミュニティ &#x200B;](https://experienceleaguecommunities.adobe.com/t5/adobe-journey-optimizer/ct-p/journey-optimizer?profile.language=ja){target="_blank"} とつながることもできます。

## 一般概念

+++ [!DNL Adobe Journey Optimizer] でのジャーニーとは
...
[!DNL Adobe Journey Optimizer] では、次の 4 種類のジャーニーをサポートしています。

* **単一ジャーニー**：イベント（例：購入、アプリのログイン）により個別にトリガーされます。イベントが発生すると、プロファイルが 1 つずつジャーニーにエントリします。
* **オーディエンスジャーニーの読み取り**:[!DNL Adobe Experience Platform] からのオーディエンスで開始し、そのオーディエンス内のすべてのプロファイルにメッセージを一括で送信します。
...
**ベストプラクティス**：高度なオーケストレーションを伴う複雑でイベントトリガーのエンゲージメントには、[&#x200B; ジャーニー &#x200B;](journey.md) を使用します。 スケジュールされた、オーディエンスベースの通信には [&#x200B; アクションキャンペーン &#x200B;](../campaigns/create-campaign.md) を使用します。 [API トリガーキャンペーン &#x200B;](../campaigns/api-triggered-campaigns.md) を使用して、外部システムからプログラムでトリガーします。 キャンペーン固有の要件を持つ複数手順の通信には、[&#x200B; 調整されたキャンペーン &#x200B;](../orchestrated/gs-orchestrated-campaigns.md) を使用します。
...
[!DNL Adobe Journey Optimizer] では、それぞれ異なる特性とガードレールを持つ、次の 4 種類のオーディエンスをサポートしています。
...
* **説明**：連合データを使用して作成されたオーディエンス。データを [!DNL Adobe Experience Platform] にコピーすることなく、外部データウェアハウスからオーディエンスをクエリおよび作成できます
...
* **オーディエンスベースのジャーニー**:[!DNL Adobe Experience Platform] でオーディエンスを作成します
...
* **データセットルックアップ**：外部システムから [!DNL Adobe Experience Platform] にデータを読み込むことができる場合、データセットルックアップ機能を使用して、Experience Platform データセットに保存されている情報を取得します。
...
はい。**プロファイルを更新** アクティビティを使用すると、ジャーニーのイベントまたは条件に基づいて [!DNL Adobe Experience Platform] のプロファイル属性を変更できます。 これは、ロイヤルティポイントの更新、ジャーニーマイルストーンの記録、環境設定の変更、顧客エンゲージメントスコアのトラッキングに役立ちます。
...
>[!IMPORTANT]
>
>**[!UICONTROL 反応]**&#x200B;イベントは[チャネルアクション](journeys-message.md)の直後に配置する必要があります。チャネルアクションと&#x200B;**[!UICONTROL 反応]**&#x200B;アクティビティの間に&#x200B;**[!UICONTROL 待機]**&#x200B;アクティビティを配置しないでください。
...
* **ストリーミングオーディエンスの選定タイミング**：ストリーミングオーディエンスでオーディエンスの選定を使用するジャーニーの場合、ジャーニーが公開される前に既にオーディエンスに含まれていると、プロファイルが入力されない場合があります。 また、ジャーニーがアクティブ化期間を完了していない場合（公開後 10 分以内）も、遅延する可能性があります。
...
Journey Optimizerは、[!DNL Adobe Campaign] とネイティブに統合され、その高度な機能を活用しています。

* **[!DNL Adobe Campaign]Standard**:Campaign Standard アクションを使用してトランザクションメッセージを送信します
* **[!DNL Adobe Campaign]v7/v8**: Campaign ワークフローをトリガーし、Campaign の配信インフラストラクチャを使用する
...
**結合ポリシー** は、複数のソースのデータ [!DNL Adobe Experience Platform] 結合して統合されたプロファイルビューを作成する方法を決定します。 異なるデータセットをまたいでプロファイルフラグメントが存在する場合のデータの優先順位付けと ID ステッチのルールを定義します。
...
* 可能な場合は、外部呼び出しを実行する代わりに [!DNL Adobe Experience Platform] データセットルックアップ [&#x200B; を使用して、頻繁に使用するデータを &#x200B;](dataset-lookup.md) に保存します
