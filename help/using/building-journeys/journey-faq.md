...
はい。**[!UICONTROL タイムアウト]** で **反応** イベントを使用：

1. メッセージを送信した後、チャネルアクションの後に **[!UICONTROL 反応]** イベント **即時** を追加します（間に **[!UICONTROL 待機]** アクティビティは追加しません）
2. **[!UICONTROL 反応]** イベントのタイムアウト期間（3 日など）を設定して、メールの開封またはクリックをリッスンします
3. 2 つのパスを作成します。
...
2. **最初のメッセージを送信** （任意）：買い物かごの品目に対する確認のメール送信
3. **チャネルアクションの直後に [!UICONTROL  反応 ] イベントを追加**：購入イベントをリッスンするように設定します
4. **タイムアウト期間を設定**:**[!UICONTROL 反応]** イベントのタイムアウト（1～2 時間など）を定義して、顧客が自然に完了するまでの時間を設定します
5. **2 つのパスを作成します**。
   * **購入イベントが発生した場合**：ジャーニーを終了するか、購入後のフローに進みます
   * **タイムアウトパス（購入なし）**：買い物かごの内容を含む放棄リマインダーメールを送信します
6. **任意**：別の **[!UICONTROL 反応]** イベント **直後** タイムアウト付きリマインダーメール（24 時間）を追加し、インセンティブ付きの 2 回目のリマインダーを送信します（例：10% 割引）
...
* **ストリーミングオーディエンスの選定タイミング**：ストリーミングオーディエンスでオーディエンスの選定を使用するジャーニーの場合、ジャーニーが公開される前に既にオーディエンスに含まれているか、ジャーニーがまだアクティブ化期間を完了していない（公開後 10 分以内）場合、プロファイルが入力されない場合があります

詳しくは、[ エントリ管理 ](entry-management.md) および [ ストリーミングオーディエンスの選定のタイミングに関する考慮事項 ](audience-qualification-events.md#streaming-entry-caveats) を参照してください。
