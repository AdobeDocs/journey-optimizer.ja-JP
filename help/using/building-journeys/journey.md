...
title: ジャーニーの基本を学ぶ
description: ジャーニーの基本を学ぶ – ジャーニーのタイプ、ワークフロー、機能およびベストプラクティスについて説明し、パーソナライズされた顧客体験を  [!DNL Adobe Journey Optimizer] の場所で作成できます。
...
[!DNL Adobe Journey Optimizer] を使用すると、オーディエンスの行動やニーズにリアルタイムで適応する、パーソナライズされた複数のステップから成るカスタマージャーニーを作成できます。 直感的なドラッグ＆ドロップキャンバスを使用すると、コンテキストデータとオーディエンスターゲティングを活用して、複数のチャネルをまたいでメッセージとアクションを調整し、最大限の影響を得ることができます。
...
[!DNL Adobe Journey Optimizer] は、お客様へのリーチに次の 3 つのアプローチを提供します。**ジャーニー** （1:1 リアルタイムオーケストレーション）、**キャンペーン** （シンプルなバッチまたは API トリガー配信）、**オーケストレードキャンペーン** （マルチエンティティデータを含むバッチキャンバスワークフロー）。
...
[!DNL Adobe Journey Optimizer] では、4 つのジャーニータイプをサポートしており、それぞれが様々なエントリメカニズムとビジネスシナリオに合わせて設計されています。
...
イベント、[!DNL Adobe Experience Platform]、サードパーティの API サービスなどのコンテキストデータを使用できます。
...
[!DNL Adobe Journey Optimizer] には、次の 3 つのアプローチがあります。
...
これらのリンクを使用して、ガイダンス、トラブルシューティング、リソースを検索します。
