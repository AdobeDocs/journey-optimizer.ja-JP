...
1. **[!UICONTROL セグメントを読み取り]**&#x200B;アクティビティを使用すると、セグメントがジャーニーにエントリする時間を指定できます。これを行うには、「**[!UICONTROL ジャーニースケジュールを編集]** 」リンクをクリックしてジャーニーのプロパティにアクセスし、**[!UICONTROL スケジューラータイプ]**&#x200B;フィールドを設定します。
   ...
   **増分読み取り** オプション：定期的な **セグメントを読み取り** が初めて実行されると、セグメント内のすべてのプロファイルがジャーニーにエントリします。 次に発生すると、すべてのプロファイルは、既にジャーニー内にあった場合でも、再びジャーニーにエントリします。 ジャーニー内のプロファイルの古いインスタンスが停止し、新しいインスタンスが作成されます。 この **増分読み取り** 「 」オプションを使用すると、初回発生後に、ジャーニーの最後の実行以降にセグメントにエントリした個人をターゲットにすることができます。
