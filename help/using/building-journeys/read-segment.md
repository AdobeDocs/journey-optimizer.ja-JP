...
>
>ジャーニーの存続期間が次回の実行までに終わるので、セグメントジャーニーの繰り返し読み取りで待機アクティビティを使用する際は注意が必要です。 つまり、ジャーニーが毎日実行される場合、今日開始されたジャーニーインスタンスは明日の実行まで持続します。 例えば、そのジャーニーに 2 日間の待ち時間を追加した場合、プロファイルは、次回実行されるオーディエンスにあるかどうかに関わらず、次のジャーニー実行時（つまり翌日）に常に移動されます。 プロファイルは、2 日間そのジャーニーに滞在することはできません。
