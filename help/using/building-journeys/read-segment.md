...
1. **[!UICONTROL セグメントを読み取り]**&#x200B;アクティビティを使用すると、セグメントがジャーニーにエントリする時間を指定できます。これを行うには、「**[!UICONTROL ジャーニースケジュールを編集]** 」リンクをクリックしてジャーニーのプロパティにアクセスし、**[!UICONTROL スケジューラータイプ]**&#x200B;フィールドを設定します。
   ...
   「**増分読み取り**」オプション：繰り返しの&#x200B;**セグメントを読み取り**&#x200B;を含むジャーニーが初めて実行されると、セグメント内のすべてのプロファイルがジャーニーにエントリします。次に実行されると、すべてのプロファイルは（既にジャーニー内にあったものも含め）、再びジャーニーにエントリします。 ジャーニー内のプロファイルの古いインスタンスが停止し、新しいインスタンスが作成されます。**増分読み取り**&#x200B;オプションを使用すると、最初の実行後、ジャーニーの最後の実行以降にセグメントにエントリした個人をターゲットにすることができます。
...
>
>繰り返しの「セグメントを読み取り」を含むジャーニーで待機アクティビティを使用する場合は、ジャーニーの存続期間は次回の実行までなので注意が必要です。 つまり、ジャーニーが毎日実行される場合、今日開始されたジャーニーインスタンスは明日の実行まで持続します。 例えば、そのジャーニーに 2 日間の待機時間を追加した場合、プロファイルは、次回実行されるオーディエンスにあるかどうかにかかわらず、常に次のジャーニー実行時（つまり翌日）に移動されます。プロファイルは、そのジャーニーに 2 日間とどまることはできません。
