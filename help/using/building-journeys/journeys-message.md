...
プロファイルは、ウィンドウの外側にある最適な開放時間を持つことができます。 例えば、John の最適なクリック開封は午後 5 時です。

プロファイルレベルでは、1 時間ごとのスコアが表示されます。 この例では、E メールは常にウィンドウ内で送信されます。 実行時に、その期間（午前 10 時から 3 時間）内のスコアのリストがチェックされます。 次に、10、11、正午のスコアを比較し、最高のスコアを選択します。 その時点で E メールが送信されます。
