...
**このページでは：** WhatsApp API資格情報、Webhook、およびチャネル設定を設定してWhatsApp Business アカウントを接続し、環境がJourney OptimizerでWhatsApp メッセージを送信する準備ができるようにします。
...
WhatsApp API資格情報が正常に作成されたら、Webhookを次のように設定できるようになりました。
...
1. 次のような&#x200B;**[!UICONTROL インバウンドキーワードカテゴリ]**&#x200B;を選択します。
   ...
   * **[!UICONTROL ヘルプキーワード]**
   * **[!UICONTROL 既定]** – 他のキーワードと一致しないすべての受信メッセージのフォールバック カテゴリ。 このカテゴリを使用すると、Adobe Experience Platform データセットでトラッキングイベント（開封数、配信レポート）を有効にできます。
...
1. **詳細ログを有効にする**：内部サーバーまたは認証の設定ミスを特定するには、Journey Optimizer 環境で詳細ログを有効にして、API 呼び出しエラーに関する詳細情報を提供します。

1. **サポートへのお問い合わせ**：環境と使用権限が有効であることが確認されても、HTTP 500 エラーが解決しない場合は、アドビ担当者にお問い合わせください。
