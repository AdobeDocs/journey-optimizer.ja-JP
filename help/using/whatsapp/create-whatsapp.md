...
| WhatsApp を呼び出す | メッセージから直接指定された電話番号で WhatsApp チャットを開くボタンを提供します。 |
| 電話番号に電話する | ユーザーがタップしたときに、指定した番号への通話を開始するボタンを提供します。 |
...
1. 「**[!UICONTROL 画像 URL]**」フィールドにメディア URL を追加して、テンプレートのプレースホルダーを置き換えます。 メタのテンプレートメディアはプレースホルダーにすぎません。 画像、オーディオまたはビデオを正しく表示するには、Adobe Experience Managerまたはその他のソースの外部 URL を使用する必要があります。
...
以下のビデオでは、Adobe Journey Optimizerを使用して複数ステップの WhatsApp ジャーニーを作成する方法を確認できます。
