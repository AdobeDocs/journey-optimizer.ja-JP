...
実験は一連のランダム化試験で、オンラインテストのコンテキストは、ランダムに選択された一部のユーザーには特定のメッセージのバリエーションが処理され、別のランダムに選択された一連のユーザーには別の処理を施すことを意味します。メッセージを送信した後、E メールの開封数やクリック数など、興味のある結果指標を測定できます。

## 実験を実行する理由

![](assets/content_experiment_schema.png)

実験を通じて、指標の改善につながる変更を特定できます。上の画像に示すように、ランダムに選択された一部のユーザーは、各処理グループにさらされるため、平均してグループは同じ特性を共有します。したがって、結果の違いは、受け取った治療の違い、すなわち、あなたが行った変化と、あなたが興味を持った結果との間の因果関係を確立することができるので、その結果の違いは、と解釈することができます。
...
Adobe Journey Optimizerでのコンテンツ実験では、訪問者 ID の擬似ランダムハッシュを使用して、定義した処理の 1 つに対して、ターゲットオーディエンスのユーザーをランダムに割り当てます。 ハッシュメカニズムにより、訪問者が複数回キャンペーンに入った場合に、決定的に同じ処理を受け取るようにします。

詳細には、 MumurHash3 32 ビットアルゴリズムを使用して、ユーザー ID 文字列を 10,000 個のバケットの 1 つにハッシュ化します。 トラフィックの 50%を各治療に割り当てたコンテンツ実験では、バケツ 1 ～ 5,000 に入ったユーザーが最初の治療を受け、バケツ 5,001 ～ 10,000 のユーザーが 2 番目の治療を受けます。 擬似ランダムハッシュが使用されるので、観察する訪問者の分割は 50-50 とは限りません。ただし、統計的には、分割はターゲットの分割率と同じになります。

コンテンツ実験を含むすべてのキャンペーンを設定する際に、ランダム化アルゴリズム用に userId を選択する ID 名前空間を選択する必要があります。 これは、 [実行アドレス](../configuration/primary-email-addresses.md).

### データ収集と分析

割り当て時（メッセージがアウトバウンドチャネルで送信されたとき、またはユーザーがインバウンドチャネルでキャンペーンに入ったとき）に、「割り当てレコード」が適切なシステムデータセットに記録されます。 これには、ユーザーが割り当てられた処理と、実験 ID およびキャンペーン ID が記録されます。

目標指標は、次の 2 つの主なクラスにグループ化できます。

* ダイレクト指標。ユーザーが処理に直接反応する場合（E メールを開く、リンクをクリックするなど）。
* 間接的または「ファネルの底」指標。ユーザーが治療を受けた後に発生します。

Adobe Journey Optimizerがメッセージを追跡する直接目標指標の場合、エンドユーザーの応答イベントにキャンペーンと治療識別子が自動的にタグ付けされ、反応指標と治療とを直接関連付けることができます。 [トラッキングの詳細情報](../design/message-tracking.md).

![](assets/technote_2.png)

購入などの間接的または「ファネルの底」の目標の場合、エンドユーザーの応答イベントにはキャンペーンと処理 ID がタグ付けされません。つまり、処理への暴露後に購入イベントが発生し、前の処理割り当てとの直接の関連はありません。 これらの指標に関して、Adobeは、次の場合に、ファネルコンバージョンイベントの下部に処理を関連付けます。

* 割り当て時とコンバージョンイベント時に、ユーザ ID が同じである。
* 治療の割り当てから 7 日以内にコンバージョンが発生します。

![](assets/technote_3.png)

次に、Adobe Journey Optimizerは、高度な「いつでも有効な」統計的手法を使用して、この生のレポートデータを解釈します。これにより、実験レポートを解釈できます。 詳しくは、[このページ](../campaigns/experiment-calculations.md)を参照してください。
