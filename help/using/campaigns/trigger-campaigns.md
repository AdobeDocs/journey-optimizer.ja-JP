...
### メール配信の遅延 {#delivery-delays}

メール配信の時間が予想を超える場合は、クラウドインフラストラクチャープロバイダーやメールサービスプロバイダーなどの外部サービスの潜在的な停止やパフォーマンスの問題を調査します。 Journey Optimizer ログには、メッセージ送信のタイムスタンプが記録されます。これは、配信パイプラインのダウンストリームで遅延が発生したかどうかを判断するのに役立ちます。

### Azure cosmos DB認証エラー（500内部サーバーエラー） {#cosmosdb-auth-errors}
