...
キャンペーンメッセージを一度にすべてではなく、バッチで時間をかけて配信するには、ウェーブ送信を使用します。 これにより、負荷のバランスを取り、配信品質をサポートし、ダウンストリームの大量のシステム（コールセンターやランディングページなど）を回避できます。 ウェーブの数、サイズ（パーセンテージまたは絶対数）、各ウェーブのスケジュールを定義します。
