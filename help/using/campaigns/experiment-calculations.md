...
実験では [高度な統計的手法](../campaigns/assets/confidence_sequence_technical_details.pdf) 計算する **信頼性シーケンス** および **信頼性**：必要に応じて実験を実行し、結果を継続的に監視できます。
...
実験を実行する際に、2 つの母集団に違いがあるかどうか、および違いが偶然による可能性があるかどうかを判断しようとします。

一般に、次の 2 つの仮説があります。

* の **Null 仮説** これは治療に効果がないという意味だ
* の **代替仮説** つまり治療に効果があるのです

統計的有意性において、目標は、null 仮説を拒否する証拠の強さを評価することです。 重要な点の 1 つは統計的有意性を使用して治療の違いの可能性を判断し成功の確率を判断するのではなく違いの確率を判断するのです 統計的有意性が **上昇率**.

効果的な実験では、誤った推測を引き起こす可能性のある様々なタイプのエラーを考慮に入れる必要があります。

![](assets/technote_1.png)

上の表に、様々なエラーのタイプを示します。

* **偽陽性（Type-I エラー）**:は、実際には真の場合に、null 仮説を誤って拒否したものです。 オンライン実験の観点から言えば、結果指標は同じであったにもかかわらず、治療ごとに異なると誤って結論付けられる。
   </br>実験を実行する前に、通常、しきい値を選択します`\alpha`。実験が実行された後、 `p-value` が計算され、を拒否します。 `null if p < \alpha`. `/alpha` は、例えば、人の人生に影響を与える可能性がある臨床試験で間違った答えを得た結果に基づいています。 `\alpha = 0.005`. オンライン実験で一般的に使用されるしきい値は、 `\alpha = 0.05`つまり、長い目で見ると、100 個の実験のうち 5 個が偽陽性であると予想されます。
...
>[!NOTE]
>
>信頼シーケンスは、信頼区間の安全な順次類似物として解釈できます。 信頼区間では、事前に定義されたサンプルサイズに達した場合にのみ実験を解釈できます。 ただし、信頼性シーケンスを使用すると、必要に応じて実験内のデータを確認し、解釈し、安全に実験を停止または続行できます。 対応する任意の時間の有効な信頼性、または `p-value`は、いつでも解釈しても安全です。
