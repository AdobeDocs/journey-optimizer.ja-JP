...
* **キャンペーンの状況、ジャーニーのパフォーマンス、チャネル設定などを簡単な言語で即座に確認でき、** - メニューを操作したり手動でレポートを取得したりすることなく、即座に回答を得られます。
...
| **ジャーニーを一覧表示** | [!DNL Adobe Journey Optimizer]のカスタマージャーニー（自動ワークフロー）を表示し、オプションでステータス別にフィルタリング（ドラフト、ライブ、クローズ、完了）します。 |
...
| **ジャーニーインベントリ** | 「すべてのライブジャーニーを一覧表示」/「ドラフトステータスのジャーニーを表示」 |
