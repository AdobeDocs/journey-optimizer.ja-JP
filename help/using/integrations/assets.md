...
+++ アセットの有効期限が切れたことにより画像が破損した場合、どうすれば解決できますか？

アセットの有効期限が切れた際に画像の可用性を復元するには：

1. **影響を受けたアセットを再公開**：Adobe Experience Manager でアセットに移動し、再公開します。 これにより、アセットは CDN で再び使用できるようになります。

2. **コンテンツ参照を更新**：有効期限が切れたアセットを参照するコンテンツフラグメントまたはテンプレートを使用している場合は、次の操作を実行します。
   * コンテンツフラグメントのドラフトまたは複製を作成
   * アセットを再追加または再選択
   * 更新したコンテンツを公開

3. **プロアクティブな管理**：今後の中断を回避するために、アクティブなメールキャンペーンで使用されているアセット、特に TTL 有効期限が近づいているアセットを定期的に確認し、再公開することを考慮します。
