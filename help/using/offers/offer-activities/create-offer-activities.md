...
1. プロファイルごとに最適なオファーを選択するためのランキング方法を定義します。[詳細情報](../offer-activities/configure-offer-selection.md)
   ...
   * デフォルトでは、複数のオファーがこの配置の実施要件を満たす場合、 **[!UICONTROL オファーの優先度]** メソッドは、オファーで定義された値を使用します。最も優先度の高いオファーがユーザーに配信されます。

   * 特定の計算済みスコアを使用して配信する適格なオファーを選択する場合は、「 **[!UICONTROL 数式]** または **[!UICONTROL AI モデル]**. [詳細情報](../offer-activities/configure-offer-selection.md)
...
上記のように、評価基準は、コレクション、実施要件制約、およびランキング方法で構成されます。 評価条件の順序を設定できますが、複数の評価条件を組み合わせて、個別ではなく一緒に評価することもできます。

例えば、2 つのコレクションがあり、1 つは評価条件 A に、もう 1 つは評価条件 B にあります。リクエストは、2 つのオファーを送り返すようにします。 評価条件 A の適格なオファーが 2 つ、評価条件 B の適格なオファーが 3 つあるとします。

* 2 つの評価基準が **結合されない** また、順番（1 と 2）で、評価条件の上位 2 つの適格なオファーが最初の行に返されます。 最初の評価条件に適合するオファーが 2 つもない場合、決定エンジンは順番に次の評価条件に進み、まだ必要な数のオファーを見つけ、最終的には必要に応じてフォールバックを返します。

   ![](../assets/activity_consecutive-rank-collections.png)

* 2 つのコレクションが **同時に評価される**&#x200B;に含まれる場合、評価基準 A からの適格なオファーは 2 つ、評価基準 B からの適格なオファーが 3 つあるので、5 つのオファーはすべて、それぞれのランキングメソッドで決定された値に基づいて積み重ねられます。 2 つのオファーがリクエストされるので、これら 5 つのオファーの実施要件を満たす上位 2 つのオファーが返されます。
