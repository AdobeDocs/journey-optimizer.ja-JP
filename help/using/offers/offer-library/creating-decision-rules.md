...
次に、決定ルールを使用する際に考慮すべき制限事項を示します。

* ルールを作成する際には履歴イベントを使用できますが、これらのルールを使用できるタイミングには制限があります。
* エッジ判定は、イベントを保存しないエッジプロファイルを使用するので、エッジの判定で使用されるルールは無効になります。
* オファーの決定を使用するジャーニーは、履歴イベントを参照しないので、これらのルールは無効になります。
* ハブプロファイルを使用する判定リクエストでは、プロファイル上の直近 100 件のエクスペリエンスイベントを調べ、過去のエクスペリエンスイベントを参照するルールを評価します。
