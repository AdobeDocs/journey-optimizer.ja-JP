...
1. 次を確認します。 **[!UICONTROL キャッピングを有効にする]** 切り替えボタンがオンになっている。 キャッピングはデフォルトで有効になっています。
...
1. 複数の [表示域](add-representations.md) オファーに対して、キャッピングを適用するかどうかを指定します **すべての配置をまたいで** または **各配置に対して**. [詳細情報](#placements)
...
The **[!UICONTROL キャッピングイベント]** 「 」フィールドでは、カウンターを増やすために考慮に入れるイベントを定義できます。
...
The **[!UICONTROL 上限数の制限]** 「 」フィールドでは、オファーを提示できる回数を指定できます。
...
例えば、チェックアウト数が考慮されるなどのカスタムキャッピングイベントを定義したとします。 10 と入力すると、 **[!UICONTROL 上限数の制限]** 「 」フィールドに値を入力すると、10 件のチェックアウト後はこれ以上オファーが送信されなくなります。
...
例えば、2 週間ごとに制限数をリセットする場合は、「 **[!UICONTROL 毎週]** 対応するドロップダウンリストから、「 」と入力します。 **2** 他のフィールドに。 リセットは毎週日曜日の午後 12 時（UTC）に発生します。

>[!CAUTION]
>
>オファーを公開した後は、頻度に選択した期間（毎月、毎週、毎日）を変更できなくなります。
>
>オファーのステータスが&#x200B;**[!UICONTROL ドラフト]**&#x200B;で、これまでにフリークエンシーキャップを有効にして公開したことがない場合であれば、フリークエンシーキャップを編集できます。

### キャッピングと配置 {#placements}

複数の [表示域](add-representations.md) オファーの場合、すべての配置に対してキャッピングを適用するか、各配置に対してキャッピングを適用するかを指定します。

![](../assets/offer-capping-placement.png)

* **[!UICONTROL すべての配置に対する制限の適用]**：上限数は、オファーに関連付けられた配置全体のすべての決定の合計となります。
...
* **[!UICONTROL 各プレースメントにキャッピングを適用]**：上限カウントでは、各配置に対して個別に判定回数が適用されます。
