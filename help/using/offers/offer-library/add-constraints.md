...
1. 複数の [表示域](#representations) オファーに対して、次の条件を満たす制限を適用するかどうかを指定します： **[!UICONTROL すべての配置]** または **[!UICONTROL 配置ごと]**.
   ...
   * **[!UICONTROL 配置ごと]**:キャッピングカウントでは、各プレースメントに対して個別に判定回数が適用されます。
