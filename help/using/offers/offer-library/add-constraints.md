...
1. **[!UICONTROL キャッピングを有効にする]**&#x200B;切替スイッチがオンになっていることを確認します。キャッピングは、デフォルトで有効です。
...
メールの準備時に、オファーが提案された回数が計算されます。例えば、複数のオファーを含んだメールを準備する場合、そのメールが送信されるかどうかに関係なく、それらのオファーの数はオファーの提示回数に加算されます。
...
「**[!UICONTROL キャッピングイベント]**」フィールドでは、カウンターを増やすためにどのイベントを考慮するかを定義できます。
...
「**[!UICONTROL 頻度]**」セクションでは、キャッピングカウントをリセットする頻度を定義できます。 これをおこなうには、カウントの期間（日単位、週単位、月単位）を定義し、日数/週数/月数を入力します。 例えば、2 週間ごとに制限数をリセットする場合は、「 **[!UICONTROL 毎週]** 対応するドロップダウンリストから、「 」と入力します。 **2** 他のフィールドに。

![](../assets/offer-capping-frequency.png)

>[!NOTE]
>
>頻度キャップカウンターのリセットは、次の時点で発生します： **UTC の午前 12 時**：定義した日、または該当する場合は週の最初の日。 週の開始日は **日曜日**. 選択した期間は次の値を超えることはできません： **2 年** （つまり、対応する月数、週数、日数）。
>
>オファーを公開すると、頻度に選択した期間（月、週、日）を変更できなくなります。 オファーのステータスが&#x200B;**[!UICONTROL ドラフト]**&#x200B;で、これまでにフリークエンシーキャップを有効にして公開したことがない場合であれば、フリークエンシーキャップを編集できます。

+++ **必読：頻度キャップと Edge Decisioning API**

フリークエンシーキャップカウンターが 3 秒以内に更新され、Edge Decisioning API の決定で使用できるようになります。

各ハブ領域は、1 つ以上のエッジ領域に関連付けられています。 頻度キャップルールが生成され、各ハブ領域から関連するエッジ領域に書き出されます。 Edge Decisioning API を使用して決定がおこなわれるたびに、システムは同じエッジ領域で使用可能なルールを適用します。

* 一致ルールがある場合、プロファイルの頻度キャップカウンターが増分されます。
* そうしないと、プロファイルにカウンターが作成されず、頻度キャップルールは適用されません。 その結果、上限のしきい値を超えても、プロファイルは引き続きパーソナライズされたオファーを受け取ります。

例えば、組織のハブ地域をと考えてみましょう。 *NLD2*&#x200B;を検索し、ヨーロッパから判定リクエストを送信している (*IRL1* エッジ領域 ) で使用されます。 このシナリオでは、ルールが（アイルランド）で使用可能なので、判定リクエストはプロファイルのカウンターを増分します *IRL1* 地域。 ただし、判定リクエストが日本のような地域 (*JPN3*) で、（オランダ）に結び付けられた端の地域ではありません *NLD2* ハブ領域の場合は、カウンターは作成されず、頻度キャップルールは適用されません。

組織に関連付けられているハブ領域とエッジ領域の詳細については、Adobe担当者にお問い合わせください。
