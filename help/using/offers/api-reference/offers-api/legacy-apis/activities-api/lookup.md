...
| `id` | に一致させる文字列 `@id` エンティティのプロパティ。 文字列は完全に一致します。 パラメーター `id` および `name` 一緒に使用することはできません。 | `xcore:offer-activity:124527ab00b2ebbc` |
| `name` | エンティティの xdm:name プロパティと一致させるために使用される文字列。 文字列は大文字と小文字が完全に一致しますが、ワイルドカード文字を使用できます。 パラメーター&quot;id&quot;と&quot;name&quot;は併用できません | `LBAR` |
