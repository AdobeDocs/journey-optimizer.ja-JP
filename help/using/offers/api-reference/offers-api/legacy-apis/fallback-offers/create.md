...
正常な応答では、新たに作成されたフォールバックオファーに関する情報（一意のインスタンス ID とプレースメントを含む）が返されます `@id`. 後の手順で、このインスタンス ID を使用してフォールバックオファーを更新または削除できます。 後のチュートリアルでは、一意のフォールバックオファー `@id` を使用して決定を作成できます。
