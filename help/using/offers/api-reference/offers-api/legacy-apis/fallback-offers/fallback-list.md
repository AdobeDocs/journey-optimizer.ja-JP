...
に対してGETリクエストを 1 回送信すると、コンテナ内のすべてのフォールバックオファーのリストを表示できます。 [!DNL Offer Library] API です。
...
| パラメーター | 説明 | 例 |
| --------- | ----------- | ------- |
| `q` | 選択したフィールドで検索するオプションのクエリ文字列。 クエリ文字列は小文字にする必要があり、トークン化を防ぎ、特殊文字をエスケープするために、二重引用符で囲むことができます。 文字 `+ - = && \|\| > < ! ( ) { } [ ] ^ \" ~ * ? : \ /` 特別な意味を持ち、クエリ文字列に含める場合は、バックスラッシュでエスケープする必要があります。 | `default` |
...
応答が成功すると、アクセス権を持つコンテナ内に存在するフォールバックオファーのリストが返されます。
