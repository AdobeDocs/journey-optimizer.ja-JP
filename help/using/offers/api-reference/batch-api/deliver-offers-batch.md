...
# を使用してオファーを配信 [!DNL Batch Decisioning] API
...
を使用 [!DNL Batch Decisioning] API を使用すると、決定範囲に対して、Adobe Experience Platformセグメント内のすべてのプロファイルに最適なオファーをデータセットに入力できます。 例えば、組織は、メッセージ配信ベンダーにオファーを送信できるようにバッチ判定を実行する場合があります。 その後、これらのオファーは、同じセグメントのユーザーに対するバッチメッセージ配信用に送信されるコンテンツとして使用されます。

これをおこなうには、次の手順を実行します。

* を実行します。 [!DNL Batch Decisioning] 次の 2 つのリクエストを含む API。

   1. A **バッチGETリクエスト** ：バッチ・ワークロード・ステータスを取得します。

   2. A **バッチPOSTリクエスト** ：オファーの選択をバッチ処理するワークロードを開始します。
...
* 決定結果を書き出すには、「ODE 決定イベント」スキーマを使用してデータセットを作成します。
...
すべてのバッチ判定リクエストには、次の追加ヘッダーが必要です。
...
| プロパティ | 説明 | 例 |
| -------- | ----------- | ------- |
| `xdm:segmentIds` | セグメントの一意の ID。 値を 1 つだけ含めることができます。 | `609028e4-e66c-4776-b0d9-c782887e2273` |
...
各バッチ判定の終了時間は、ワークロードが作成された時点から出力データセットで使用可能な判定結果までの期間です。 POSTリクエストペイロードのセグメントサイズが、エンドツーエンドのバッチ判定時間に影響する主な要因です。 次に、異なるセグメントサイズに関する観察を示します。
...
には次の制限があります。 [!DNL Batch Decisioning] API:

* **単一のバッチジョブ**:現在、一度に 1 つのバッチジョブのみをデータセットごとに実行できます。 その他の要求は HTTP 429（要求が多すぎます）で応答します。
* **頻度キャップ**:1 日に 1 回発生するプロファイルスナップショットからバッチが実行されます。 この [!DNL Batch Decisioning] API は頻度を制限し、常に最新のスナップショットからプロファイルを読み込みます。

## 次の手順 {#next-steps}

この API ガイドに従うことで、 [!DNL Batch Decisioning] API 詳しくは、[意思決定管理の概要](../../get-started/starting-offer-decisioning.md)をご覧ください。
