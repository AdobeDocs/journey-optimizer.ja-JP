...
コンテンツから「**[!UICONTROL コンテンツをシミュレート]**」、「**[!UICONTROL コンテンツのバリエーションをシミュレート]**」を選択して、次の操作を実行できる単一のエクスペリエンスを開きます。
...
* **バリエーションの数** - ファイルを使用するか、手動で使用するか、自動生成を使用して、コンテンツをテストするには、最大30個のバリエーションを追加できます。
...
[!DNL Journey Optimizer]は、AI ベースのシミュレーションを使用してコンテンツのバリエーションを自動的に生成できるため、バリエーションを手作業で作成することなくパーソナライゼーション ロジックを検証できます。

シミュレーションや校正のためにコンテンツをレンダリングする場合、コンテンツを分析し、パーソナライゼーションフィールドを特定して、ほぼ現実的なプレビューのために有意義な値に置き換えます。

バリエーションを自動生成するには、「**[!UICONTROL 生成]**」ボタンをクリックし、システムがバリエーションを生成するのを待ちます。
...
バリエーション リストとそのレンダリングで、生成されたバリエーションを確認します。
