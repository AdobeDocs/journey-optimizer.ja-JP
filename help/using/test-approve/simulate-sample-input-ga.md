...
* **バリエーションの数** - ファイルを使用するか、手動で使用するか、自動生成を使用して、コンテンツをテストするには、最大30個のバリエーションを追加できます。 AIによる自動生成を使用すると、最大20のバリエーションが生成されます。

## コンテンツバリアントを作成

コンテンツのバリエーションを作成するには、「**[!UICONTROL コンテンツをシミュレート]**」ボタンをクリックします。

![「コンテンツをシミュレート」ボタン](assets/simulate-sample.png)
...
[!DNL Journey Optimizer]は、AI ベースのシミュレーションを使用してコンテンツのバリエーションを自動的に生成できるため、バリエーションを手作業で作成することなくパーソナライゼーション ロジックを検証できます。 シミュレーションや校正のためにコンテンツをレンダリングする場合、コンテンツを分析し、パーソナライゼーションフィールドを特定して、ほぼ現実的なプレビューのために有意義な値に置き換えます。
...
>[!NOTE]
>
>生成は、単一のバリエーションを生成します。 「**[!UICONTROL 生成]**」をクリックすると、リスト内の既存のコンテンツバリエーション（手動またはファイルから追加されたものを含む）がすべて、生成されたバリエーションに置き換えられます。
