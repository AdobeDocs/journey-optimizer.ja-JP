...
[!DNL Journey Optimizer] では、CSV または JSON ファイルからアップロードされた、または手動で追加されたサンプル入力データを使用して、様々なバリエーションのコンテンツをプレビューできます。
...
![ 入力バリアントリストのサンプル ](assets/simulate-custom-variants-list.png)
...
![ サンプル入力ボタンの作成 ](assets/simulate-custom-add.png)
...
1. アップロード後、ファイル内の各エントリのリストに新しいバリアントが追加されます。
   ...
   ![ アップロードされたサンプル入力バリアント ](assets/simulate-custom-variants.png)
...
|----------|-------------|
| ![ バリアント 1 の選択 ](assets/simulate-custom-boxes.png) | ![ バリアント 2 の選択 ](assets/simulate-custom-boxes2.png) |

多言語コンテンツと実験の場合、ドロップダウンを使用して、様々な言語のバリアントまたは処理を切り替えることができます。

![ 言語または処理のセレクター ](assets/simulate-custom-experiment.png)
