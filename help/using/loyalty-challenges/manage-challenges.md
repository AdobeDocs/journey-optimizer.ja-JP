...
### チャレンジの作成* 初めてのチャレンジはシンプルから* 実稼動環境に公開する前に十分にテストする* 要件と報酬に関する明確なコミュニケーションの確保* 現実的なタイミングを設定して、お客様が課題を完了できるようにする

### 管理と監視* チャレンジのパフォーマンスを定期的に確認してください（ライブのチャレンジについては少なくとも毎週）* 目的、オーディエンス、期間を示すわかりやすい名前を使用します* 組織に一貫したタグを適用する* 完了した課題をアーカイブしてインベントリを管理できるようにする
