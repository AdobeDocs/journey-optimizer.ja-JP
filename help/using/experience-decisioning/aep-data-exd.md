...
**このページ：**&#x200B;意思決定ルール、ランキング式、キャッピングルールでAdobe Experience Platform データセット属性を使用すると、在庫や価格など、頻繁に変化する動的な外部データを使用して意思決定ロジックを強化できます。
...
>title="データセットの追加"
>abstract="[!DNL Adobe Experience Platform] データセットを使用して、動的な外部属性に基づいてキャップ条件を定義します。 「**[!UICONTROL データセットを追加]**」をクリックして、必要な属性を持つデータセットを選択します。 データセットがリストに表示されない場合は、ルックアップに対してデータセットが有効になっていて、データセットのルックアップマッピングが作成されていることを確認します。"
...
* **サポートされているチャネル** - Decisioningを使用したデータセット検索は、Decisioningが利用可能なすべてのチャネル（コードベースのエクスペリエンス（インバウンド）、電子メール、プッシュ、SMS、ジャーニー内のコンテンツ決定ノード）で機能します。
* **属性使用状況** – 決定用のデータセット検索機能は、決定項目の定義を追加の属性で拡張します。属性をプロファイルに拡張しません。
