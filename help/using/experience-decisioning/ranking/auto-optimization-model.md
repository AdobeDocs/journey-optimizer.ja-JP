...
* データセット内の 2 つ以上のオファーには、過去 14 日以内に 100 以上の表示イベントと 5 つのクリックイベントが必要です。
* 過去 14 日以内に 100 件未満のディスプレイや 5 回のクリックイベントがあるオファーは、モデルによって新しいオファーとして扱われ、探索バンディットから提供される資格のみが付与されます。
* 100 を超えるディスプレイと 5 つのクリックイベントが過去 14 日以内に含まれるオファーは、モデルによって既存のオファーとして扱われ、探索と活用の両方の盗賊から提供される資格があります。

自動最適化モデルが初めてトレーニングされるまでは、自動最適化モデルを利用した選択戦略内のオファーはランダムに提供されます。
