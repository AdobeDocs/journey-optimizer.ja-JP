...
* データセット内の 2 個以上のオファーには、過去 30 日以内に 250 回以上の表示イベント数と 25 個の成功イベント数（クリック数やコンバージョン率など）が必要です。
* 過去 30 日以内の表示回数が 250 回未満のオファーや、成功イベント数が 25 個未満のオファーは、パーソナライズされたトラフィックに含まれる実施要件が満たされますが、このしきい値を超えるまでは、パーソナライゼーションモデルによって、最もスコアが低い * オファーのレベルでパフォーマンスが行われているものとして処理されます。
* 過去 30 日以内の表示回数が 250 回未満のオファーや、成功イベント数が 25 個未満のオファーは、引き続き探索トラフィックに含まれる実施要件が満たされます。
...
* **ユーザーが検討中のオファー間で様々な環境設定を使用できるよう、オファーは十分に異なります**。 オファーが類似しすぎる場合、応答が一見ランダムに見えるので、結果として得られるモデルの影響は少なくなります。
例えば、銀行に色の違いのみがある 2 つのクレジットカードオファーがある場合、どのカードを推奨するかは問題になりませんが、各カードの条件が異なる場合、これは特定の顧客が 1 つを選択する理由の根拠となり、より影響力のあるモデルを作成するのに十分なオファー間の違いが得られます。
...
最新の機能インタラクションを学習し、モデルパフォーマンスの低下を軽減するために、モデルの再トレーニングが毎週行われます。 トレーニングの状態とモデルのパフォーマンスを監視するには、[AI モデル監視](ai-model-observability.md)を参照してください。
