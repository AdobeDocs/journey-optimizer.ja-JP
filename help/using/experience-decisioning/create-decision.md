...
コードベースのキャンペーンに決定ポリシーを活用する主な手順は次のとおりです。

1. [決定ポリシーをコードベースのキャンペーンに作成する](#add-decision)
1. [決定ポリシーをコードベースのキャンペーンに使用](#use-decision-policy)
1. [カスタムCustomer Journey Analyticsレポートダッシュボードの作成](#cja)
...
「**[!UICONTROL 戦略シーケンス]**」セクションでは、決定ポリシーで提示する決定項目と選択戦略を選択できます。

1. 「**[!UICONTROL 追加]**」ボタンをクリックして、ポリシーに含めるオブジェクトのタイプを選択します。

   * **[!UICONTROL 選択戦略]**:1 つまたは複数の選択戦略を追加します。 決定戦略では、実施要件の制約とランキング方法に関連付けられたコレクションを活用して、表示する項目を決定します。 既存の選択戦略を選択するか、「選択戦略を作成 **[!UICONTROL ボタンを使用して新しい選択戦略を作成でき]** す。[ 選択戦略の作成方法を説明します ](selection-strategies.md)。

   * **[!UICONTROL 決定項目]**：選択戦略を実行しなくても提示する決定項目を 1 つ追加します。 一度に選択できる決定項目は 1 つだけです。 品目に設定されている実施要件制約が適用されます。

   ![](assets/decision-code-based-strategy-sequence.png)

   >[!NOTE]
   >
   >少なくとも 1 つの決定項目または戦略が必要です。 10 個以上の戦略を追加することはできません。

1. 複数の決定項目や戦略を追加する場合、それらは特定の順序で評価されます。 シーケンスに最初に追加されたオブジェクトが最初に評価され、以下同様に評価されます。     デフォルトの順序を変更するには、オブジェクトやグループをドラッグ&amp;ドロップして、必要に応じて並べ替えることができます。 [ 決定ポリシーの評価順序について詳しくはこちらを参照 ](#evaluation-order)

### 決定ポリシー内の評価順序の管理 {#evaluation-order}

決定項目と選択戦略をポリシーに追加したら、その順序を調整して評価順序を決定し、選択戦略を組み合わせて評価することができます。

各オブジェクトまたはオブジェクトのグループの左側には、アイテムとストラテジーが評価される **順序順** が数字で示されます。 シーケンス内の選択戦略（または戦略のグループ）の位置を移動するには、別の位置にドラッグ&amp;ドロップします。

>[!NOTE]
>
>シーケンス内でドラッグ&amp;ドロップできるのは、選択ストラテジーのみです。 決定項目の位置を変更するには、決定項目を削除し、前に評価する他の項目を追加した後に、「**[!UICONTROL 追加]**」ボタンを使用して元に戻す必要があります。

![](assets/decision-code-based-strategy-groups.png)

複数の選択戦略をグループに **結合** して、個別にではなくまとめて評価することもできます。 これを行うには、選択戦略の下の「**`+`**」ボタンをクリックして、別の選択戦略と組み合わせます。 選択戦略を別の戦略にドラッグ&amp;ドロップして、2 つの戦略をグループにグループ化することもできます。

>[!NOTE]
>
>決定項目を他の項目や選択戦略と一緒にグループ化することはできません。
...
### フォールバックオファーを追加 {#fallback}

決定項目や選択戦略を選択したら、上記の項目や選択戦略のいずれも適格でない場合にユーザーに表示されるフォールバックオファーを追加できます。
...
>[!NOTE]
>
>フォールバックはオプションです。フォールバックが選択されておらず、戦略が選定されていない場合、[!DNL Journey Optimizer] によって何も表示されません。 決定ポリシーがリクエストしている項目数まで追加できます。 これにより、ユースケースで必要に応じて、返される一定数の項目が保証されます。
