...
決定ポリシーは、決定エンジンを活用してオーディエンスメンバーごとに配信する最適なコンテンツを動的に返す、オファーのコンテナです。 目標は、各プロファイルに最適なオファーを選択することです。一方、キャンペーン／ジャーニーのオーサリングでは、メッセージに含める項目属性など、選択した決定項目の表示方法を指定できます。

## 主な手順 {#key}

決定ポリシーをメッセージに活用する主な手順は次のとおりです。

1. [メールまたはコードベースのエクスペリエンスでの決定ポリシーの作成](#add-decision)

   返す項目数の選択、選択戦略、フォールバックオプションおよび評価順序の設定により、メールまたはコードベースのエクスペリエンスに決定ポリシーを設定します。

1. [コンテンツでの決定ポリシーの使用](#use-decision-policy)

   決定ポリシーの出力と、メッセージに表示する決定項目の属性を使用して、コンテンツをパーソナライズします。

1. [レポートダッシュボードの作成](cja-reporting.md)

   カスタム Customer Journey Analytics ダッシュボードを作成してパフォーマンスを測定し、意思決定ポリシーとオファーがどのように配信され、エンゲージメントされているかに関するインサイトを得ます。

## ガードレールと制限

* **限定提供（LA） – メールの決定ポリシー** – 現時点では、メールの決定ポリシー作成は限定提供で利用できます。 アクセス権を取得するには、Adobe担当者にお問い合わせください。
* **ミラーページ** – 現時点では、決定項目はメールのミラーページではレンダリングされません。
* **トラッキングとリンクのタイプ** - Decisioning によって生成されたリンクをトラッキングするには、スキーマでリンクを「Decisioning Assets」として定義します。 属性ベースのリンクは追跡できません。
* **決定ポリシーのメールへのネスト** – 既に決定ポリシーが関連付けられている親メールコンポーネント内に複数の決定ポリシーをネストすることはできません。
* **決定を含む重複したジャーニー/キャンペーン** – 決定ポリシーを含むジャーニーまたはキャンペーンを複製した場合、複製されたバージョンは元のメールまたはコードベースのエクスペリエンスを参照し、エラーを引き起こします。 複製後は、常に決定ポリシーを再設定します。
* **同意ポリシー** – 同意ポリシーの更新が有効になるまでに最大 24 時間かかる場合があります。 決定ポリシーが最近更新された同意ポリシーに関連付けられた属性を参照した場合、変更は直ちに適用されません。
...
* **AI ランキング** – 現時点では、意思決定を含むジャーニーのメールチャネルでは、AI ランキングはサポートされていません。

## メールまたはコードベースのエクスペリエンスでの決定ポリシーの作成 {#add-decision}
...
Web サイトまたはモバイルアプリでメールの受信者と訪問者に最適な動的なオファーとエクスペリエンスを提示するには、メールまたはコードベースのキャンペーンまたはジャーニーに決定ポリシーを追加します。 これを行うには、以下の手順に従います。
...
1. メールの場合は、設定画面で **[!UICONTROL 決定を有効にする]** を切り替えます。
   ...
   >[!IMPORTANT]
   >
   >決定を有効にすると、既存のメールコンテンツがクリアされます。 メールを既にデザインしている場合は、事前にコンテンツをテンプレートとして保存しておいてください。
   >
   >メール内で設定された決定ポリシーは、テンプレートには保存されません。 テンプレートを別のメールに適用する場合は、ポリシーを再設定する必要があります。

1. ポリシーは、パーソナライゼーションエディターを使用して、メールおよびコードベースのエクスペリエンスで作成できます。 また、メールDesignerの専用メニューからメールで作成することもできます。 詳しくは、以下の節を展開してください。
...
+++
   ...
   1. **[!UICONTROL 決定出力を再利用]** を選択して、このメール内で既に作成された決定ポリシーを再利用します。
...
1. 返す項目数を選択します。 例えば、「2」を選択した場合、現在の設定に対して最適な 2 つの実施要件を満たすオファーが表示されます。

   ![](assets/decision-code-based-details.png)

   メールの場合、複数の項目は **[!UICONTROL 繰り返しグリッド]** コンテンツコンポーネントでのみ返すことができます。 詳しくは、以下の節を展開してください。

+++ メールで複数の決定項目を返す

   1. **[!UICONTROL グリッドを繰り返す]** コンポーネントをキャンバスにドラッグし、**[!UICONTROL 設定]** パネルを使用して必要に応じて設定します。

      ![](assets/decision-policy-repeat.png)

   1. キャンバスツールバーの **[!UICONTROL 決定]** アイコンをクリックするか、**[!UICONTROL 決定]** パネルを開いて「**[!UICONTROL 決定ポリシーを追加]**」を選択します。

   1. 返す項目数を **[!UICONTROL 項目数]** フィールドに指定し、以下に記載されているように決定ポリシーを設定します。 選択できる項目の最大数は、**[!UICONTROL グリッドを繰り返し]** コンポーネントで定義されたタイル数によって制限されます。
...
1. 複数の決定項目や戦略を追加した場合、それらは特定の順序で評価されます。シーケンスに最初に追加されたオブジェクトが最初に評価され、それ以降も追加された順に評価されます。デフォルトの順序を変更するには、オブジェクトやグループをドラッグ&amp;ドロップして、必要に応じて並べ替えます。 詳しくは、以下の節を展開してください。
   ...
   **複数の戦略がある場合の例**
   ...
   次に、複数の戦略を異なるグループに分割した例について考えてみましょう。 3 つの戦略を定義しました。戦略 1 と戦略 2 はグループ 1 で結合され、戦略 3 は独立しています（グループ 2）。 各戦略の実施要件を満たすオファーとその優先度（ランキング関数評価で使用）は、次のとおりです。
...
決定項目や選択戦略を選択したら、フォールバックオファーを追加して、上記の項目や選択戦略のいずれも選定されていない場合に表示できます。
...
>[!NOTE]
> フォールバックはオプションです。 リクエストされた数を上限に、アイテムを選択できます。 何も適格ではなく、フォールバックが設定されていない場合は、何も表示されません。

### 決定ポリシーの保存と管理 {#save}

決定ポリシーの準備が整ったら、保存し、「**[!UICONTROL 作成]**」をクリックします。

メールの場合、決定ポリシーに関連付けられたコンポーネントのプレースメントを定義する必要があります。 これを行うには、コンポーネントプロパティパネルの「**[!UICONTROL 決定]**」ボタンをクリックし、「**[!UICONTROL 配置を割り当て]**」を選択します。 [ プレースメントの操作方法については、こちらを参照してください ](../experience-decisioning/placements.md)

![](assets/decision-policy-rail.png)

パーソナライゼーションエディター、またはコンポーネントのプロパティペイン内の **[!UICONTROL 決定]** メニューの省略記号ボタンを使用して、いつでも決定ポリシーを編集または削除できます。
...
## コンテンツでの決定ポリシーの使用 {#use-decision-policy}
...
1. メールの場合、「**[!UICONTROL 構文を挿入]**」をクリックして、決定ポリシーに対応するコードを追加します。 コードベースのエクスペリエンスの場合は、「**[!UICONTROL ポリシーを挿入]**」をクリックします。
   ...
   メールの場合、コンポーネントにプレースメントが事前に関連付けられていない場合は、リストからプレースメントを選択し、「**[!UICONTROL 割り当て]**」をクリックします。
...
* [キャンペーンのアクティブ化のレビュー](../campaigns/review-activate-campaign.md)
* [コードベースのエクスペリエンスの公開とアクティブ化](../code-based/publish-code-based.md)

コードベースのエクスペリエンスの場合、デベロッパーが API またはSDK呼び出しを実行し、チャネル設定で定義されたサーフェスのコンテンツを取得するとすぐに、変更内容が web ページまたはアプリに適用されます。
