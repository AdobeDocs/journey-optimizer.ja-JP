...
1. フラグメントを使用して、決定項目に複数のコンテンツを追加します。例えば、複数のモバイルデバイスモデルに対して異なるコンテンツを表示する場合などです。[詳しくは、フラグメントを参照してください。](../content-management/fragments.md)
   ...
   「**[!UICONTROL フラグメント]**」セクションで、使用する公開済みフラグメントを選択し、参照キーを割り当てます。その後、これらのフラグメントを決定ポリシーで活用できます。[詳細情報](fragments-decision-policies.md)
...
キャッピングルールを設定する際に、Adobe Experience Platform データセットに保存されている属性を参照して、しきい値を定義できます。データセットを使用するには、「**[!UICONTROL データセット]**」セクションでデータセットを選択します。 [Adobe Experience Platform データを意思決定に使用する方法を説明します ](../experience-decisioning/aep-data-exd.md)
...
1. カウンターを増やすために、どの&#x200B;**[!UICONTROL キャップイベント]**&#x200B;を考慮するかを定義します。
   ...
   * **[!UICONTROL カスタムイベント]**：引き換え、購入、買い物かごのチェックアウトなど、Adobe Experience Platformで追跡するビジネスまたは行動エクスペリエンスイベントに基づくキャップ。 カスタムイベントキャッピングでは、取り込んだ [Adobe Experience Platform XDM](https://experienceleague.adobe.com/docs/experience-platform/xdm/home.html?lang=ja){target="_blank"} エクスペリエンスイベントを使用します。 ドロップダウンで、キャッピングカウンターがイベントを受け取るたびに増分されるように、キャップを駆動する特定のエクスペリエンスイベントをマッピングします。 メール送信など、チャネル配信イベントによるキャッピングはサポートされていません。カスタムイベントは、取り込んだエクスペリエンスイベントにのみ適用され、配信イベントや送信イベントには適用されません。

   +++プッシュチャネルのキャッピング

   標準の **[!UICONTROL クリック数]** および **[!UICONTROL インプレッション]** キャッピングは、プッシュチャネルではサポートされていません。 プッシュを介して配信されるオファーをキャップするには、**[!UICONTROL カスタムイベント]** キャッピングを使用し、イベントタイプを **プッシュトラッキングアプリケーションが開封されました** または **プッシュトラッキングカスタムアクション** に設定します。

   プッシュ通知の場合、モバイルチャネルからのトラッキングイベントには、Experience Cloud ID （ECID）が含まれます。 ID の一貫性を維持し、キャッピングが期待どおりに動作するように、キャンペーンまたはジャーニー設定で ECID を使用することをお勧めします。

   ![](assets/push-capping.png)

   +++

   +++キャッピングイベントのトラッキング（スキーマとデータ収集）
