...
1. キャッピングしきい値を定義します。 そのためには、静的な値を入力するか、式を使用してしきい値を計算します。 詳しくは、以下の節を展開してください。
   ...
   キャッピングしきい値に静的な値を使用する代わりに、独自の式を定義できます。 これにより、Adobe Experience Platform データセットの決定属性や外部属性を使用して、しきい値を動的に計算できます。
   ...
   >[!NOTE]
   >
   >キャッピングルール **式** は現在、すべてのユーザーに対する使用制限として使用できます。 これらは、**[!UICONTROL 合計]** キャッピングタイプでのみサポートされます。
