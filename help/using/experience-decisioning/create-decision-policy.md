...
1. [決定ポリシーの追加](#add)
1. [決定ポリシーを設定](#configure) – 名前を追加し、電子メール チャネルとダイレクトメールチャネルの場合は、返すアイテム数を指定します。
...
ジャーニーまたはキャンペーンで設定済みのチャネルアクションにアクセスし、メッセージのコンテンツを編集します。 選択したチャネルに基づいて決定ポリシーを追加する方法について詳しくは、以下のタブを参照してください。
...
1. 「**[!UICONTROL 項目数]**」フィールドでは、決定ポリシーで返される決定項目の数を定義できます。 例えば、「2」を選択した場合、現在の設定に対して最適な 2 つの実施要件を満たすオファーが表示されます。
   ...
   >[!NOTE]
   >
   >**[!UICONTROL 項目数]** フィールドは、**電子メール**、**コードベースのエクスペリエンス**、**ダイレクトメール**&#x200B;の各チャネルで使用できます。 **SMS**&#x200B;および&#x200B;**プッシュ** チャネルの場合、1つのアクションにつき1つの決定項目のみを返すことができます。
...
1. 複数の決定項目や戦略を追加した場合、それらは特定の順序で評価されます。 シーケンスに最初に追加されたオブジェクトが最初に評価され、それ以降も追加された順に評価されます。 デフォルトのシーケンスを変更するには、オブジェクトやグループをドラッグ＆ドロップして、任意の順序に並べ替えます。 詳しくは、以下の節を参照してください。
   ...
   * **反復 1：**
     ...
     オファー1 - 10
オファー2 - 20
オファー3～30は戦略1から、45は戦略2から。両方の最高が考慮されるので、45が考慮されます。
オファー4 - 40
オファー5 - 50
