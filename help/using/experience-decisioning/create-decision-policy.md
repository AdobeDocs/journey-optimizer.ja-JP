...
1. [決定ポリシーの追加](#add)
1. [ 決定ポリシーを設定 ](#configure) – 名前を追加し、メールチャネルに返す項目数を指定します。
...
1. [ プレースメントの割り当て ](#placement) （メールチャネル）

>[!AVAILABILITY]
>
>決定ポリシーは、すべてのお客様が **コードベースのエクスペリエンス**、**プッシュ通知** および SMS チャネルで利用できます。
>
>メールチャネルの決定は、限定提供（LA）で利用できます。 アクセス権をリクエストするには、Adobe担当者にお問い合わせください。 詳細情報 [ 可用性ラベル ](../rn/releases.md#availability-labels)。

## 決定ポリシーの追加 {#add}

メッセージに決定ポリシーを追加するには、ジャーニーまたはキャンペーンを開き、[ チャネルアクション ](../building-journeys/journeys-message.md) を選択します。

メッセージのコンテンツを編集し、以下のタブを参照して、選択したチャネルに基づいて決定ポリシーを追加する方法を確認してください。

>[!BEGINTABS]

>[!TAB  コードベースのエクスペリエンス ]

コードベースのエクスペリエンスの場合は、プロパティペインの **コードエディター** または **決定** メニューを使用して、新しい決定ポリシーを追加できます。

+++コードエディターからの決定ポリシーの追加

1. 「**[!UICONTROL コードを編集]** ボタンを使用して、コードエディターエディターを開きます。
...
1. プロパティパネルの「![](assets/do-no-localize/decisioning-icon.png)」アイコンをクリックして **[!UICONTROL Decisioning]** メニューにアクセスします。
...
1. **パーソナライゼーションエディター** または電子メールデザイナーで使用可能な **決定** メニューを使用して、新しい決定ポリシーを追加します。

   +++Personalization エディターから決定ポリシーを追加

   1. パーソナライゼーションを追加できる、「件名」フィールドまたはメール本文の任意のフィールドで使用できる ![](assets/do-no-localize/editor-icon.svg) アイコンを使用して、パーソナライゼーションエディターを開きます。
   ...
   >[!NOTE]
   >
   >**[!UICONTROL 決定出力を再利用]** オプションを使用すると、このメールで既に作成された決定ポリシーを再利用できます。 同じオファーを複数の場所（ヘッダーやフッターなど）に表示する場合に特に便利です。
   >
   >メール本文の複数の決定ポリシーで同じオファーを選択できる場合、エンジンによってオファーの重複が排除されます。各プレースメントは異なるオファーを受け取るので、同じオファーが両方の場所に表示されることはありません。 同じオファーを複数のプレースメントに表示するには、**[!UICONTROL 決定出力を再利用]** を使用して、このメール内の既存の決定ポリシーの出力を再利用します。

>[!TAB SMS]

SMS の場合、プロパティペインにある **パーソナライゼーションエディター** または **決定** メニューを使用して、新しい決定ポリシーを追加できます。

+++パーソナライゼーションエディターから決定ポリシーを追加

1. パーソナライゼーションアイコンを使用してパーソナライ ![](assets/do-no-localize/editor-icon.svg) ーションエディターを開きます。
1. **[!UICONTROL 決定ポリシー]** メニューに移動し、「**[!UICONTROL 決定ポリシーを追加]**」ボタンをクリックします。
...
プッシュ通知の場合、プロパティペインの **パーソナライゼーションエディター** または **決定** メニューを使用して、新しい決定ポリシーを追加できます。
...
新しい決定ポリシーをコンテンツに追加すると、決定ポリシー設定画面が開きます。 決定ポリシーを設定するには、次の手順に従います。

1. 決定ポリシーの名前を入力してから、カタログを選択します（現在はデフォルトの&#x200B;**[!UICONTROL オファー]**&#x200B;カタログに限定されています）。

   ![](assets/decision-code-based-details.png)

1. **[!UICONTROL 項目数]** フィールドでは、決定ポリシーで返す決定項目の数を定義できます。 例えば、「2」を選択した場合、現在の設定に対して最適な 2 つの実施要件を満たすオファーが表示されます。

   >[!NOTE]
   >
   >このオプションは、メールおよびコードベースのエクスペリエンスチャネルでのみ使用できます。 その他のすべてのチャネルでは、1 つのアクションにつき 1 つの決定項目のみを返すことができます。

   メールチャネルに対して複数の項目を返すには、**[!UICONTROL 繰り返しグリッド]** コンポーネント内に決定ポリシーを追加する必要があります。 詳しくは、以下の節を展開してください。
