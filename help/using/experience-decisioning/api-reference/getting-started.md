...
>[!NOTE]
>
>既存の意思決定管理オブジェクトをDecisioningに移行する必要がある場合は、専用の[Decisioning Migration API](../decisioning-migration-api.md)を使用します。 この専用APIは、サンドボックスをまたいだエンティティ移行の意思決定に特化した自動依存関係解決とロールバック機能を提供します。
