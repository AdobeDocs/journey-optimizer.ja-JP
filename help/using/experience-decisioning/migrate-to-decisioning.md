...
| 使用例 | 意思決定管理 | 決定 |
|----------|---------------------|-------------|
| **マルチプレースメント戦略** | 1 つの戦略で、ホームページとモバイルアプリの両方が強化されます | 特定のプレースメントに関連付けられた決定ロジック （web やメールの場所など） |
| **一貫したオファー属性** | マーケターは「discountType」と「offerValue」を 1 回定義します。すべてのオファーがこれらのフィールドを自動的に継承します | 各オファーは、独自の属性を手動で管理します。スキーマレベルでの一貫性はありません |
| **動的 AI ランキング** | マーケターは、売上高とエンゲージメントの目標のバランスを取るために重み付け（例：60% AI コンバージョンスコア + 40% 利益率）を調整できます | ランキングは、モデル出力または静的ルールのみに依存します |
| **A/B テスト戦略** | チームは、「AI + ビジネスルール」が「優先度ベースのランキング」よりも優れているかどうかを A/B テストできます | 組み込みの実験はサポートされていません |
| **カスタム AI 指標** | Retailerは、「購入する可能性」モデルのトレーニングを実施し、新製品と既知製品の上昇率を監視します | クリックの傾向に対してのみ最適化します。モデルの調査やリフトは表示されません。 |
| **コンテンツの再利用性** | ヘッダーまたはCTAの更新は、数百ものオファーに自動的に反映されます | 各オファーには、完全なコンテンツが個別に格納されます |
| **統合オーサリング** | マーケターは、メッセージエディターを離れることなく、パーソナライズされたオファーをメールに挿入します | 意思決定とメッセージングは、統合が制限された別々のフレームワークで行われます |
| **プライバシーコンプライアンス** | マーケターは、同意環境設定が特定のプロファイルを自動的に除外することを知って、オファールールを作成します | 実施のためにエンジニアリング・チームおよびデータ・チームとの手動による調整が必要 |
| **リアルタイムインベントリ** | 製品在庫データセットを使用して、在庫切れ項目のオファーをリアルタイムで抑制する | 静的データ。外部データセットやコンテキストデータセットを使用できる柔軟性は限られている |
| **スケールパフォーマンス** | 100 ms の応答時間未満で数百万の受信リクエストに対応するリアルタイムパーソナライゼーション | 待ち時間の長いハブでの決定 |
