...
+++**AI モデルのデータセット全体に対するオーディエンスの役割は何ですか？**

[AI モデル ](ranking/ai-models.md) を設定する場合、データセットとオーディエンスの両方が異なる目的を果たします。
...
効果的な [ パーソナライズされた最適化モデル ](ranking/personalized-optimization-model.md) モデルのパフォーマンスを得るには、両方のコンポーネントが必要です。

+++

+++**オファーコレクションを変更すると、自動最適化モデルやパーソナライズされた最適化モデルにどのような影響がありますか？**

自動最適化モデルは、パーソナライズされた最適化モデルが過去 30 日間のトラフィックデータを使用しているかどうかに関わらず、過去 14 日間のトラフィックデータに基づいて、次善のオファーへとトラフィックを提供します。

複数のオファーが同時に削除され、残りのオファーの 14 日間または 30 日間のウィンドウ内のトラフィックデータが最小限の場合、モデルは、ランダムな配分パターンや、限られたインプレッションデータに基づいて、コンバージョン率が高いオファーにバイアスをかけるなど、最適でない動作を示す場合があります。
...
* **自動最適化** は、次のトレーニングサイクルで新しいオファーを特定し、テストを開始します。 自動最適化トレーニングは 1 日に 3～4 回、約 6 時間ごとに行われます。
* **パーソナライズされた最適化** は、新しいオファーがオファー戦略に追加されると、それらを識別し、テストを開始します。 それらは、ランダムな探索トラフィックに含まれます。 その後、これらのオファーは、モデルの次のトレーニングサイクルで毎週パーソナライズされます。
...
+++**同様のオファーは、AI モデルのパフォーマンスにどのような影響を与えますか？**
