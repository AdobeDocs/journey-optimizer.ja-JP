...
# チャネルサーフェスを設定する {#message-presets-creation}
...
➡️ [E メールの表面を作成して使用する方法については、このビデオを参照してください](#video-presets)

## チャネルサーフェスを作成する {#create-message-preset}

>[!CONTEXTUALHELP]
>id="ajo_admin_message_presets"
>title="チャネルサーフェスの詳細と設定"
>abstract="チャネルサーフェスを設定することで、適用するチャネルを選択し、E メールのタイプ、使用するサブドメイン、送信者名、モバイルアプリなど、配信に必要なすべての技術的パラメーターを定義できます。"
...
1. **[!UICONTROL プッシュ通知]**&#x200B;チャネルを選択した場合は、少なくとも 1 つのプラットフォーム（**iOS** または **Android**）を選択し、各プラットフォームで使用するモバイルアプリケーションを選択します。
...
1. **[!UICONTROL SMS]** チャネルを選択した場合は、[この節](sms-configuration.md#message-preset-sms)の説明に従って設定を行ってください。
...
1. すべてのパラメーターを設定したら、「**[!UICONTROL 送信]**」をクリックして確定します。また、チャンネルサーフェスをドラフトとして保存し、後で設定を再開することもできます。
   ...
   >[!NOTE]
   >
   >選択した IP プールが下にある間は、サーフェスの作成を続行できません [版](ip-pools.md#edit-ip-pool) (**[!UICONTROL 処理中]** ステータス ) との間に存在し、選択したサブドメインに関連付けられていない。 [詳細情報](#subdomains-and-ip-pools)
   >
...
## チャネルサーフェスの監視 {#monitor-message-presets}
...
## チャネルサーフェスを編集する {#edit-message-preset}
...
変更が送信されると、チャネルサーフェスは、次の場合と同様に検証サイクルを経ます。 [チャネルサーフェスの作成](#create-message-preset). 編集の処理時間は、最大で **3 時間**&#x200B;かかる場合があります。
...
処理時間は、最大で **3 時間**&#x200B;かかる場合があります。検証サイクルで実行されるチェックについて詳しくは、[この節](#create-message-preset)を参照してください。
...
## チャネルサーフェスを非アクティブ化する {#deactivate-preset}
...
>[!NOTE]
>
>更新の処理中は、チャンネルサーフェスを非アクティブにすることはできません。 更新が正常に完了するか失敗するまで待つ必要があります。詳細情報： [チャンネルサーフェスの編集](#edit-message-preset) そして [ステータスを更新](#update-statuses).
...
チャネルサーフェスの作成方法、使用方法、サブドメインをデリゲートして IP プールを作成する方法を説明します。
