...
**このページでは、** DMARCの概要、ポリシーの仕組み、デリゲートされたサブドメインのDMARC レコードをAdobe Journey Optimizerで直接設定および管理する方法について説明します。
...
* アドビに&#x200B;**新しいサブドメインをデリゲート**&#x200B;すると、直接 [!DNL Journey Optimizer] 管理インターフェイスで **DMARC を設定**&#x200B;できるようになります。 [方法についてはこちらを参照](#set-up-dmarc)
...
1. DMARC レコードが関連付けられていないサブドメインを選択し、組織の要件に応じて **[!UICONTROL DMARC レコード]**&#x200B;セクションに入力します。 DMARC レコードフィールドに値を入力する手順について詳しくは、[この節](#set-up-dmarc)を参照してください。
   ...
   >[!NOTE]
   >
   >親ドメインで DMARC レコードが見つかるかどうかに応じて、親ドメインの値を使用するか、アドビに DMARC レコードを管理させるかを選択できます。 [詳細情報](#manage-dmarc-with-adobe)
...
1. 必要に応じて、1 つまたは複数のメールアドレスを選択して追加し、[認証に失敗した](#how-dmarc-works)メールに関する **DMARC レポート**&#x200B;が送信される組織内の場所を指定します。 レポートごとにアドレスを 5 つまで追加できます。
   ...
   >* これらの高度に技術的なレポートでは、スプーフィングが試行され、サードパーティツールを通じて最適に処理されたメールの概要が提供されます。
...
1. **レポート間隔**&#x200B;を 24 時間から 168 時間の間で選択します。 これにより、ドメイン所有者はメール認証結果に関するアップデートを定期的に受け取り、メールのセキュリティを向上させるために必要な措置を講じることができます。

### トラブルシューティング {#troubleshooting}

DMARC レコードを設定する際、DNS TXT レコードが、DMARC ポリシーを指定するドメインの DNS 設定に追加されます。

**DNS 生成タイミング**

DNS の変更がインターネットをまたいで生成されるようになるまで、通常、数分から 48 時間かかります。 DMARC 設定を変更した直後に更新を確認しようとすると、エラーが表示されたり、変更がまだ検出されなかったりする場合があります。

DMARC 設定の確認を試みる前に、DNS レコードが生成されるまで十分な時間を取ります。 48 時間経過しても引き続き問題が発生する場合は、DNS レコードがホスティングソリューションに正しく追加されていることを確認します。
