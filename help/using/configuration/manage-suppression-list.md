...
を使用 [!DNL Journey Optimizer]を使用すると、ジャーニーやキャンペーンでの送信から自動的に除外されるすべての電子メールアドレスを監視できます。以下に例を示します。
...
## 抑制リストをダウンロード {#download-suppression-list}
...
1. を選択します。 **[!UICONTROL CSV をダウンロード]** 」ボタンをクリックします。
...
1. ファイルが生成されると、通知が届きます。 画面の右上にあるベルのアイコンをクリックして、表示します。
...
>title="抑制リストにメールもしくはドメインを追加"
>abstract="抑制リストを入力するには、電子メールアドレスまたはドメインを手動で追加します。一度に 1 回、または CSV ファイルのアップロードによる一括モードです。 これらの特定のメールアドレスやドメインは、送信から除外されます。"
