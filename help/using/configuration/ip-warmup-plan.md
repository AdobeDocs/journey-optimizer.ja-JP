...
オブジェクトレベルのアクセス制御を使用すると、データを保護し、プランを表示および管理するための特定のアクセス権を付与できます。 IP ウォームアッププランにラベルが割り当てられていない場合は、すべてのユーザーが表示および編集するために開かれます。

権限の付与 **[!UICONTROL IP ウォームアッププランを表示]** 権限は、を割り当てる間、表示および公開のみにアクセスを制限します **[!UICONTROL IP ウォームアッププランの管理]** 権限を持つユーザーは、プランを表示および編集できます。
...
このアクティビティは、通常、配信品質のエキスパートが、業界のドメイン、ユースケース、地域、ISP、その他の様々な要因に基づいてよく考えられたプランの準備を支援して実行します。
...
各ドメインがドメイングループに固有であり、他のドメイングループと重複していないことを確認します。 グローバルドメイングループは自動的に定義されるので、カスタムドメイングループを作成する際は、このことを考慮する必要があります。
