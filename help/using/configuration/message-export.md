...
この機能を使用すると、書き出し用にマークされた[!DNL Journey Optimizer]経由で送信された電子メールとSMS メッセージの内容が[!DNL Experience Platform] **AJO メッセージ書き出しデータセット**&#x200B;に書き込まれます。 [詳しくは、データセットを参照してください](../data/get-started-datasets.md)
...
データセット内のすべてのレコードは、取り込みから&#x200B;**7暦日にわたって保持されます**。 この保存期間中は、コンプライアンス監査、法的問い合わせ、または設定されたExperience Platformの宛先を介して独自のストレージシステムに書き出すデータにアクセスできます。
