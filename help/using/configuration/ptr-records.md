...
In [!DNL Journey Optimizer]に値を指定しない場合、PTR レコードを手動で作成することはできません。 代わりに、 [delegate](delegate-subdomain.md) 最初のサブドメインをAdobeにすると、PTR レコードが IP 用に自動的に作成されます。

各 IP が 1 つの PTR レコードを受け取ります。 すべての PTR レコードの形式は「rxx.subdomain」です。ここで、「subdomain」は最初にデリゲートしたサブドメインです。 [!DNL Journey Optimizer].

追加のサブドメインを作成する場合は、1 つ以上の PTR レコードを変更し、その新しいサブドメインをそれらのレコードに割り当てる必要があります。 これを行うには、以下の手順に従います。
