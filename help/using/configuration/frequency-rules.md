...
>title="ビジネス ルール"
>abstract="メッセージ頻度ルールは、1 つまたは複数のチャネルをまたいで、ユーザーがメッセージを受信する回数やジャーニーにエントリする回数を制限するビジネスルールの一種です。 これらのクロスチャネルルールにより、過剰に配信を受けているプロファイルが、メッセージやアクションから自動的に除外されます。"
...
例えば、あるブランドについて、顧客に月に 4 件を超えるマーケティングメッセージを送信しないというルールを設定できます。これを行うには、ビジネスルールを使用して、月別のカレンダー期間に、1 つ以上のチャネルに基づいて送信されるメッセージの件数に上限を設定することができます。
...
## ビジネス・ルールへのアクセス {#access-rules}

ビジネスルールは、から使用できます **[!UICONTROL 管理]** > **[!UICONTROL ビジネスルール]** メニュー。 すべてのルールが、変更日順に表示されます。 フィルターアイコンを使用して、カテゴリ、ステータスまたはチャネル（あるいはその両方）に基づいてフィルターを適用します。また、メッセージラベルで検索することもできます。

![](assets/message-rules-filter.png)

### 権限{#permissions-frequency-rules}

ビジネス・ルールにアクセス、作成、編集または削除するには、 **[!UICONTROL 頻度ルールの管理]** 権限。
...
## ビジネス・ルールの作成 {#create-new-rule}

>[!CONTEXTUALHELP]
>id="ajo_rules_category"
>title="メッセージルールカテゴリの選択"
>abstract="アクティブにしてメッセージに適用すると、選択したカテゴリに一致するすべてのビジネス ルールがこのメッセージに自動的に適用されます。 現在、マーケティングカテゴリのみが使用可能です。"
...
新しいビジネス・ルールを作成するには、次の手順に従います。

1. へのアクセス **[!UICONTROL ビジネスルール]** リストを選択し、 **[!UICONTROL ルールを作成]**.
...
## ビジネス ルールのアクティブ化 {#activate-rule}

作成されたビジネス・ルールには、 **[!UICONTROL ドラフト]** ステータスおよびは、まだどのメッセージにも影響を与えていません。 有効にするには、ルールの横の楕円をクリックし、「**[!UICONTROL アクティブ化]**」を選択します。

![](assets/message-rules-activate.png)

ルールをアクティブ化すると、次回の実行時に適用されるメッセージに影響します。方法を学ぶ [メッセージにビジネス ルールを適用する](#apply-frequency-rule).

>[!NOTE]
>
>ルールが完全にアクティブ化されるまでに、最大 10 分かかる場合があります。ルールを有効にするために、メッセージを変更したり、ジャーニーを再公開したりする必要はありません。

ビジネス・ルールを非アクティブ化するには、ルールの横の省略記号をクリックして選択します **[!UICONTROL 非アクティブ化]**.
...
## メッセージにビジネス ルールを適用する {#apply-frequency-rule}
...
1. 定義したカテゴリを[作成したルール](#create-new-rule)に選択します。
   ...
   >[!NOTE]
   >
   >現在のみ **[!UICONTROL Marketing]** カテゴリは、ビジネス ルールで使用できます。
...
1. 配信から除外されたプロファイルの数は、で確認できます [グローバルレポート](../reports/global-report.md)、および [ライブレポート](../reports/live-report.md)。ビジネスルールが、配信から除外されるユーザーの理由として考えられるものとしてリストされる場合。
...
1. [ビジネス・ルールの作成](#create-new-rule) 呼び出し *全般的なマーケティングキャッピング*:
...
>[!NOTE]
>
>ビジネスルールをテストする場合は、新規作成したを使用することをお勧めします [テストプロファイル](../audience/creating-test-profiles.md)プロファイルのフリークエンシーキャップに達すると、翌月までカウンターをリセットする方法はないからです。 ルールを非アクティブ化すると、上限を設定されたプロファイルがメッセージを受け取ることはできますが、カウンターの増分は削除されません。
