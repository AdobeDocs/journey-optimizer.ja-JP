...
例えば、あるブランドについて、顧客に月に 4 件を超えるマーケティングメッセージを送信しないというルールを設定できます。これを行うには、ビジネスルールを使用して、月別のカレンダー期間中に 1 つ以上のチャネルに基づいて送信されるメッセージの件数をキャップすることができます。
...
1. **[!UICONTROL 期間]**&#x200B;ドロップダウンリストから、キャップを適用する時間枠を選択します。[詳細情報](#frequency-cap)

1. ルールのキャップを設定します。これは、上記の選択に応じて、1 つの個別ユーザープロファイルに送信できる、1 か月または 1 週間あたり<!--or day-->のメッセージの最大数を意味します。
...
1. 選択したすべてのチャネルに対して合計数としてキャップを適用する場合は、複数のチャネルを選択します。
   ...
   例えば、キャップを 15 に設定し、メールチャネルとプッシュチャネルの両方を選択します。プロファイルが既に 10 件のマーケティングメールと 5 件のマーケティングプッシュ通知を受信している場合、このプロファイルは、次に配信されるマーケティングメールまたはプッシュ通知の対象から除外されます。
...
**[!UICONTROL 期間]**&#x200B;ドロップダウンリストから、キャップを毎月に適用するか毎週に適用するかを選択します。
...
>[!NOTE]
>
>毎日のフリークエンシーキャッピングルールの精度を確保するには、[ ストリーミングセグメント化 ](https://experienceleague.adobe.com/docs/experience-platform/segmentation/ui/streaming-segmentation.html?lang=ja){target="_blank"} を使用することをお勧めします。 オーディエンスの評価方法について詳しくは、[この節](../audience/about-audiences.md#evaluation-method-in-journey-optimizer)を参照してください。
...
1. 「*全マーケティングキャップ*」という名前の[ビジネスルールを作成](#create-new-rule)します。
...
1. ユーザーに送信されるマーケティングベースのプッシュ通知の数をさらにキャップするには、2 つ目のルールとして「*プッシュマーケティングのキャップ*」という名前のルールを作成します。
...
>[!NOTE]
>
>プロファイルのフリークエンシーキャップに達すると、翌月までカウンターをリセットする方法はないので、ビジネスルールをテストする場合は、新しく作成した[テストプロファイル](../audience/creating-test-profiles.md)を使用することをお勧めします。ルールを非アクティブ化すると、キャップを設定したプロファイルがメッセージを受け取ることはできますが、カウンターの増分は削除されません。
