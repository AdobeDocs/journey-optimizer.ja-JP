...
1. チャネルサーフェスが作成されると、リストに「**[!UICONTROL 処理中]**」のステータスで表示されます。
   ...
   >[!NOTE]
   > サブドメインの E メールサーフェスを作成する場合、処理時間は次に示すように異なります。
   >
   > * の場合 **新しいサブドメイン**&#x200B;を使用する場合、最初のチャンネルサーフェスを作成するプロセスを実行します **10 分～ 10 日**.
   > * の場合 **非実稼働用サンドボックス**、または選択したサブドメインが **既に使用済み** 別の承認済みチャネルサーフェスでは、処理の対象は **3 時間**.
