...
**このページでは、**&#x200B;既存のDNS レコードを削除し、SSL証明書をアップロードし、フィードバックループを完了し、新しいDNS レコードを作成することで、メールまたはランディングページのサブドメインをCNAME委任からAdobe Journey Optimizerのカスタム委任に移行する方法について説明します。
