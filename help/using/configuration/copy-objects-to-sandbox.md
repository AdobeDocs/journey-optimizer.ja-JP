...
このページでは、Journey Optimizer のコンテキストにおけるサンドボックスツールのユースケースについて説明します。この機能自体の詳細については、Adobe Experience Platform [ サンドボックスツールガイド ](https://experienceleague.adobe.com/docs/experience-platform/sandbox/ui/sandbox-tooling.html?lang=ja#abobe-journey-optimizer-objects){target="_blank"} を参照してください。
...
1. [ ソースサンドボックス内のパッケージとしてエクスポートするオブジェクトを追加 ](#export)
1. [パッケージを公開します](#publish)
1. [ターゲットサンドボックスへのパッケージの読み込み](#import)

## エクスポートしたオブジェクトとベストプラクティス {#objects}

Journey Optimizerを使用すると、ジャーニー、カスタムアクション、コンテンツテンプレート、フラグメント、その他のオブジェクトを別のサンドボックスに書き出すことができます。 次の節では、各タイプのオブジェクトに関する情報とベストプラクティスについて説明します。
...
* **コピーされた依存関係** - ジャーニーを書き出す際には、ジャーニー自体に加えて、Journey Optimizerは、ジャーニーに依存するほとんどのオブジェクト（オーディエンス、カスタムアクション、スキーマ、イベントおよびアクション）もコピーします。 コピーされたオブジェクトについて詳しくは、Adobe Experience Platform [ サンドボックスツールガイド ](https://experienceleague.adobe.com/docs/experience-platform/sandbox/ui/sandbox-tooling.html?lang=ja#abobe-journey-optimizer-objects){target="_blank"} を参照してください。

* **手動検証をお勧めします** - リンクされたすべての要素が宛先サンドボックスにコピーされるとは限りません。 例えば、ジャーニーを公開する前に、徹底的に確認することを強くお勧めします。これにより、欠けている可能性のあるオブジェクトを特定できます。
...
* 決定ポリシーに選択戦略がある場合は、決定項目を別途追加する必要があります。 手動/フォールバックの決定項目がある場合は、直接依存関係として自動的に追加されます。

* 決定エンティティをコピーする際は、他のオブジェクトの&#x200B;**前**&#x200B;に決定項目をコピーします。例えば、最初にコレクションをコピーし、新しいサンドボックスにオファーがない場合、その新しいコレクションは空のままになります。

* 依存関係（スキーマ、セグメントなど）を持つエンティティをコピーする場合は、エンティティに対して「新規作成」をクリックして選択を解除し、依存アーティファクトに対して「既存を使用」オプションを表示します。 追加の依存関係を使用するには、階層の下方でこの手順を繰り返す必要が生じる場合があります。

  例：キャンペーンをインポート中に、ルールでデータストリームスキーマを再利用するには、DECISIONING_STRATEGY に対して「新規作成」をクリックし、次に DECISIONING_RULES で「既存を使用」オプションを表示します。

* データストリームのコンテキストスキーマに依存するエンティティの場合は、データストリームが事前に作成されていることを確認し、そのデータストリームの既存のスキーマを選択します。

* 読み込み中に「完了」を直接クリックすると、すべての依存関係が新たに作成されます。
