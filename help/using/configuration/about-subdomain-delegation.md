...
**このページでは、** Adobe Journey Optimizerでメール送信サブドメインを設定する理由と方法について説明します。ドメインのレピュテーションと配信品質を保護するために、完全なデリゲーション、CNAME、およびカスタムデリゲーションの方法を比較します。
...
| 設定方法 | 仕組み | 労力のレベル |
|---|---|---|
| **完全なデリゲーション** | サブドメインと名前空間レコードを作成します。 Adobeは、Adobe Journey Optimizerに必要なすべてのDNS レコードを設定します。<br/><br/>この設定では、AdobeがサブドメインとすべてのDNS レコードを管理する完全な責任を負います。 | 低 |
