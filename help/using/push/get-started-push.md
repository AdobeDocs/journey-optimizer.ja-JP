...
[!DNL Journey Optimizer] はプッシュ通知をサポートし、関連性の高い通知を業界最高のスループット率で送信できます。 プッシュ通知には、お客様の企業が[!DNL Adobe CX Enterprise]で得たデータ インサイトを活用するために、パーソナライゼーションとジャーニーベースのコンテキストが含まれる場合があります。
...
プッシュ通知は、アプリの利用や受信トレイの確認など、デバイス上で顧客に迅速かつ直接リーチする必要がある場合に最も効果を発揮します。

| 利点 | 理由 | ユースケースの例 |
| --- | --- | --- |
| 時間的制約のある更新 | ユーザーがアプリを積極的に使用していない場合でも、すぐに配信されます | フライト遅延アラート、注文状況の変更、最新ニュース |
| リエンゲージメント | 非アクティブな期間が経過したユーザーにアプリに戻るよう促します | カート放棄のリマインダー、ウィンバック施策 |
| コスト削減とSMS比較 | SMSとは異なり、メッセージごとの通信事業者手数料はありません | 大量のプロモーション通知やトランザクション通知 |
| リッチでインタラクティブなコンテンツ | 画像、アクションボタン、ディープリンクをサポート | タップして購入するボタン、リッチメディアプレビュー付きの製品プロモーション |
| デバイスネイティブ機能 | 他のチャネルでは利用できないOS レベルの機能を活用 | 振動アラート、アプリアイコンのバッジ、ジオフェンス設定された場所のトリガー |
| 高いオプトイン可能性 | ユーザーは、アプリのインストールまたは初回起動の早い段階でオプトインするように求められます | オンボーディングフロー、1日1回のエンゲージメント施策 |

## 使用しない場合

プッシュ通知は、あらゆるメッセージに適しているわけではありません。 次のような状況では、別のチャネルを検討してください。

* オーディエンスのプッシュオプトイン率は低いか、メッセージが届かない可能性があるため、通知に対する抵抗を示している
* メッセージには長文のコンテンツが必要です。このコンテンツは、電子メールの処理を改善し、より詳細なフォーマットを可能にします
* コンテンツは機密性の高いものやプライベートなものであり、デバイスの近くにいる人が見ることができるロック画面に表示されるべきではありません
* 多くの顧客は、モバイルアプリではなくデスクトップからサービスにアクセスします。このアプリでは、プッシュ通知のリーチが制限または制限されます
