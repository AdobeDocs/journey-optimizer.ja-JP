...
### アプリ ID をパーソナライズ （オプション） {#app-id-personalization}

>[!CONTEXTUALHELP]
>id="ajo_surface_dynamic_app_id"
>title="アプリ ID をパーソナライズ"
>abstract="複数のモバイルアプリを管理する際は、各アプリ ID をプロファイルに保存し、単一のプッシュチャネル設定を使用します。 アプリ ID フィールドの横にあるパーソナライゼーションエディターを開き、プロファイル属性を選択します。式は、送信時に各受信者に対して評価されます。 式が返す場合があるすべてのアプリ ID に対して、プッシュ資格情報が存在することを確認します。"
