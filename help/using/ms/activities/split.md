...
1. 複数ステップのキャンペーンに **分割** アクティビティを追加します。
...
1. アクティビティ設定パネルが開き、デフォルトのサブセットが表示されます。「**セグメントを追加**」ボタンをクリックして、入力母集団をセグメント化する必要な数のサブセットを追加します。
   ...
   >
   >複数手順のキャンペーンを開始する前に、ニーズに合った順序でサブセットを注文していることを確認してください。 これを行うには、矢印ボタンを使用して、サブセットの位置を変更します。

1. サブセットを追加したら、アクティビティにはサブセットと同数の出力トランジションが表示されます。複数の手順から成るキャンペーンキャンバスでサブセットを簡単に識別できるように、各サブセットのラベルを変更することを強くお勧めします。
...
1. 各サブセットで入力母集団をフィルタリングする方法を設定します。それには、次の手順に従います。
   ...
   1. 入力母集団が空の場合にトランジションを無効にするには、「**[!UICONTROL 空のトランジションをスキップ]**」オプションをオンに切り替えます。サブセットに一致するプロファイルがない場合、複数手順キャンペーンは次のアクティビティにトランジションしません。
...
これでアクティビティが設定されました。複数の手順のキャンペーンを実行する際は、アクティビティに追加された順序で、母集団が様々なサブセットにセグメント化されます。
