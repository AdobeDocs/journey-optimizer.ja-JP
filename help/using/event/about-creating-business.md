...
複数のビジネスイベントを連続で受信した場合に適用される重要な注意事項を以下に示します。

**ジャーニーの処理中にビジネスイベントを受け取った場合の動作は何ですか？**

ビジネスイベントは、単一イベントの場合と同じ方法で再入場ルールに従います。 ジャーニーが再入場を許可する場合、次のビジネスイベントが処理されます。

**マテリアライズドセグメントの過負荷を回避するガードレールは何ですか？**

ビジネスイベントの場合、トピックの再利用性は1時間に設定されます。 つまり、特定のジャーニーに対して1時間の時間枠内に新しい書き出しジョブは作成されません。 最初のイベントジョブによってプッシュされたデータは再利用されます。 スケジュールされたジャーニーの場合、ガードレールはありません。
...
1.  「**[!UICONTROL イベント ID 条件]** 」フィールド内をクリックします。簡単な式エディターを使用して、ジャーニーをトリガーするイベントを識別するためにシステムが使用する条件を定義します。
   ...
   >[!NOTE]
   >
   >シンプルな式エディターでは、すべての演算子が使用できるわけではなく、データタイプに応じて異なります。 例えば、フィールドの文字列タイプには、「次を含む」または「次と等しい」を使用できます。
