...
イベントを使用すると、ジャーニーを個別にトリガー付けし、ジャーニーにエントリした各ユーザーにリアルタイムメッセージを配信できます。
...
**単一イベント** と **ビジネスイベント** の 2 種類のイベントを設定できます。
...
**単一** イベントイベントは人物にリンクされます。 関連付ける対象は、人の行動（製品の購入、ショップへの訪問、web サイトからの離脱など）や、人物にリンクして発生した事象（10,000 ロイヤルティポイントに到達など）です。[!DNL Journey Optimizer] はジャーニーでこのイベントをリッスンして、次の最適なアクションを編成します。単一のイベントは、ルールベースとすることも、システム生成とすることもできます。単一イベントの作成方法については、この[ページ](../event/about-creating.md)を参照してください。
...
**ビジネス** イベントは、特定のプロファイルにはリンクされません。 例えば、ニュースアラート、スポーツの最新情報、航空便の変更やキャンセル、在庫の更新、気象情報などがあります。これらのイベントは一人ひとりのプロファイルに固有のものではなく、特定のニューストピックの購読者、航空便の乗客、在庫切れ商品に興味を持つ買い物客など 、関心を持つプロファイルの数は任意です。ビジネスイベントは常にルールベースです。ビジネスイベントをジャーニーにドロップすると、**オーディエンスを読み取り** アクティビティが自動的に追加されます。ビジネスイベントを作成する方法については、このページ [ を参照してください ](../event/about-creating-business.md)。

## レコメンデーション

イベントの設定は **必須** で、データエンジニアが実施する必要があります。

既存のジャーニーが壊れないように、ドラフトジャーニーまたはライブジャーニーで使用されるイベントを編集する場合は、名前、説明を変更するか、ペイロードフィールドを追加するだけです。

## イベント ID のタイプ {#event-id-type}

**ビジネス** イベントの場合、イベント ID のタイプは常にルールベースです。

**単一** イベントの場合、イベント ID には次の 2 種類があります。
...
ペイロードには、ストリーミング取得 API が機能するために必要な情報（ヘッダー内）、[!DNL Journey Optimizer] が機能するために必要な情報、ジャーニー内で使用する情報（本文内で使用する、放棄された買い物かごの金額など）が含まれます。ストリーミング取り込みには、認証済みと非認証の 2 つのモードがあります。ストリーミング取得 API について詳しくは、[ このリンク ](https://experienceleague.adobe.com/docs/experience-platform/xdm/api/getting-started.html?lang=ja){target="_blank"} を参照してください。
