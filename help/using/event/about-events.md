...
既存のジャーニーが壊れないように、ドラフト、ライブまたはクローズされたジャーニーで使用されるイベントを編集する場合は、名前、説明を変更するか、ペイロードフィールドを追加するだけです。

ライブジャーニー、ドラフトまたはクローズ済みジャーニーで使用されたイベントは、削除できません。 使用済みイベントを削除するには、そのイベントを使用しているジャーニーを停止するか、使用されているドラフトジャーニーから削除する（またはその両方）必要があります。 「**[!UICONTROL 使用されている場所]** フィールドを確認できます。 特定のイベントを使用するジャーニーの数が表示されます。 「**[!UICONTROL ジャーニーを表示]**」ボタンをクリックすると、対応するジャーニーのリストを表示できます。
