...
>[!NOTE]
>
>ドラフトまたはライブジャーニーで使用されているイベントを編集した場合は、名前、説明を変更するか、ペイロードフィールドを追加することのみ可能です。 ジャーニーが壊れないように、ドラフトまたはライブジャーニーの編集は厳密に制限します。
...
ペイロードには、ストリーミング取得 API が機能するために必要な情報（ヘッダー内）、[!DNL Journey Optimizer] が機能するために必要な情報、ジャーニー内で使用する情報（本文内で使用する、放棄された買い物かごの金額など）が含まれます。ストリーミング取得には、認証済みと未認証の 2 つのモードがあります。 ストリーミング取得 API について詳しくは、次を参照してください。 [このリンク](https://experienceleague.adobe.com/docs/experience-platform/xdm/api/getting-started.html?lang=ja).
